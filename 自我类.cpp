...
namespace {
        ...
        const std::string& 调用点) noexcept
    {
        ...
        (void)任务类::写入任务节点种类(步骤, 枚举_任务节点种类::步骤节点, 步骤->主信息.场景.指针, now);
...
bool 自我类::更新待处理方法数量(I64 数量, 时间戳 now, const std::string& 调用点) noexcept
{
    if (数量 < 0) {
        数量 = 0;
    }
...
bool 自我类::更新方法补齐状态(bool 正在方法补齐, 时间戳 now, const std::string& 调用点) noexcept
{
    I64 旧值 = 0;
    (void)读取自我I64特征当前值(
        自我特征定义类::类型_自我_方法补齐状态(),
...
void 自我类::初始化自我特征与默认值_(const std::string& 调用点) noexcept
{
    ...
    确保自我I64特征已初始化_(自我存在, 自我特征定义类::类型_自我_待处理方法数量(), 0, 调用点 + "/待处理方法数量");
    确保自我I64特征已初始化_(自我存在, 自我特征定义类::类型_自我_可用方法数量(), 0, 调用点 + "/可用方法数量");
    确保自我I64特征已初始化_(自我存在, 自我特征定义类::类型_自我_外设可用性(), 0, 调用点 + "/外设可用性");
    确保自我I64特征已初始化_(自我存在, 自我特征定义类::类型_自我_方法补齐状态(), 0, 调用点 + "/方法补齐状态");
    ...
    const std::string& 调用点) noexcept
{
    ...
    auto& 主信息 = 治理任务->主信息;
    私有_写入引用(主信息.对应需求, 当前主需求);
    if (当前主需求 && 任务类::读取任务调度优先级(治理任务) == 0) {
        (void)任务类::写入任务调度优先级(
            治理任务,
            当前主需求->主信息.服务权重 != 0
            ? 当前主需求->主信息.服务权重
