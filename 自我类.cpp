...
namespace {
    ...
    enum class 枚举_自我根特征类型_自我类 {
        安全,
        服务,
    };

    const 语素入口节点类* 私有_抽象特征名词_自我类(const char* 文本)
    {
        return 语素集.添加信息入口词(
            文本 ? 文本 : "",
            枚举_信息入口类型::特征模板入口);
    ...
    const 语素入口节点类* 私有_自我根特征类型词_自我类(
        const 枚举_自我根特征类型_自我类 类型) noexcept
    {
        switch (类型) {
        case 枚举_自我根特征类型_自我类::安全:
            return 自我特征定义类::类型_自我_安全值();
        case 枚举_自我根特征类型_自我类::服务:
    ...
    bool 私有_读取自我根特征抽象目标值_自我类(
        const 枚举_自我根特征类型_自我类 根特征类型,
        I64& 输出目标值) noexcept
    {
        auto* 抽象根 = 世界树.抽象特征根;
        const auto* 自我特征类型 = 私有_自我根特征类型词_自我类(根特征类型);
        if (!抽象根 || !自我特征类型) {
            return false;
        }

        auto* 根定义节点 = 世界树.特征().查找子抽象特征_按类型(
            抽象根,
            自我特征类型);
        if (!根定义节点 || !世界树.特征().取抽象特征主信息(根定义节点)) {
            return false;
        }

        auto* 目标状态节点 = 世界树.特征().查找子抽象特征_按类型(
            根定义节点,
            私有_抽象特征名词_自我类("高位"));
        const auto* 目标主信息 = 世界树.特征().取抽象特征主信息(目标状态节点);
        I64区间 目标区间{ 0, -1 };
        if (!私有_读取抽象特征I64区间_自我类(目标主信息, 目标区间)) {
            return false;
        }
    ...
    I64 私有_自我根特征目标差值_自我类(
        const 枚举_自我根特征类型_自我类 根特征类型,
        const I64 当前值) noexcept
    {
        I64 目标值 = 0;
        if (!私有_读取自我根特征抽象目标值_自我类(根特征类型, 目标值)) {
            return 1;
        }
        return 私有_饱和减(目标值, 当前值);
    }
    ...
    方法节点* 私有_查找本能方法首节点(const 自我类& 自我对象, 枚举_本能方法ID 本能ID) noexcept
    {
        ...
        const auto* 因动作名称 = 私有_本能动作名称词(本能ID);
        ...
        return 私有_查找直接子节点_按谓词<方法节点>(方法根节点, [&](const 方法节点* 节点) noexcept {
            ...
            return 因动作名称 && 私有_语素入口相同(主信息.首节点信息().动作名, 因动作名称);
        });
    }

    const 语素入口节点类* 私有_方法镜像特征词(const char* 名称) noexcept
    {
        return 语素集.添加信息入口词(
            名称 ? 名称 : "",
            枚举_信息入口类型::特征模板入口);
    ...
    I64 私有_统计方法条件结果对数量_镜像(const 方法节点* 方法头) noexcept
    {
        if (!方法头 || !方法头->子) {
            return 0;
        }
    ...
    bool 私有_方法达到可用镜像口径(const 方法节点* 方法头) noexcept
    {
        if (!方法头) {
            return false;
        }

        const auto* 方法虚拟存在 = 方法头->主信息.首节点信息().方法虚拟存在.指针;
        const I64 最近连续成功次数 = 私有_读取方法I64特征_镜像(
            方法虚拟存在,
            私有_方法镜像特征词("方法最近连续成功次数"),
            0);
        if (最近连续成功次数 > 3) {
            return true;
        }

        return 私有_统计方法条件结果对数量_镜像(方法头) > 0;
    }
        ...
        时间戳 now) noexcept
    {
        ...
        if (auto* 来源方法 = 私有_查找本能方法首节点(自我对象, 本能ID)) {
            (void)世界树.动态().绑定来源方法(
                ...
                来源方法->主信息.首节点信息().动作名 ? 来源方法->主信息.首节点信息().动作名 : 因动作名称,
                枚举_动作事件相位::完成);
        }
...
需求类::节点类* 自我类::获取安全根需求() const noexcept
{
    ...
    const auto* 目标特征类型 = 自我特征定义类::类型_自我_安全值();
    return 私有_查找直接子节点_按谓词<需求类::节点类>(需求根节点, [&](const 需求类::节点类* 节点) noexcept {
        return 私有_需求目标特征相同(节点, 目标特征类型);
    });
}
...
需求类::节点类* 自我类::获取服务根需求() const noexcept
{
    ...
    auto* 需求根节点 = 世界树.存在().获取需求根节点(自我存在);
    const auto* 目标特征类型 = 自我特征定义类::类型_自我_服务值();
    return 私有_查找直接子节点_按谓词<需求类::节点类>(需求根节点, [&](const 需求类::节点类* 节点) noexcept {
        return 私有_需求目标特征相同(节点, 目标特征类型);
    });
}
...
bool 自我类::增加可用方法数量(I64 增量, 时间戳 now) noexcept
{
    ...
    // 可用方法数量是内部统计值，不进入本能方法动作动态链。
    return 已变化;
}

I64 自我类::统计结构可用方法数量() const noexcept
{
    auto* 自我存在 = const_cast<存在节点类*>(获取自我存在());
    auto* 方法根节点 = 自我存在
    ...
    for (auto* 当前 = reinterpret_cast<方法节点*>(方法根节点->链下);
         当前 && 当前 != 方法根节点 && 保护 < 16384;
         当前 = reinterpret_cast<方法节点*>(当前->链下), ++保护) {
...
bool 自我类::同步可用方法数量镜像(I64 统计数量, 时间戳 now) noexcept
{
    if (统计数量 < 0) {
        统计数量 = 0;
    }
...
结构_自我根需求压力评估 自我类::评估根需求压力() const noexcept
{
    ...
    评估.安全根需求 = 获取安全根需求();
    评估.服务根需求 = 获取服务根需求();
    评估.安全差值 = 私有_自我根特征目标差值_自我类(
        枚举_自我根特征类型_自我类::安全,
        获取安全值());
    评估.服务差值 = 私有_自我根特征目标差值_自我类(
        枚举_自我根特征类型_自我类::服务,
        获取服务值());
...
{
    auto* 自我存在 = 确保自我存在_();
    if (鱼巢_不允许空指(自我存在, "自我类/自我根需求与根任务初始化/自我存在缺失")) {
    ...
    auto* 任务根节点 = 世界树.存在().获取任务根节点(自我存在);
    auto* 方法根节点 = 世界树.存在().获取方法根节点(自我存在);
    const bool 需求根缺失 = 鱼巢_不允许空指(需求根节点, "自我类/自我根需求与根任务初始化/需求根节点缺失");
    const bool 任务根缺失 = 鱼巢_不允许空指(任务根节点, "自我类/自我根需求与根任务初始化/任务根节点缺失");
    const bool 方法根缺失 = 鱼巢_不允许空指(方法根节点, "自我类/自我根需求与根任务初始化/方法根节点缺失");
    if (需求根缺失 || 任务根缺失 || 方法根缺失) {
        私有_报告自我结构根创建失败(
            自我存在,
            需求根节点 != nullptr,
            任务根节点 != nullptr,
    ...
    auto* 安全值目标方法 = 方法类::注册需求位方法首节点(
        方法根节点,
        自我特征定义类::类型_自我_安全值(),
        枚举_方法来源::运行时方法沉淀,
        now);
    auto* 服务值目标方法 = 方法类::注册需求位方法首节点(
        方法根节点,
        自我特征定义类::类型_自我_服务值(),
        枚举_方法来源::运行时方法沉淀,
        now);
    项目运行日志(
        "自我类/根目标方法头确认"
        " | 安全值方法=" + std::string(安全值目标方法 ? 安全值目标方法->获取主键() : "空")
        + " | 服务值方法=" + std::string(服务值目标方法 ? 服务值目标方法->获取主键() : "空"));
    ...
    I64 默认权重) noexcept
{
    ...
    const bool 是安全根需求 = 私有_语素入口相同(目标特征类型, 自我特征定义类::类型_自我_安全值());
    const bool 是服务根需求 = 私有_语素入口相同(目标特征类型, 自我特征定义类::类型_自我_服务值());
    if (是安全根需求 || 是服务根需求) {
        const I64 当前值 = 是安全根需求 ? 获取安全值() : 获取服务值();
        I64 目标值 = 当前值;
        (void)私有_读取自我根特征抽象目标值_自我类(
            是安全根需求
                ? 枚举_自我根特征类型_自我类::安全
                : 枚举_自我根特征类型_自我类::服务,
            目标值);
