...
{
    return 写入自我指针特征当前值(自我特征定义类::类型_自我_当前主需求(), 节点, now, 调用点);
}

bool 自我类::设置当前主任务(任务类::节点类* 节点, 时间戳 now, const std::string& 调用点) noexcept
{
    return 写入自我指针特征当前值(自我特征定义类::类型_自我_当前主任务(), 节点, now, 调用点);
}

bool 自我类::设置当前主方法(方法类::节点类* 节点, 时间戳 now, const std::string& 调用点) noexcept
{
    return 写入自我指针特征当前值(自我特征定义类::类型_自我_当前主方法(), 节点, now, 调用点);
}
...
void 自我类::初始化自我特征与默认值_(const std::string& 调用点) noexcept
{
    ...
    确保自我I64特征已初始化_(自我存在, 自我特征定义类::类型_自我_物理安全(), 默认最大安全值, 调用点 + "/物理安全");
