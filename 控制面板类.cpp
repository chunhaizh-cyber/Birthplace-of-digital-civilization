...
namespace {
        ...
        const 可解析引用<任务节点类>& 引用) noexcept
    {
        ...
        // 回归诊断只认当前运行态中已经绑定的指针，避免只读诊断触发隐式创建。
...
std::string 渲染需求树生长摘要(const 结构_控制面板快照& 快照)
{
    ...
    输出 << "需求树生长摘要\n"
        ...
        << " | 工作包=" << 快照.任务管理界面线程累计派发工作包数
        << " | 工作结果=" << 快照.任务管理界面线程累计收到工作结果数
        << " | 上行=" << 快照.任务管理界面线程累计上行消息数
        << " | 当前等待工作包=" << 快照.任务管理界面线程当前等待工作包数
        << '\n';
    return 输出.str();
}
...
枚举_控制面板命令 解析控制面板命令行(const int argc, char** argv) noexcept
{
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (参数 == "--demand-tree-growth-check"
            || 参数 == "--demand-growth-check"
            || 参数 == "--need-tree-growth-check") {
            输出 = 枚举_控制面板命令::检查需求树生长;
        }
        else if (参数 == "--internal-loop-check"
            || 参数 == "--self-internal-loop-check"
            || 参数 == "--self-loop-check") {
            输出 = 枚举_控制面板命令::检查内部循环闭环;
        }
        else if (参数 == "--demand-difference-refresh-check"
            || 参数 == "--demand-diff-refresh-check"
            || 参数 == "--need-difference-refresh-check") {
            输出 = 枚举_控制面板命令::检查需求差距刷新;
        }
        else if (参数 == "--demand-difference-child-check"
            || 参数 == "--demand-diff-child-check"
        ...
        else if (参数 == "--learning-demand-execution-entry-check"
            || 参数 == "--learning-demand-entry-deficit-check"
            || 参数 == "--method-learning-execution-entry-check") {
            输出 = 枚举_控制面板命令::检查学习需求缺执行入口入树;
        }
        else if (参数 == "--learning-demand-condition-result-check"
            || 参数 == "--learning-demand-condition-pair-check"
            || 参数 == "--method-learning-condition-result-check") {
            输出 = 枚举_控制面板命令::检查学习需求缺条件结果对入树;
        }
        else if (参数 == "--learning-demand-placeholder-followup-check"
            || 参数 == "--learning-demand-placeholder-check"
            || 参数 == "--method-learning-placeholder-followup-check") {
            输出 = 枚举_控制面板命令::检查学习需求占位方法后具体入树;
        }
        else if (参数 == "--learning-demand-execution-entry-closure-check"
            || 参数 == "--learning-demand-entry-deficit-closure-check"
            || 参数 == "--method-learning-execution-entry-closure-check") {
            输出 = 枚举_控制面板命令::检查学习需求缺执行入口闭合;
        }
        else if (参数 == "--learning-demand-condition-result-closure-check"
            || 参数 == "--learning-demand-condition-pair-closure-check"
            || 参数 == "--method-learning-condition-result-closure-check") {
            输出 = 枚举_控制面板命令::检查学习需求缺条件结果对闭合;
        }
        else if (参数 == "--learning-demand-placeholder-followup-closure-check"
            || 参数 == "--learning-demand-placeholder-closure-check"
            || 参数 == "--method-learning-placeholder-followup-closure-check") {
            输出 = 枚举_控制面板命令::检查学习需求占位方法后具体闭合;
        }
        else if (参数 == "--task-method-multicandidate-selection-check"
            || 参数 == "--task-method-selection-check"
            || 参数 == "--method-multicandidate-selection-check") {
            输出 = 枚举_控制面板命令::检查任务方法多候选选择;
        }
        else if (参数 == "--formal-settlement-gate-check"
            || 参数 == "--d0-d1-actual-settlement-gate-check"
