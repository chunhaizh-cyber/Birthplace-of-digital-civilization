...
namespace {
    ...
    std::string 私有_线程恢复摘要(const 结构_控制面板快照& 快照)
    {
        ...
        if (快照.任务管理恢复存在待消费学习回流 || 快照.任务管理恢复存在待消费外部反馈) {
            输出 << " | 待消费="
                << "学习回流=" << (快照.任务管理恢复存在待消费学习回流 ? "是" : "否")
    ...
    std::size_t 树广度上限)
{
    ...
    快照.自我线程解封前验收通过 = 自我线程快照.解封前验收通过;
    快照.自我线程重外部输入保持封闭 = 自我线程快照.重外部输入保持封闭;
    快照.自我线程动作最近摘要 = 自我线程快照.动作最近摘要;
    快照.自我线程解封前验收摘要 = 自我线程快照.解封前验收摘要;
    ...
    快照.任务管理锚点裁决摘要 = 自我线程快照.任务管理锚点裁决摘要;
    ...
    快照.任务管理恢复点类型 = 自我线程快照.任务管理恢复点类型;
    快照.任务管理恢复投影摘要 = 自我线程快照.任务管理恢复投影摘要;
    快照.任务管理恢复存在待消费学习回流 = 自我线程快照.任务管理恢复存在待消费学习回流;
    快照.任务管理恢复存在待消费外部反馈 = 自我线程快照.任务管理恢复存在待消费外部反馈;
    快照.治理mailbox待消费数 = 自我线程快照.治理mailbox待消费数;
    快照.治理mailbox最老等待微秒 = 自我线程快照.治理mailbox最老等待微秒;
    快照.治理mailbox可恢复事件数 = 自我线程快照.治理mailbox可恢复事件数;
    ...
    快照.治理mailbox摘要 = 自我线程快照.治理mailbox摘要;
    快照.治理mailbox类型计数摘要 = 自我线程快照.治理mailbox类型计数摘要;
    快照.治理mailbox最老等待摘要 = 自我线程快照.治理mailbox最老等待摘要;
    快照.治理mailbox最近拦截摘要 = 自我线程快照.治理mailbox最近拦截摘要;
    快照.治理mailbox待恢复摘要 = 自我线程快照.治理mailbox待恢复摘要;
    快照.最近消费治理事件摘要 = 自我线程快照.最近消费治理事件摘要;
    快照.最近消费治理事件优先级 = 自我线程快照.最近消费治理事件优先级;
    快照.最近消费治理事件幂等键 = 自我线程快照.最近消费治理事件幂等键;
    快照.最近消费治理事件重放代数 = 自我线程快照.最近消费治理事件重放代数;
    快照.最近消费治理事件消费次数 = 自我线程快照.最近消费治理事件消费次数;
    快照.最近消费治理事件结果 = 自我线程快照.最近消费治理事件结果;
