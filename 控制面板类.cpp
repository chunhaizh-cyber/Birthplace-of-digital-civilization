...
namespace {
    ...
    {
        if (!节点) {
            return 私有_新节点("空需求列表项");
        }
        if (!私有_尝试读取需求节点主键_控制面板(节点, nullptr)) {
            return 私有_新节点(
                std::string("异常需求列表项 | 已停止读取详情 | 地址=")
                    + 私有_十六进制指针(私有_地址(节点)),
                私有_地址(节点),
                false,
                false);
        ...
        auto 列表项 = 私有_新节点(
            std::string("需求列表项 详情 | ") + 私有_需求标题(节点),
            私有_地址(节点),
            true,
            false);
        try {
            ...
            日志 << "控制面板/需求列表详情读取失败"
                ...
                << " | 异常=未知异常";
            项目运行错误日志(日志.str());
            列表项.子项.push_back(私有_新节点("需求详情读取失败 | 异常=未知异常"));
        }
        if (列表项.子项.empty()) {
            列表项.子项.push_back(私有_新节点("需求详情为空"));
        }
        return 列表项;
    }
    ...
    const 需求节点* 私有_查找根需求_按类型文本_控制面板(const char* 类型文本) noexcept
    {
        if (!类型文本 || !*类型文本 || !世界树.自我指针) {
            return nullptr;
        }
    ...
    std::string 私有_推断自检父需求选择原因_控制面板(const 需求节点* 节点)
    {
        ...
        const auto 父类型 = 私有_词文本(父需求->主信息.类型);
        if (父类型 == "安全根需求") {
            return "安全根";
        }
        if (父类型 == "服务根需求") {
        ...
        const 需求类::结构_需求派生归因兼容视图& 派生归因)
    {
        ...
        const auto* 固定来源需求 = 私有_查找根需求_按类型文本_控制面板("安全根需求");
