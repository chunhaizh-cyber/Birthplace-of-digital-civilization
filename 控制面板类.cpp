...
namespace {
    ...
    std::string 私有_需求显示名_控制面板(const 需求节点* 节点)
    {
        ...
        if (私有_需求是根层节点_控制面板(节点)
            && 全局_语素入口同一(目标特征类型, 自我特征定义类::类型_自我_安全值())) {
            return "安全根需求";
        }
        if (私有_需求是根层节点_控制面板(节点)
            && 全局_语素入口同一(目标特征类型, 自我特征定义类::类型_自我_服务值())) {
            return "服务根需求";
        }
        ...
        路径集合& 已访问) noexcept
    {
        ...
        if (当前方法节点->主信息.公共.节点种类 == 枚举_方法节点种类::方法首节点
            ...
            && std::any_of(
                ...
                [&](const 结构_方法结果项& 结果项) {
                    return 全局_语素入口同一(结果项.关系目标特征类型(), 目标特征类型);
                })) {
    ...
    std::string 私有_推断自检父需求选择原因_控制面板(const 需求节点* 节点)
    {
        ...
        if (私有_需求是根层节点_控制面板(父需求)
            && 全局_语素入口同一(父目标特征, 自我特征定义类::类型_自我_安全值())) {
            return "安全根";
        }
        if (私有_需求是根层节点_控制面板(父需求)
            && 全局_语素入口同一(父目标特征, 自我特征定义类::类型_自我_服务值())) {
            return "服务根";
        }
    ...
    if (展开类型 == "thread-self"
        ...
        if (展开类型 == "thread-self") {
            ...
            私有_追加叶字段(
                ...
                "最近任务工作线程原因",
                私有_控制面板任务工作线程显示文本(
                    私有_截断文本(界面线程快照.最近后台worker原因, 200)));
