...
namespace {
    ...
    {
        return "起=" + 时间戳文本(时间段.起) + " | 止=" + 时间戳文本(时间段.止);
    }
    ...
    std::string 私有_任务状态数量摘要(const 结构_控制面板快照& 快照)
    {
        std::ostringstream 输出;
        bool 已写 = false;
        追加非零文本计数项(输出, 已写, "未定义", 快照.任务状态未定义数);
        追加非零文本计数项(输出, 已写, "未启动", 快照.任务状态未启动数);
        追加文本计数项(输出, 已写, "运行中", 快照.任务状态运行中数);
        追加非零文本计数项(输出, 已写, "挂起", 快照.任务状态挂起数);
        if (快照.任务数 > 0) {
            追加文本计数项(输出, 已写, "完成", 快照.任务状态完成数);
        }
        else {
            追加非零文本计数项(输出, 已写, "完成", 快照.任务状态完成数);
        }
        追加非零文本计数项(输出, 已写, "已结算", 快照.任务状态已结算数);
        追加非零文本计数项(输出, 已写, "失败", 快照.任务状态失败数);
        追加非零文本计数项(输出, 已写, "取消", 快照.任务状态取消数);
        追加非零文本计数项(输出, 已写, "超时", 快照.任务状态超时数);
        追加非零文本计数项(输出, 已写, "就绪", 快照.任务状态就绪数);
        追加非零文本计数项(输出, 已写, "执行中", 快照.任务状态执行中数);
        追加非零文本计数项(输出, 已写, "筹办中", 快照.任务状态筹办中数);
        追加非零文本计数项(输出, 已写, "排队中", 快照.任务状态排队中数);
        追加非零文本计数项(输出, 已写, "等待中", 快照.任务状态等待中数);
        追加非零文本计数项(输出, 已写, "无法执行", 快照.任务状态无法执行数);
        追加非零文本计数项(输出, 已写, "待重筹办", 快照.任务状态待重筹办数);
        return 已写 ? 输出.str() : "暂无任务状态";
    }

    // 功能：服务所在模块的内部辅助流程。
    std::string 私有_需求状态数量摘要(const 结构_控制面板快照& 快照)
    {
        std::ostringstream 输出;
        输出 << "本轮已截止=" << 快照.需求本轮已截止数
    ...
    // 功能：判断节点能否作为自我场景复现特征读取宿主。
    bool 私有_场景复现特征宿主可读(const 基础信息节点类* 宿主) noexcept
    {
        if (!宿主 || !宿主->主信息) {
            return false;
        }
        return dynamic_cast<const 场景节点主信息类*>(宿主->主信息)
            || dynamic_cast<const 存在节点主信息类*>(宿主->主信息);
    }
    ...
    基础信息节点类* 私有_选择自我场景复现宿主(场景节点类* 自我所在场景) noexcept
    {
        ...
        while (!栈.empty() && 已扫描 < 4096) {
            ...
            const int 当前评分 = 私有_场景复现特征宿主可读(当前)
                ? 私有_场景复现宿主评分(当前)
                : 0;
            I64 当前深度帧号 = -1;
        ...
        基础信息节点类* 复现宿主) noexcept
    {
        ...
        快照.自我场景存在样例摘要 = 场景子树统计.存在样例.empty()
            ? std::string("空")
            : 文本列表摘要(场景子树统计.存在样例, 6);
        快照.自我场景复现宿主存在样例摘要 = 宿主子树统计.存在样例.empty()
            ? std::string("空")
            : 文本列表摘要(宿主子树统计.存在样例, 6);
    }
    ...
    {
        auto 统计节点 = 私有_新节点("统计");
        私有_追加叶字段(统计节点, "创建时间", 时间戳文本(统计.创建时间));
        私有_追加叶字段(统计节点, "最后观测时间", 时间戳文本(统计.最后观测时间));
    ...
    std::vector<结构_控制面板树节点> 私有_构建方法主信息详情(方法节点* 节点)
    {
        ...
        私有_追加等号字段(字段节点, "公共.统计.创建时间", 时间戳文本(节点->主信息.公共.统计.创建时间));
        私有_追加等号字段(字段节点, "公共.统计.最后观测时间", 时间戳文本(节点->主信息.公共.统计.最后观测时间));
        ...
        if (方法类::方法是首节点(节点)) {
            ...
            私有_追加等号字段(字段节点, "角色.条件节点.时间起", 时间戳文本(条件信息->时间起));
            私有_追加等号字段(字段节点, "角色.条件节点.时间止", 时间戳文本(条件信息->时间止));
        }
        else if (结果信息) {
    ...
    std::vector<结构_控制面板树节点> 私有_构建任务主信息详情(任务节点* 节点)
    {
        ...
        私有_追加等号字段(字段节点, "创建时间", 时间戳文本(节点->主信息.创建时间));
        私有_追加等号字段(字段节点, "启动时间", 时间戳文本(节点->主信息.启动时间));
        私有_追加等号字段(字段节点, "完成时间", 时间戳文本(节点->主信息.完成时间));
        ...
        const 结构_构建上下文& 上下文)
    {
        ...
        私有_追加等号字段(字段节点, "需求有效截止", 时间戳文本(节点->主信息.需求有效截止));
        ...
        const auto* 父需求 = reinterpret_cast<const 需求节点*>(节点->父);
        const auto 父子角色 = 需求类::读取父子关系角色视图(父需求, 节点);
        ...
        私有_追加等号字段(字段节点, "最近结算时间", 时间戳文本(节点->主信息.最近结算时间));

        私有_追加等号字段(字段节点, "统计.创建时间", 时间戳文本(节点->主信息.统计.创建时间));
        私有_追加等号字段(字段节点, "统计.最后观测时间", 时间戳文本(节点->主信息.统计.最后观测时间));
    ...
    void 私有_追加状态节点字段(
        ...
        const 路径集合& 路径)
    {
        私有_追加叶字段(字段节点, "收到时间", 时间戳文本(主信息.收到时间));
        私有_追加叶字段(字段节点, "发生时间", 时间戳文本(主信息.发生时间));
        ...
        const 路径集合& 路径)
    {
        ...
        私有_追加叶字段(字段节点, "分步时长", 时间戳文本(主信息.分步时长));
        私有_追加叶字段(字段节点, "总时长", 时间戳文本(主信息.总时长));
        私有_追加叶字段(字段节点, "窗口时长", 时间戳文本(主信息.窗口时长));
        ...
        const 路径集合& 路径)
    {
        ...
        私有_追加叶字段(字段节点, "最近命中时间", 时间戳文本(主信息.最近命中时间));
        私有_追加叶字段(字段节点, "最近失败时间", 时间戳文本(主信息.最近失败时间));
        私有_追加引用字段(
            字段节点,
            "因方法模板",
            主信息.因方法模板,
        ...
        const bool 列表模式)
    {
        ...
        私有_追加叶字段(字段节点, "需求有效截止", 时间戳文本(节点->主信息.需求有效截止));
        ...
        路径集合 路径)
    {
        ...
        私有_追加叶字段(字段节点, "创建时间", 时间戳文本(节点->主信息.创建时间));
        ...
        私有_追加叶字段(字段节点, "启动时间", 时间戳文本(节点->主信息.启动时间));
        私有_追加叶字段(字段节点, "完成时间", 时间戳文本(节点->主信息.完成时间));
        ...
        路径集合 路径)
    {
        ...
        私有_追加叶字段(字段节点, "时间起", 时间戳文本(条件信息 ? 条件信息->时间起 : 0));
        私有_追加叶字段(字段节点, "时间止", 时间戳文本(条件信息 ? 条件信息->时间止 : 0));
        ...
        const 时间戳 详情加载时间)
    {
        ...
        私有_追加线程显示详情(
            ...
            生命周期,
            运行状态,
            线程.创建时间 == 0 ? std::string("未记录") : 时间戳文本(线程.创建时间),
            起止微秒时长文本(线程.创建时间, 结束时间),
            详情加载时间,
        ...
        const std::string& 说明)
    {
        ...
        私有_追加叶字段(字段节点, "详情加载时间", 时间戳文本(详情加载时间));
        ...
        const 任务管理工作线程::结构_工作线程实例快照& 工作线程快照)
    {
        ...
        私有_追加线程显示详情(
            ...
            工作线程快照.最近开始时间 == 0
                ? std::string("未记录")
                : 时间戳文本(工作线程快照.最近开始时间),
            起止微秒时长文本(工作线程快照.最近开始时间, 工作线程结束时间),
            详情加载时间,
            工作线程快照.最近推进阶段.empty()
...
SELECT
    ...
    CONVERT(nvarchar(1), COALESCE([本轮已截止], 0)) AS [本轮已截止],
    CONVERT(nvarchar(19), [更新时间], 120) AS [更新时间],
    COALESCE([显示摘要], N'') AS [显示摘要]
FROM [鱼巢].[当前需求显示项]
...
COALESCE([辅助文本], N'') AS [辅助文本])SQL";
    ...
    std::string 私有_SQL控制面板区段刷新JSON(std::string_view 区段ID)
    {
        ...
        if (区段ID == "currentDemands") {
            return 私有_SQL控制面板表格区段刷新JSON(区段ID, "当前需求", { "需求", "父需求", "满足掩码", "本轮已截止", "更新时间", "摘要" }, 数据.当前需求);
        }
...
<body>
        ...
        私有_追加SQL控制面板表(输出, "当前需求", "currentDemands", { "需求", "父需求", "满足掩码", "本轮已截止", "更新时间", "摘要" }, 数据.当前需求);
...
function treeLabel(row){
  ...
  if(name)parts.push(`<span class="tree-title">名称=${escapeHtml(name)}</span>`);
  else if(display)parts.push(`<span class="tree-title">${escapeHtml(display)}</span>`);
...
window.__panelApplyDetail=function(){};
    ...
    // 功能：把整数值混入自我场景复现稳定签名。
    void 私有_混合自我场景复现签名(std::uint64_t& 签名, const std::uint64_t 值) noexcept
    {
        签名 ^= 值 + 0x9E3779B97F4A7C15ull + (签名 << 6) + (签名 >> 2);
    }

    // 功能：把文本值混入自我场景复现稳定签名。
    void 私有_混合自我场景复现签名(std::uint64_t& 签名, const std::string& 文本) noexcept
    {
        for (const unsigned char 字符 : 文本) {
            私有_混合自我场景复现签名(签名, static_cast<std::uint64_t>(字符));
        }
        私有_混合自我场景复现签名(签名, static_cast<std::uint64_t>(文本.size()));
    }

    // 功能：根据只读控制面板快照生成自我场景复现稳定签名。
    std::uint64_t 私有_自我场景复现签名(const 结构_控制面板快照& 快照) noexcept
    {
        std::uint64_t 签名 = 1469598103934665603ull;
        const auto 混合I64 = [&](const I64 值) noexcept {
            私有_混合自我场景复现签名(签名, static_cast<std::uint64_t>(值));
        };
        const auto 混合U64 = [&](const std::uint64_t 值) noexcept {
            私有_混合自我场景复现签名(签名, 值);
        };
        const auto 混合文本 = [&](const std::string& 文本) noexcept {
            私有_混合自我场景复现签名(签名, 文本);
        };

        混合U64(快照.自我所在场景指针);
        混合U64(快照.自我场景复现宿主指针);
        混合U64(快照.自我场景安全评估宿主指针);
        混合文本(快照.自我所在场景标题);
        混合文本(快照.自我场景复现宿主标题);
        混合文本(快照.自我场景安全评估宿主标题);
        混合I64(快照.自我场景直接子节点数量);
        混合I64(快照.自我场景子树节点数量);
        混合I64(快照.自我场景直接存在数量);
        混合I64(快照.自我场景子树存在数量);
        混合I64(快照.自我场景复现宿主子树节点数量);
        混合I64(快照.自我场景当前观察帧);
        混合I64(快照.自我场景深度帧号);
        混合I64(快照.自我场景彩色帧号);
        混合I64(快照.自我场景相机帧宽度);
        ...
        混合I64(快照.自我场景观察存在假设);
        混合I64(快照.自我场景来源空间候选编号);
        混合I64(快照.自我场景中心空间坐标X);
        混合I64(快照.自我场景中心空间坐标Y);
        混合I64(快照.自我场景中心空间坐标Z);
        混合I64(快照.自我场景范围坐标AABB最小X);
        混合I64(快照.自我场景范围坐标AABB最小Y);
        混合I64(快照.自我场景范围坐标AABB最小Z);
        混合I64(快照.自我场景范围坐标AABB最大X);
        混合I64(快照.自我场景范围坐标AABB最大Y);
        混合I64(快照.自我场景范围坐标AABB最大Z);
        混合I64(快照.自我场景像素归属率);
        混合I64(快照.自我场景深度一致率);
        混合I64(快照.自我场景范围内点比例);
        混合I64(快照.自我场景投影覆盖率);
        混合I64(快照.自我场景轮廓吻合率);
        混合I64(快照.自我场景已验证观察存在数量);
        混合I64(快照.自我场景诊断区域数量);
        混合I64(快照.自我场景当前场景评估安全值候选);
        混合I64(快照.自我场景当前场景安全度候选);
        混合I64(快照.自我场景当前场景安全判定状态);

        混合I64(快照.自我场景体素快照有效 ? 1 : 0);
        // 体素版本是本次只读同步序号，每次读取都会递增，不作为画面内容变化依据。
        混合I64(快照.自我场景体素坐标系版本);
        混合I64(快照.自我场景体素枚举存在数量);
        混合I64(快照.自我场景体素有坐标存在数量);
        混合I64(快照.自我场景体素有体素存在数量);
        混合I64(快照.自我场景体素入选存在数量);
        混合I64(快照.自我场景体素缺口数量);
        混合I64(快照.自我场景存在复现项列表.size());
        for (const auto& 项 : 快照.自我场景存在复现项列表) {
            混合U64(项.节点指针);
            混合文本(项.标题);
            混合文本(项.类型);
            混合I64(项.来源范围);
    ...
    const bool 读取自我场景快照)
{
    ...
    if (auto* 当前主需求节点 = 私有_解析当前树节点_可写(自我.获取当前主需求())) {
        ...
        快照.需求树当前主需求父子关系角色视图 =
            需求类::父子需求关系角色文本(
                需求类::读取父子关系角色视图(
                    reinterpret_cast<const 需求节点*>(当前主需求节点->父),
                    当前主需求节点));
        快照.需求树当前主需求可普通任务化 =
            目标语义视图.可进入普通候选方法筹办;
    ...
    快照.任务管理工作线程最近父任务收束任务主键 =
        工作线程快照.最近父任务收束任务主键;
    快照.任务管理工作线程最近父任务收束需求主键 =
        工作线程快照.最近父任务收束需求主键;
    快照.任务管理工作线程最近父任务收束直接子需求数 =
        工作线程快照.最近父任务收束直接子需求数;
    快照.任务管理工作线程最近父任务收束活动必须子需求数 =
        工作线程快照.最近父任务收束活动必须子需求数;
    快照.任务管理工作线程最近父任务收束活动可选路径子需求数 =
        工作线程快照.最近父任务收束活动可选路径子需求数;
    快照.任务管理工作线程最近父任务收束活动非必须子需求数 =
        工作线程快照.最近父任务收束活动非必须子需求数;
    快照.任务管理工作线程最近父任务收束截止子需求数 =
        工作线程快照.最近父任务收束截止子需求数;
    ...
    const std::size_t 树广度上限)
{
    ...
    if (展开类型 == "thread-self"
        ...
        if (展开类型 == "thread-self") {
            ...
            私有_追加叶字段(字段节点, "最近推进时间", 时间戳文本(工作线程快照.最近推进时间));
            ...
            私有_追加叶字段(字段节点, "最近父任务收束任务主键", 工作线程快照.最近父任务收束任务主键);
            私有_追加叶字段(字段节点, "最近父任务收束需求主键", 工作线程快照.最近父任务收束需求主键);
            私有_追加叶字段(
                字段节点,
                "最近父任务收束直接子需求数",
                工作线程快照.最近父任务收束直接子需求数);
    ...
    std::size_t)
{
    ...
    输出 << "任务管理摘要\n"
        ...
        << '\n'
        << "  - 最近父任务收束角色视图: 任务=" << 私有_页面摘要(
            快照.任务管理工作线程最近父任务收束任务主键.empty()
                ? std::string("空")
                : 快照.任务管理工作线程最近父任务收束任务主键)
        << " | 需求=" << 私有_页面摘要(
            快照.任务管理工作线程最近父任务收束需求主键.empty()
                ? std::string("空")
                : 快照.任务管理工作线程最近父任务收束需求主键)
        << " | 直接子需求=" << 快照.任务管理工作线程最近父任务收束直接子需求数
        << " | 必须=" << 快照.任务管理工作线程最近父任务收束活动必须子需求数
        << " | 可选路径=" << 快照.任务管理工作线程最近父任务收束活动可选路径子需求数
        << " | 非必须=" << 快照.任务管理工作线程最近父任务收束活动非必须子需求数
        << " | 截止=" << 快照.任务管理工作线程最近父任务收束截止子需求数
        << '\n'
...
<head>
  ...
  <script>
    ...
    function 应用自我场景复现帧(data) {
      自我场景帧请求中 = false;
      if (自我场景帧请求超时句柄) {
        window.clearTimeout(自我场景帧请求超时句柄);
        自我场景帧请求超时句柄 = 0;
      }
      if (!data || typeof data !== 'object') {
    ...
    function 请求自我场景复现帧() {
      ...
      if (自我场景帧请求中) return false;
      自我场景帧请求中 = true;
      if (自我场景帧请求超时句柄) {
        window.clearTimeout(自我场景帧请求超时句柄);
      }
      自我场景帧请求超时句柄 = window.setTimeout(() => {
        自我场景帧请求中 = false;
        自我场景帧请求超时句柄 = 0;
        设置自我场景状态('自我场景帧请求超时，等待下一轮刷新。');
      }, 8000);
