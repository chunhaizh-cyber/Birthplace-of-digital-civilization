...
namespace {
    ...
    存在节点类* 私有_解析方法虚拟存在_控制面板(const 方法节点* 方法) noexcept
    {
        ...
        if (首信息->方法虚拟存在.指针) {
            return 首信息->方法虚拟存在.指针;
        }
        if (首信息->方法虚拟存在.主键.empty()) {
            return nullptr;
        }
    ...
    void 私有_追加等号字段(
        ...
        const bool 值)
    {
        父节点.子项.push_back(私有_新节点(私有_等号字段显示文本(名称, "bool", 私有_布尔文本(值))));
    }
    ...
    std::string 私有_需求显示名_控制面板(const 需求节点* 节点)
    {
        ...
        if (私有_需求是根层节点_控制面板(节点)
            && 私有_语素入口相同_控制面板(目标特征类型, 自我特征定义类::类型_自我_安全值())) {
            return "安全根需求";
        }
        if (私有_需求是根层节点_控制面板(节点)
            && 私有_语素入口相同_控制面板(目标特征类型, 自我特征定义类::类型_自我_服务值())) {
            return "服务根需求";
        }
        ...
        路径集合& 已访问) noexcept
    {
        ...
        if (当前方法节点->主信息.公共.节点种类 == 枚举_方法节点种类::方法首节点
            ...
            && std::any_of(
                ...
                [&](const 结构_方法结果项& 结果项) {
                    return 私有_语素入口相同_控制面板(结果项.关系目标特征类型(), 目标特征类型);
                })) {
    ...
    std::vector<结构_控制面板树节点> 私有_构建方法主信息详情(方法节点* 节点)
    {
        ...
        私有_追加等号字段(字段节点, "公共.节点种类", 私有_方法节点种类文本(节点->主信息.节点种类()));
        ...
        if (首信息) {
            私有_追加等号引用字段(字段节点, "角色.首节点.方法虚拟存在", 首信息->方法虚拟存在, 路径);
            私有_追加等号字段(字段节点, "角色.首节点.动作名", 私有_词文本(首信息->动作名));
            私有_追加等号字段(字段节点, "角色.首节点.动作句柄", 私有_动作句柄文本(首信息->动作句柄));
            私有_追加等号字段(字段节点, "角色.首节点.来源", 私有_方法来源文本(首信息->来源));
            私有_追加等号引用字段(字段节点, "角色.首节点.来源任务", 首信息->来源任务, 路径);
            私有_追加等号引用字段(字段节点, "角色.首节点.父方法", 首信息->父方法, 路径);
            私有_追加等号引用字段(字段节点, "角色.首节点.前置方法", 首信息->前置方法, 路径);
            私有_追加等号引用字段(字段节点, "角色.首节点.后续方法", 首信息->后续方法, 路径);
            私有_追加等号字段(
                字段节点,
                "角色.首节点.能力.结果包.结果项数",
                static_cast<std::uint64_t>(首信息->能力.结果包.结果项集.size()));
            私有_追加等号字段(
                字段节点,
                "角色.首节点.能力.可被方法查找命中",
                首信息->能力.可被方法查找命中);
            私有_追加等号字段(
                ...
                "角色.首节点.能力.是否根写入原语",
                首信息->能力.是否根写入原语);
            私有_追加等号字段(字段节点, "角色.首节点.成熟度阶段", 首信息->成熟度阶段);
            私有_追加等号字段(字段节点, "角色.首节点.是否允许自动查找", 首信息->是否允许自动查找);
        }
        else if (条件信息) {
            ...
            私有_追加等号字段(
                字段节点,
                "角色.条件节点.条件面.可用条件组数",
                static_cast<std::uint64_t>(条件信息->条件面.可用条件组集.size()));
            私有_追加等号字段(
                字段节点,
                "角色.条件节点.条件判定索引数",
    ...
    std::vector<结构_控制面板树节点> 私有_构建任务主信息详情(任务节点* 节点)
    {
        ...
        私有_追加等号引用字段(字段节点, "结果状态信息", 节点->主信息.结果状态信息, 路径);
    ...
    基础信息节点类* 私有_需求目标宿主基础节点_控制面板(const 需求节点* 需求) noexcept
    {
        ...
        if (目标状态主信息 && !目标状态主信息->状态主体.主键.empty()) {
            return 世界树.基础信息().查找主键(目标状态主信息->状态主体.主键);
        }
        return nullptr;
    }
    ...
    {
        return 派生归因.派生本能能力缺口类型抽象特征指针 != 0
            || 派生归因.派生本能能力缺口类型值 != 0;
    }
    ...
    std::string 私有_推断自检父需求选择原因_控制面板(const 需求节点* 节点)
    {
        ...
        const auto* 父目标特征 = 父需求->主信息.目标特征类型缓存
            ? 父需求->主信息.目标特征类型缓存
            : 私有_需求目标特征类型_控制面板(父需求);
        if (私有_需求是根层节点_控制面板(父需求)
            && 私有_语素入口相同_控制面板(父目标特征, 自我特征定义类::类型_自我_安全值())) {
            return "安全根";
        }
        if (私有_需求是根层节点_控制面板(父需求)
            && 私有_语素入口相同_控制面板(父目标特征, 自我特征定义类::类型_自我_服务值())) {
            return "服务根";
        }
        const auto 父目标文本 = 私有_词文本(父目标特征);
        if (父目标文本.find("维护管理状态") != std::string::npos) {
            return "管理分支";
        }
        const auto 派生归因 = 需求类::读取派生归因兼容视图(节点->主信息);
        const auto 父派生归因 = 需求类::读取派生归因兼容视图(父需求->主信息);
        if (!派生归因.派生来源方法主键文本().empty()
            && 派生归因.派生来源方法主键文本()
                == 父派生归因.派生来源方法主键文本()) {
            return "局部来源需求";
        }
    ...
    {
        auto 视图节点 = 私有_新节点("自检投影只读视图");
        私有_追加等号字段(视图节点, "自检来源", "自检候选正式需求");

        const auto* 固定来源需求 = 私有_查找根需求_按目标特征_控制面板(
            自我特征定义类::类型_自我_安全值());
        const auto* 固定来源任务 = 私有_需求首个任务_控制面板(固定来源需求);
        私有_追加等号字段(视图节点, "固定来源任务", "安全根任务");
        私有_追加等号字段(
            视图节点,
            "固定来源任务主键",
            固定来源任务 ? 固定来源任务->获取主键() : std::string("空"));
        私有_追加等号字段(视图节点, "固定来源需求", "安全根需求");
        私有_追加等号字段(
            视图节点,
            "固定来源需求主键",
            固定来源需求 ? 固定来源需求->获取主键() : std::string("空"));
        私有_追加等号字段(
            视图节点,
            "安全根任务来源绑定缺失",
            固定来源任务 ? 0 : 1);

        auto* 目标宿主 = 私有_需求目标宿主基础节点_控制面板(节点);
        const auto* 目标特征类型 = 节点->主信息.目标特征类型缓存
            ? 节点->主信息.目标特征类型缓存
            : 私有_需求目标特征类型_控制面板(节点);
        auto* 目标特征 = 目标宿主 && 目标特征类型
            ? 世界树.特征().查找子特征_按类型(目标宿主, 目标特征类型)
            : nullptr;

        结构_自检原子目标因果投影输入 输入{};
        输入.固定来源任务 = const_cast<任务节点类*>(
            reinterpret_cast<const 任务节点类*>(固定来源任务));
        输入.固定来源需求 = const_cast<需求节点类*>(
            reinterpret_cast<const 需求节点类*>(固定来源需求));
        输入.目标宿主 = 目标宿主;
        输入.目标特征类型 = 目标特征;
        输入.目标特征类型词 = 目标特征类型;
        因果类 因果(&世界树.基础信息());
        const auto 投影 = 因果.查询自检原子目标投影(输入);

        std::vector<std::string> 维度片段{};
        for (const auto 维度 : 投影.目标维度集) {
            私有_追加唯一片段_控制面板(维度片段, 私有_目标投影维度文本_控制面板(维度));
        }
        std::vector<std::string> 权重片段{};
        for (const auto 权重 : 投影.候选权重类型集) {
            私有_追加唯一片段_控制面板(权重片段, 私有_目标权重类型文本_控制面板(权重));
        }
        std::vector<std::string> 结算层级片段{};
        std::vector<std::string> 距离层级片段{};
        for (const auto& 路径 : 投影.候选投影路径) {
            私有_追加唯一片段_控制面板(
                结算层级片段,
                std::to_string(路径.结算贡献层级));
            私有_追加唯一片段_控制面板(
                距离层级片段,
                std::to_string(路径.因果距离层级));
        }
        私有_追加等号字段(视图节点, "投影维度", 私有_拼接片段_控制面板(维度片段));
        私有_追加等号字段(视图节点, "候选权重类型", 私有_拼接片段_控制面板(权重片段));
        私有_追加等号字段(视图节点, "结算贡献层级", 私有_拼接片段_控制面板(结算层级片段));
        私有_追加等号字段(视图节点, "因果距离层级", 私有_拼接片段_控制面板(距离层级片段));
        私有_追加等号字段(
            视图节点,
            "来源因果主键",
            派生归因.派生来源因果主键文本().empty()
        ...
        私有_追加等号字段(
            视图节点,
            "父需求选择原因",
            私有_推断自检父需求选择原因_控制面板(节点));
        私有_追加等号字段(
            视图节点,
            "缺失证据",
            私有_拼接片段_控制面板(投影.缺失证据));
        私有_追加等号字段(视图节点, "展示来源", "运行态只读投影，不新增需求主信息字段");
        return 视图节点;
    }
        ...
        const 结构_构建上下文& 上下文)
    {
        ...
        const auto 派生归因 = 需求类::读取派生归因兼容视图(节点->主信息);
        私有_追加等号字段(字段节点, "派生来源方法主键", 派生归因.派生来源方法主键文本());
        私有_追加等号字段(字段节点, "派生来源因果主键", 派生归因.派生来源因果主键文本());
        私有_追加等号字段(
            字段节点,
            "派生需求类型抽象特征指针",
            私有_十六进制指针(派生归因.派生需求类型抽象特征指针));
        私有_追加等号字段(字段节点, "派生需求类型值", 派生归因.派生需求类型值);
        私有_追加等号字段(
            字段节点,
            "派生方法需求位抽象特征指针",
            私有_十六进制指针(派生归因.派生方法需求位抽象特征指针));
        私有_追加等号字段(字段节点, "派生方法需求位值", 派生归因.派生方法需求位值);
        私有_追加等号字段(
            字段节点,
            "派生本能能力缺口类型抽象特征指针",
            私有_十六进制指针(派生归因.派生本能能力缺口类型抽象特征指针));
        私有_追加等号字段(字段节点, "派生本能能力缺口类型值", 派生归因.派生本能能力缺口类型值);
        if (私有_需求是自检派生需求_控制面板(派生归因)) {
            字段节点.子项.push_back(
                私有_构建自检投影只读视图_控制面板(节点, 派生归因));
        }
        ...
        const 抽象特征主信息类& 主信息)
    {
        ...
        私有_追加叶字段(字段节点, "弃用组合规则数量", static_cast<std::uint64_t>(主信息.组合规则.size()));
        ...
        const 路径集合& 路径)
    {
        ...
        私有_追加叶字段(字段节点, "推导业务语义类型", 私有_动态业务语义类型文本(动态类::推导业务语义类型(主信息)));
        私有_追加叶字段(字段节点, "业务语义缓存", 私有_动态业务语义类型文本(主信息.业务语义类型));
        ...
        const bool 列表模式)
    {
        ...
        const auto 派生归因 = 需求类::读取派生归因兼容视图(节点->主信息);
        私有_追加叶字段(字段节点, "兼容派生来源方法主键", 派生归因.派生来源方法主键文本());
        私有_追加叶字段(字段节点, "兼容派生来源因果主键", 派生归因.派生来源因果主键文本());
        私有_追加叶字段(字段节点, "兼容派生需求类型抽象特征指针", 私有_十六进制指针(派生归因.派生需求类型抽象特征指针));
        私有_追加叶字段(字段节点, "兼容派生需求类型值", 派生归因.派生需求类型值);
        私有_追加叶字段(字段节点, "兼容派生方法需求位抽象特征指针", 私有_十六进制指针(派生归因.派生方法需求位抽象特征指针));
        私有_追加叶字段(字段节点, "兼容派生方法需求位值", 派生归因.派生方法需求位值);
        ...
        路径集合 路径)
    {
        ...
        if (节点->主信息.公共.节点种类 == 枚举_方法节点种类::方法首节点) {
            ...
            私有_追加叶字段(字段节点, "条件下结果节点数量", static_cast<I64>(计数.条件下结果数));
            私有_追加叶字段(字段节点, "直接结果节点数量", static_cast<I64>(计数.直接结果数));
            私有_追加叶字段(
                字段节点,
                "条件结果是否成对",
                计数.直接结果数 == 0 && 计数.条件无结果数 == 0);
        }
        const auto* 首信息 = 节点->主信息.取首节点信息();
        const auto* 条件信息 = 节点->主信息.取条件节点信息();
        const auto* 结果信息 = 节点->主信息.取结果节点信息();
        ...
        const 时间戳 详情加载时间)
    {
        ...
        if (!节点.详情.empty()) {
            ...
            私有_追加叶字段(字段节点, "创建者名称", 线程.创建者名称.empty() ? std::string("未记录") : 线程.创建者名称);
            私有_追加叶字段(字段节点, "所属线程池ID", 线程.所属线程池ID.empty() ? std::string("未记录") : 线程.所属线程池ID);
            私有_追加叶字段(字段节点, "所属线程池名称", 线程.所属线程池名称.empty() ? std::string("未记录") : 线程.所属线程池名称);
        ...
        const std::string& 说明)
    {
        ...
        私有_追加叶字段(字段节点, "线程名称", 线程名称);
        私有_追加叶字段(字段节点, "线程用途", 线程用途);
        ...
        const 任务管理工作线程::结构_工作线程实例快照& 工作线程快照)
    {
        ...
        私有_追加线程显示详情(
            ...
            详情加载时间,
            快照.控制面板摘要线程摘要.empty()
                ? std::string("快照=") + std::to_string(快照.控制面板摘要快照序号)
    ...
    const bool 读取自我场景快照)
{
    ...
    if (需求根节点) {
        ...
        auto 统计需求满足 = [&](const 需求节点* 节点) {
            ...
            if (需求类::存在派生归因兼容缓存(节点->主信息)) {
                ++快照.需求树派生归因需求数;
                const auto 派生归因 = 需求类::读取派生归因兼容视图(节点->主信息);
                if (!派生归因.派生来源因果主键文本().empty()) {
                    ++快照.需求树派生来源因果需求数;
                }
            }
    ...
    快照.任务管理界面线程最近入队原因 = 界面线程快照.最近入队原因;
    ...
    快照.任务管理界面线程最近后台worker工作项类型 = 界面线程快照.最近后台worker工作项类型;
    快照.任务管理界面线程最近后台worker状态 = 界面线程快照.最近后台worker状态;
    快照.任务管理界面线程最近后台worker原因 = 界面线程快照.最近后台worker原因;
    快照.任务管理工作线程池最近参数保存结果 = 界面线程快照.最近参数保存结果;
    快照.任务管理工作线程池最近参数错误 = 界面线程快照.最近参数错误;
    快照.任务管理工作线程池最近参数应用结果 = 界面线程快照.最近参数应用结果;
    ...
    const std::size_t 树广度上限)
{
    ...
    if (展开类型 == "thread-self"
        ...
        if (展开类型 == "thread-self") {
            ...
            私有_追加叶字段(字段节点, "累计后台worker取项数", 界面线程快照.累计后台worker取项数);
            私有_追加叶字段(字段节点, "累计后台worker派发数", 界面线程快照.累计后台worker派发数);
            ...
            私有_追加叶字段(字段节点, "后台worker池运行", 界面线程快照.后台worker池运行);
            私有_追加叶字段(字段节点, "后台worker线程数", 界面线程快照.后台worker线程数);
            私有_追加叶字段(字段节点, "worker pool 默认大小", 界面线程快照.任务管理工作线程池默认大小);
            私有_追加叶字段(字段节点, "worker pool 最小大小", 界面线程快照.任务管理工作线程池最小大小);
            私有_追加叶字段(字段节点, "worker pool 最大大小", 界面线程快照.任务管理工作线程池最大大小);
            私有_追加叶字段(字段节点, "worker pool 当前有效大小", 界面线程快照.任务管理工作线程池当前有效大小);
            私有_追加叶字段(字段节点, "worker pool 待保存大小", 界面线程快照.任务管理工作线程池待保存大小);
            私有_追加叶字段(字段节点, "worker pool 参数版本", 界面线程快照.任务管理工作线程池参数版本);
            私有_追加叶字段(字段节点, "最近参数保存结果", 界面线程快照.最近参数保存结果);
            私有_追加叶字段(字段节点, "最近参数错误", 私有_截断文本(界面线程快照.最近参数错误, 200));
            私有_追加叶字段(字段节点, "最近参数应用结果", 界面线程快照.最近参数应用结果);
            ...
            私有_追加叶字段(字段节点, "最近排序原因", 私有_截断文本(界面线程快照.最近排序原因, 200));
            ...
            私有_追加叶字段(字段节点, "最近不可推进原因", 私有_截断文本(界面线程快照.最近不可推进原因, 200));
            ...
            私有_追加叶字段(字段节点, "最近入队原因", 私有_截断文本(界面线程快照.最近入队原因, 200));
            私有_追加叶字段(字段节点, "最近工作项类型", 界面线程快照.最近工作项类型);
            私有_追加叶字段(字段节点, "最近后台worker序号", 界面线程快照.最近后台worker序号);
            私有_追加叶字段(字段节点, "最近后台worker工作项ID", 界面线程快照.最近后台worker工作项ID);
            私有_追加叶字段(字段节点, "最近后台worker任务主键", 界面线程快照.最近后台worker任务主键);
            私有_追加叶字段(字段节点, "最近后台worker工作项类型", 界面线程快照.最近后台worker工作项类型);
            私有_追加叶字段(字段节点, "最近后台worker状态", 界面线程快照.最近后台worker状态);
            私有_追加叶字段(字段节点, "最近后台worker原因", 私有_截断文本(界面线程快照.最近后台worker原因, 200));
    ...
    std::size_t)
{
    ...
    输出 << "控制面板摘要\n"
        ...
        << " | 排队=" << 快照.任务管理工作线程当前排队数
        << " | worker池=" << (快照.任务管理界面线程后台worker池运行 ? "运行" : "停止")
    ...
    std::size_t)
{
    ...
    输出 << "任务管理摘要\n"
        ...
        << " | worker池=" << (快照.任务管理界面线程后台worker池运行 ? "运行" : "停止")
        << "/" << 快照.任务管理界面线程后台worker线程数
        << " | worker池参数=" << 快照.任务管理工作线程池当前有效大小
        << " (默认" << 快照.任务管理工作线程池默认大小 << ")"
        << " | worker取项=" << 快照.任务管理界面线程累计后台worker取项数
        << " | worker派发=" << 快照.任务管理界面线程累计后台worker派发数
    ...
    枚举_控制面板HTML用途 用途)
{
    ...
    const auto 界面线程卡片说明 = 私有_转义HTML(
        ...
        + " | worker池=" + std::string(快照.任务管理界面线程后台worker池运行 ? "运行" : "停止")
        + "/" + std::to_string(快照.任务管理界面线程后台worker线程数)
        + " | worker池大小=" + std::to_string(快照.任务管理工作线程池当前有效大小)
  ...
  <div class="shell">
    <div class="layout">
      ...
      <main class="content">
        ...
        << 私有_转义HTML(私有_页面摘要(快照.任务管理工作线程池最近参数保存结果))
        << R"HTML(</div>
                  </div>
                  <div class="settings-result">
                    <div class="settings-result-label">最近应用</div>
                    <div class="settings-result-value">)HTML"
        << 私有_转义HTML(私有_页面摘要(快照.任务管理工作线程池最近参数应用结果))
        << R"HTML(</div>
                  </div>
                  <div class="settings-result">
                    <div class="settings-result-label">最近参数错误</div>
                    <div class="settings-result-value">)HTML"
        << 私有_转义HTML(私有_页面摘要(快照.任务管理工作线程池最近参数错误))
        << R"HTML(</div>
                  </div>
                </div>
              </div>
        ...
        << R"HTML(</div>
                ...
                <div class="settings-slot">
                  <div class="settings-side-label">后台 worker 池</div>
                  <div class="settings-side-value">)HTML"
