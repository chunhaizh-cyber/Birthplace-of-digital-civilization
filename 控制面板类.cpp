...
namespace {
    ...
    void 私有_构建主派发输出快照(结构_控制面板快照* 快照)
    {
        ...
        if (!快照->任务管理专项摘要.empty()) {
            快照->任务管理专项摘要 += "\n需求与权重治理: " + 私有_安全文本(快照->需求与权重治理输出摘要, "无");
            快照->任务管理专项摘要 += "\n主派发: " + 私有_安全文本(快照->主派发输出摘要, "无");
            快照->任务管理专项摘要 += "\n队列治理: " + 私有_安全文本(快照->队列治理输出摘要, "无");
            快照->任务管理专项摘要 += "\n结算治理: " + 私有_安全文本(快照->结算治理输出摘要, "无");
        }
    }
    ...
    std::size_t 列表预览上限)
{
    ...
    输出 << "需求与权重治理输出: " << 私有_安全文本(快照.需求与权重治理输出摘要, "暂无需求与权重治理输出") << '\n';
    输出 << "主派发输出: " << 私有_安全文本(快照.主派发输出摘要, "暂无主派发输出") << '\n';
    输出 << "队列治理输出: " << 私有_安全文本(快照.队列治理输出摘要, "暂无队列治理输出") << '\n';
    输出 << "结算治理输出: " << 私有_安全文本(快照.结算治理输出摘要, "暂无结算治理输出") << '\n';
    输出 << "双值结算账: " << 私有_安全文本(快照.双值结算账摘要, "暂无双值结算账") << '\n';
    输出 << "学习恢复接口: " << 私有_安全文本(快照.学习恢复接口摘要, "暂无学习恢复接口") << '\n';
    输出 << "治理实例堆: " << 私有_安全文本(快照.任务管理治理实例堆摘要, "治理实例堆=空") << '\n';
    输出 << "恢复快照: " << 私有_安全文本(快照.任务管理恢复快照摘要, "恢复快照=空") << '\n';
    ...
    渲染列表("主派发动作", 快照.主派发动作列表);
    渲染列表("主派发队列裁决", 快照.主派发队列裁决列表);
    渲染列表("队列治理状态", 快照.队列治理状态列表);
    渲染列表("队列治理成员", 快照.队列治理成员列表);
    渲染列表("队列治理事件", 快照.队列治理成员事件列表);
    渲染列表("结算回流", 快照.结算治理回流列表);
    渲染列表("结算否定项", 快照.结算治理否定项列表);
    渲染列表("结算学习触发", 快照.结算治理学习触发列表);
    渲染列表("结算关键状态", 快照.结算治理关键状态列表);
    渲染列表("主派发任务包", 快照.主派发任务包列表);
    渲染列表("主派发占用登记", 快照.主派发占用登记列表);
    渲染列表("主派发回执", 快照.主派发回执列表);
    return 输出.str();
}
