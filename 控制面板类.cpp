...
namespace {
    ...
    {
        auto 文本 = 私有_方法标题(节点);
        if (节点 && 节点->主信息.节点种类 == 枚举_方法节点种类::方法首节点) {
        ...
        路径集合 路径)
    {
        ...
        私有_追加叶字段(字段节点, "节点地址", 私有_十六进制指针(私有_地址(节点)));
        私有_追加叶字段(字段节点, "子节点数量", 节点->子节点数量);
        if (节点->主信息.节点种类 == 枚举_方法节点种类::方法首节点) {
            const auto 计数 = 私有_统计方法直接条件结果(节点);
    ...
    const std::size_t 树广度上限)
{
    ...
    auto& 自我线程 = 获取全局自我线程();
    (void)启动控制面板摘要线程("控制面板类::读取控制面板快照");
    (void)请求刷新控制面板摘要("控制面板类::读取控制面板快照");
    ...
    快照.学习首个兜底切换就绪摘要.clear();
    快照.学习首个兜底切换采样摘要.clear();
    快照.学习当前阶段.clear();
    快照.学习当前状态.clear();
    快照.学习当前任务标题.clear();
    快照.学习当前方法标题.clear();
    ...
    std::size_t)
{
    ...
    const auto 任务树交互提示 = 私有_转义HTML(
        "任务列表页中间栏只显示任务列表与每条任务树结构；单击节点后，字段在右侧按需加载。");
    ...
    const auto 学习卡片说明 = 私有_转义HTML(
        "旧学习模块已移除 | 待处理方法=" + std::to_string(快照.自我待学习方法数量)
        + " | 可用方法=" + std::to_string(快照.自我可用方法数量));
  ...
  <script>
    ...
    const 页面配置 = {
      ...
      'task-tree': {
        ...
        detailHint: '任务列表页中间栏只显示任务树结构；右侧查看当前选中任务节点的字段。'
      },
      'method-tree': {
