...
namespace {
    ...
    结构_控制面板树节点 私有_需求列表轻量骨架节点(需求节点* 节点)
    {
        // 需求列表是控制面板首页自动刷新路径。这里刻意不读取需求主键、
        // 类型或权重，避免展示层在需求树并发变化时因为旧指针/坏主键
        // 直接中断 WebView 刷新；完整信息交给按需展开详情读取。
        return 私有_新节点(
            std::string("需求列表项 | 地址=")
                + 私有_十六进制指针(私有_地址(节点))
                + " | 详情=按需展开",
    ...
    const bool 读取自我场景快照)
{
    ...
    if (需求根节点) {
        ...
        for (auto* 节点 : 分页结果.节点集) {
            需求列表树根.子项.push_back(私有_需求列表轻量骨架节点(节点));
        }
        if (分页结果.还有更多) {
    ...
    if (展开类型 == "need-list-more") {
        ...
        if (auto* 需求根节点 = 自我存在 ? 世界树.存在().获取需求根节点(自我存在) : nullptr) {
            const auto 分页结果 = 私有_读取需求列表分页(
                ...
                私有_列表分页大小);
            for (auto* 节点 : 分页结果.节点集) {
                容器.子项.push_back(私有_需求列表轻量骨架节点(节点));
            }
            if (分页结果.还有更多) {
