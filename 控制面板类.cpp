...
namespace {
        ...
        路径集合& 已访问) noexcept
    {
        ...
        if (当前方法节点->主信息.公共.节点种类 == 枚举_方法节点种类::方法首节点
            && 当前方法节点->主信息.有动作骨架()
            && std::any_of(
                当前方法节点->主信息.首节点信息().能力.能力面.能力签名集.begin(),
                当前方法节点->主信息.首节点信息().能力.能力面.能力签名集.end(),
                [&](const 结构_方法能力签名& 签名) {
        ...
        路径集合& 已访问) noexcept
    {
        ...
        for (const auto& 签名 : 方法->主信息.首节点信息().能力.能力面.能力签名集) {
            if (!签名.可被方法查找命中 || 签名.是否根写入原语) {
                continue;
            }
        ...
        路径集合 路径)
    {
        ...
        私有_追加叶字段(字段节点, "作用域", 首信息 ? 私有_方法作用域文本(首信息->作用域) : std::string{});
        私有_追加叶字段(
            字段节点,
            "最近执行时间",
