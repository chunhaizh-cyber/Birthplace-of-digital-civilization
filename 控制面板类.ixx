...
struct 结构_控制面板体素加工子候选诊断 {
    std::int64_t 包索引 = 0;
    std::int64_t 候选ID = 0;
    std::int64_t 候选状态 = 0;
    std::int64_t 彩色像素数量 = 0;
    std::int64_t 深度支撑像素数量 = 0;
    std::int64_t 空间支撑像素数量 = 0;
    std::int64_t 深度支撑率 = 0;
    ...
    std::int64_t 局部体素有占据 = 0;
    std::int64_t 局部体素触碰外边界 = 0;
    std::int64_t 局部体素压紧 = 0;
};

struct 结构_控制面板体素加工候选包诊断 {
    std::int64_t 包索引 = 0;
    std::int64_t 任务ID = 0;
    std::uintptr_t 父存在指针 = 0;
    std::int64_t 输入观察证据ID = 0;
    std::int64_t 基准体素版本 = 0;
    std::int64_t 处理状态 = 0;
    std::int64_t 可提交状态 = 0;
    std::int64_t 生成时间 = 0;
    std::int64_t 父基础颜色统计有效 = 0;
    std::int64_t 父基础颜色像素数量 = 0;
    std::int64_t 父基础颜色RGBA = 0;
    std::int64_t 子轮廓候选数量 = 0;
    std::int64_t 深度支撑子候选数量 = 0;
    std::int64_t 表面或未解释子候选数量 = 0;
    std::int64_t 局部子体素候选数量 = 0;
    std::vector<结构_控制面板体素加工子候选诊断> 子候选列表{};
};
...
struct 结构_控制面板快照 {
    ...
    std::int64_t 自我场景水平视野角毫度 = 0;
    std::int64_t 自我场景垂直视野角毫度 = 0;
    std::int64_t 自我场景视野参数状态 = 0;
    std::int64_t 自我场景姿态明确状态 = 0;
    ...
    std::int64_t 体素加工候选结果包数量 = 0;
    std::int64_t 体素加工候选最新输入观察证据ID = 0;
    std::int64_t 体素加工候选最新基准体素版本 = 0;
    std::int64_t 体素加工候选最新父基础颜色有效 = 0;
    std::int64_t 体素加工候选最新子轮廓候选数量 = 0;
    std::int64_t 体素加工候选最新局部子体素候选数量 = 0;
    std::int64_t 体素加工候选诊断分支 = 0;
    std::vector<结构_控制面板体素加工候选包诊断> 体素加工候选包诊断列表{};
    ...
    std::string 任务管理工作线程最近父任务收束任务主键{};
    std::string 任务管理工作线程最近父任务收束需求主键{};
    std::uint64_t 任务管理工作线程最近父任务收束直接子需求数 = 0;
    std::uint64_t 任务管理工作线程最近父任务收束活动必须子需求数 = 0;
    std::uint64_t 任务管理工作线程最近父任务收束活动可选路径子需求数 = 0;
    std::uint64_t 任务管理工作线程最近父任务收束活动非必须子需求数 = 0;
    std::uint64_t 任务管理工作线程最近父任务收束截止子需求数 = 0;
