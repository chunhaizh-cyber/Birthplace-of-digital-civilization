...
std::string 语言类::获取基础信息名称(const 基础信息节点类* 节点) const
{
    ...
    if (const auto* 抽象特征主信息 = dynamic_cast<const 抽象特征主信息类*>(节点->主信息)) {
        const auto 类型 = 私有_安全文本(语素_安全获取词(抽象特征主信息->类型), 语素_安全获取词(抽象特征主信息->名称));
    ...
    if (const auto* 特征主信息 = dynamic_cast<const 特征节点主信息类*>(节点->主信息)) {
        const auto 类型 = 私有_安全文本(语素_安全获取词(特征主信息->类型), 语素_安全获取词(特征主信息->名称));
        if (特征值有效(特征主信息->当前值)) {
    ...
    if (const auto* 动态主信息 = dynamic_cast<const 动态节点主信息类*>(节点->主信息)) {
        ...
        if (动作名.empty()) {
            动作名 = 语素_安全获取词(动态主信息->来源动作名);
        }
        if (动作名.empty()) {
            动作名 = 语素_安全获取词(动态主信息->名称);
        }
