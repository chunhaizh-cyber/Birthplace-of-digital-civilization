...
export struct 结构体_D455轻量观察帧阶段耗时 {
    bool 有效 = false;
    std::uint64_t 总耗时微秒 = 0;
    std::uint64_t 等帧耗时微秒 = 0;
    std::uint64_t 对齐耗时微秒 = 0;
    std::uint64_t 取帧耗时微秒 = 0;
    std::uint64_t 滤波耗时微秒 = 0;
    std::uint64_t 元数据耗时微秒 = 0;
    std::uint64_t 彩色复制耗时微秒 = 0;
    std::uint64_t 深度复制耗时微秒 = 0;
};
