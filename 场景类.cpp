...
namespace {
    ...
    void 私有_确保场景默认特征已初始化(基础信息类* 基础信息, 场景节点类* 场景)
    {
        ...
        const auto* 自我场景名称 = 语素集.添加词性词("自我初始场景", "专有名词");
        const auto* 场景主信息 = 基础信息->取主信息<场景节点主信息类>(场景);
        const bool 是自我所在场景 =
            (场景 == 世界树.自我所在场景)
            || (场景主信息
                && 场景主信息->世界类型 == 枚举_世界类型::现实世界
                && 私有_词性相同(场景主信息->名称, 自我场景名称));
        if (!是自我所在场景) return;

        const auto* 场景安全度特征 = 语素集.添加词性词("场景安全度", "名词");
        const auto* 旧安全度特征 = 语素集.添加词性词("安全度", "名词");
        if (!场景安全度特征) return;

        auto* 特征节点 = 私有_查找子特征_按类型(基础信息, 场景, 场景安全度特征);
        if (!特征节点 && 旧安全度特征) {
            特征节点 = 私有_查找子特征_按类型(基础信息, 场景, 旧安全度特征);
        }
