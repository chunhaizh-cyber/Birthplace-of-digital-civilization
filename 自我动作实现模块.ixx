...
namespace {
        ...
        时间戳 now) noexcept
    {
        ...
        if (auto* 首信息 = 方法首节点->主信息.取首节点信息()) {
            // `改变特征值` 只保留为底层特征写入原语。
            // 它不能作为任务筹办候选方法，也不能作为方法动作动态的语义动作来源。
...
export namespace 自我动作实现模块 {
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        // `改变特征值` 是通用总接口，执行前必须知道本轮要改变的
        // “目标宿主.目标特征类型”。这里显式把目标特征类型落到
        // 可执行输入包；若只能拿到抽象特征，则从抽象特征反解类型。
        const 语素入口节点类* 目标特征类型 = nullptr;
        void* 目标特征类型指针 = nullptr;
        if (读取包指针(需求包, 特征_目标特征类型(), 目标特征类型指针)
            || 读取包指针(输入条件包, 特征_目标特征类型(), 目标特征类型指针)) {
            if (auto* 类型节点 = 指针若为基础信息节点(目标特征类型指针)) {
                目标特征类型 = 特征值节点对应特征类型词(类型节点);
            }
            else {
                目标特征类型 = reinterpret_cast<const 语素入口节点类*>(
                    目标特征类型指针);
            }
        }
        if (!目标特征类型 && 目标抽象特征) {
            目标特征类型 = 特征值节点对应特征类型词(目标抽象特征);
        }
        累计绑定(
            目标特征类型
            && 写入基础节点指针(
        ...
        时间戳 now) noexcept
    {
        ...
        // 结算叶子任务价值只处理“任务完成 + 需求满足证据”后的真实价值入账。
        // 需求权重仍只是活动参与标记，不能被当作安全/服务值直接兑换。
        ok = 声明形参规格(方法首节点, 特征_任务信息节点(), 值类型_场景指针(), 值_来源_输入条件包(), true, now) && ok;
        ok = 声明形参规格(方法首节点, 特征_来源需求(), 值类型_场景指针(), 值_来源_输入条件包(), true, now) && ok;
        ok = 声明形参规格(方法首节点, 特征_安全结算增量(), 值类型_I64(), 值_来源_输入条件包(), false, now) && ok;
        ok = 声明形参规格(方法首节点, 特征_服务结算增量(), 值类型_I64(), 值_来源_输入条件包(), false, now) && ok;
        ...
        时间戳 now) noexcept
    {
        ...
        case 枚举_本能方法ID::自我_增加可用方法数量:
            return 确认增加可用方法数量规格(方法首节点, 方法存在, now);
        case 枚举_本能方法ID::自我_提交任务状态变化:
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        if (方法存在) {
            写结果(reinterpret_cast<基础信息节点类*>(方法存在), "练习方法虚拟存在");
        }
        auto* 输出运行 = 输出运行虚拟存在(运行存在, 输入参数场景, 输出结果场景, now);
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        if (环境.方法虚拟存在) {
            写结果(reinterpret_cast<基础信息节点类*>(环境.方法虚拟存在));
        }
        if (成功 && 目标方法宿主) {
            写结果(目标方法宿主);
        }
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        const bool 任务已完成 = 任务 && 任务类::读取任务状态(任务) == 枚举_任务状态::完成;
        const bool 增量合法 =
            合法叶子任务结算增量(计划安全增量)
            && 合法叶子任务结算增量(计划服务增量);
        bool 成功 = 自我对象 && 任务 && 来源需求 && 任务已完成 && 增量合法;
        ...
        if (成功) {
            const I64 安全增量 = (std::max<I64>)(0, 计划安全增量);
            const I64 服务增量 = (std::max<I64>)(0, 计划服务增量);
