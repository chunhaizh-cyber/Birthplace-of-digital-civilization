...
export namespace 自我动作实现模块 {
        ...
        时间戳 now) noexcept
    {
        ...
        if (auto* 回执根 = 方法规格根(方法首节点, 特征_执行回执表(), now)) {
            ...
            ok = 取或创建子特征(宿主, 特征_本轮是否有推进()) && ok;
            ok = 取或创建子特征(宿主, 特征_本轮推进量()) && ok;
            ok = 取或创建子特征(宿主, 特征_剩余差距()) && ok;
            ok = 取或创建子特征(宿主, 特征_执行结果类型()) && ok;
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        if (成功) {
            ...
            (void)写入目标方法I64特征(
                ...
                now);
            if (目标为方法动作补齐) {
                // 任务 5 这类“目标方法.方法动作状态=1”需求，需要一个可比较的实际结果状态。
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        if (成功) {
            ...
            (void)从回执复制I64到任务虚拟存在(任务, 回执宿主, 特征_本轮是否有推进(), 任务->主信息.场景.指针, now);
            (void)从回执复制I64到任务虚拟存在(任务, 回执宿主, 特征_本轮推进量(), 任务->主信息.场景.指针, now);
            (void)从回执复制I64到任务虚拟存在(任务, 回执宿主, 特征_剩余差距(), 任务->主信息.场景.指针, now);
            (void)从回执复制I64到任务虚拟存在(任务, 回执宿主, 特征_执行结果类型(), 任务->主信息.场景.指针, now);
            (void)从回执复制指针到任务虚拟存在(任务, 回执宿主, 特征_执行缺口类型(), 任务->主信息.场景.指针, now);
            const bool 已同步当前方法运行存在 = 从回执复制指针到任务虚拟存在(
                任务,
                回执宿主,
                特征_当前方法运行存在(),
                任务->主信息.场景.指针,
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        auto* 目标方法 = (输入包
            ...
            && 目标方法指针)
            ? reinterpret_cast<方法类::节点类*>(目标方法指针)
            : nullptr;
