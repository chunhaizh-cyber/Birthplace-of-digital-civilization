...
export namespace 自我动作实现模块 {
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        const bool 目标动作补齐可闭合 = !目标为方法动作补齐
            || (目标方法
                && (目标方法->主信息.有方法动作() || 目标已有动作句柄));
        if (目标为方法动作补齐 && !目标动作补齐可闭合) {
            std::ostringstream 输出;
            输出 << "自我_定向学习/动作目标不可闭合"
                << " | 目标方法=" << (目标方法 ? 目标方法->获取主键() : std::string{})
                << " | 目标方法有方法动作=" << (目标方法 && 目标方法->主信息.有方法动作() ? 1 : 0)
                << " | 目标已有动作句柄=" << (目标已有动作句柄 ? 1 : 0)
                << " | 说明=定向学习只能记录动作语义候选，不能伪造目标方法动作状态";
            项目运行日志(输出.str());
        }
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        if (输入有效 && !已领取) {
            ...
            const auto 旧状态指针 = OR组->主信息.被需求当前状态.指针;
            if (OR组->主信息.需求状态.指针
                ...
                需求满足已回写 = true;
                需求状态动态已输出 =
                    OR组->主信息.被需求当前状态.指针 != 旧状态指针
                    || (!OR组->主信息.被需求当前状态.主键.empty()
                        && OR组->主信息.被需求当前状态.主键
                            != (旧状态指针 ? 旧状态指针->获取主键() : std::string{}));
            }
            需求动态 = 输出需求满足动作动态(
                环境.方法首节点,
                OR组,
        ...
        auto 写结果 = [&](基础信息节点类* 宿主) noexcept {
            ...
            (void)写入基础节点I64(宿主, 特征_需求满足登记(), 需求满足已回写 ? 1 : 0, now);
            if (令牌动态) {
                (void)写入基础节点指针(宿主, 特征_动作动态(), 令牌动态, now);
                (void)写入基础节点指针(宿主, 特征_OR组令牌领取动作动态(), 令牌动态, now);
            }
            if (需求动态) {
                (void)写入基础节点指针(宿主, 特征_需求状态动作动态(), 需求动态, now);
            }
            if (!输入有效) {
...

        {
            ...
            日志 << "提交_领取OR组结算令牌"
                ...
                << " | 需求动态=" << (需求动态 ? 需求动态->获取主键() : std::string("缺"))
                << " | 令牌状态不写需求主信息=1";
            项目运行日志(日志.str());
        }
