...
export namespace 自我动作实现模块 {
    ...
    inline 存在节点类* 结算叶子任务价值(
        ...
        场景节点类* 输入参数场景,
        场景节点类* 输出结果场景) noexcept;
    inline 存在节点类* 服务_观察当前交互环境(
        方法类::节点类* 方法首节点,
        场景节点类* 输入参数场景,
        场景节点类* 输出结果场景) noexcept;
    inline 存在节点类* 服务_识别当前交互者(
        方法类::节点类* 方法首节点,
        场景节点类* 输入参数场景,
        场景节点类* 输出结果场景) noexcept;
    inline 存在节点类* 服务_识别人类服务对象(
        方法类::节点类* 方法首节点,
        场景节点类* 输入参数场景,
        场景节点类* 输出结果场景) noexcept;
    inline 存在节点类* 服务_确认权限者(
        方法类::节点类* 方法首节点,
        场景节点类* 输入参数场景,
        场景节点类* 输出结果场景) noexcept;
    inline 存在节点类* 服务_绑定当前服务对象(
        方法类::节点类* 方法首节点,
        场景节点类* 输入参数场景,
        场景节点类* 输出结果场景) noexcept;
    inline 存在节点类* 服务_解析服务目标(
        方法类::节点类* 方法首节点,
        场景节点类* 输入参数场景,
        场景节点类* 输出结果场景) noexcept;
    inline 存在节点类* 服务_获取服务对象所需服务的信息(
        方法类::节点类* 方法首节点,
        场景节点类* 输入参数场景,
        场景节点类* 输出结果场景) noexcept;
    inline 存在节点类* 服务_获取服务对象反馈信息(
        方法类::节点类* 方法首节点,
        场景节点类* 输入参数场景,
        场景节点类* 输出结果场景) noexcept;
    inline 存在节点类* 安全_检查自我运行状态(
        方法类::节点类* 方法首节点,
        场景节点类* 输入参数场景,
        场景节点类* 输出结果场景) noexcept;
    inline 存在节点类* 安全_检查资源状态(
        方法类::节点类* 方法首节点,
        场景节点类* 输入参数场景,
        场景节点类* 输出结果场景) noexcept;
    inline 存在节点类* 安全_检查权限边界(
        方法类::节点类* 方法首节点,
        场景节点类* 输入参数场景,
        场景节点类* 输出结果场景) noexcept;
    inline 存在节点类* 安全_检查风险状态(
        方法类::节点类* 方法首节点,
        场景节点类* 输入参数场景,
        场景节点类* 输出结果场景) noexcept;
    inline 存在节点类* 安全_执行降级保护(
        方法类::节点类* 方法首节点,
        场景节点类* 输入参数场景,
        场景节点类* 输出结果场景) noexcept;
    ...
    inline bool 注册本能函数执行闭环() noexcept
    {
        ...
        ok = 取本能动作集().注册(
            ...
            &结算叶子任务价值) && ok;
        ok = 取本能动作集().注册(
            枚举_本能方法ID::服务_观察当前交互环境,
            &服务_观察当前交互环境) && ok;
        ok = 取本能动作集().注册(
            枚举_本能方法ID::服务_识别当前交互者,
            &服务_识别当前交互者) && ok;
        ok = 取本能动作集().注册(
            枚举_本能方法ID::服务_识别人类服务对象,
            &服务_识别人类服务对象) && ok;
        ok = 取本能动作集().注册(
            枚举_本能方法ID::服务_确认权限者,
            &服务_确认权限者) && ok;
        ok = 取本能动作集().注册(
            枚举_本能方法ID::服务_绑定当前服务对象,
            &服务_绑定当前服务对象) && ok;
        ok = 取本能动作集().注册(
            枚举_本能方法ID::服务_解析服务目标,
            &服务_解析服务目标) && ok;
        ok = 取本能动作集().注册(
            枚举_本能方法ID::服务_获取服务对象所需服务的信息,
            &服务_获取服务对象所需服务的信息) && ok;
        ok = 取本能动作集().注册(
            枚举_本能方法ID::服务_获取服务对象反馈信息,
            &服务_获取服务对象反馈信息) && ok;
        ok = 取本能动作集().注册(
            枚举_本能方法ID::安全_检查自我运行状态,
            &安全_检查自我运行状态) && ok;
        ok = 取本能动作集().注册(
            枚举_本能方法ID::安全_检查资源状态,
            &安全_检查资源状态) && ok;
    ...
    inline bool 确认对象作用布尔规格(
        方法类::节点类* 方法首节点,
        存在节点类* 方法存在,
        时间戳 now,
        std::initializer_list<const 语素入口节点类*> 结果特征集) noexcept
    {
        if (!方法首节点 || !方法存在) return false;
        bool ok = true;
        ok = 声明形参规格(方法首节点, 特征_输入条件包(), 值类型_场景指针(), 值_来源_输入条件包(), true, now) && ok;
        ...
        ok = 声明条件特征(方法首节点, 特征_目标执行后值(), false, now) && ok;
        ok = 声明条件特征(方法首节点, 特征_来源需求(), false, now) && ok;
        ok = 声明条件特征(方法首节点, 特征_任务信息节点(), false, now) && ok;
        if (auto* 结果根 = 方法规格根(方法首节点, 特征_结果表(), now)) {
            auto* 宿主 = reinterpret_cast<基础信息节点类*>(结果根);
            ...
            ok = 取或创建子特征(宿主, 特征_服务前状态场景()) && ok;
            ok = 取或创建子特征(宿主, 特征_失败原因()) && ok;
            for (const auto* 结果特征 : 结果特征集) {
                ok = 取或创建子特征(宿主, 结果特征) && ok;
            }
        }
        for (const auto* 结果特征 : 结果特征集) {
            ok = 确保方法结果能力I64(方法首节点, 结果特征, 1) && ok;
        }
        return ok;
    }

    inline bool 确认识别当前交互者规格(
        方法类::节点类* 方法首节点,
        存在节点类* 方法存在,
        时间戳 now) noexcept
    {
        ...
        时间戳 now) noexcept
    {
        ...
        case 枚举_本能方法ID::服务_观察当前交互环境:
            return 确认观察当前交互环境规格(方法首节点, 方法存在, now);
        case 枚举_本能方法ID::服务_识别当前交互者:
            return 确认识别当前交互者规格(方法首节点, 方法存在, now);
        case 枚举_本能方法ID::服务_识别人类服务对象:
            return 确认识别人类服务对象规格(方法首节点, 方法存在, now);
        case 枚举_本能方法ID::服务_确认权限者:
            return 确认确认权限者规格(方法首节点, 方法存在, now);
        case 枚举_本能方法ID::服务_绑定当前服务对象:
            return 确认绑定当前服务对象规格(方法首节点, 方法存在, now);
        case 枚举_本能方法ID::服务_解析服务目标:
            return 确认解析服务目标规格(方法首节点, 方法存在, now);
        case 枚举_本能方法ID::服务_获取服务对象所需服务的信息:
            return 确认获取服务对象所需服务信息规格(方法首节点, 方法存在, now);
        case 枚举_本能方法ID::服务_获取服务对象反馈信息:
            return 确认获取服务对象反馈信息规格(方法首节点, 方法存在, now);
        case 枚举_本能方法ID::安全_检查自我运行状态:
            return 确认检查自我运行状态规格(方法首节点, 方法存在, now);
        case 枚举_本能方法ID::安全_检查资源状态:
            return 确认检查资源状态规格(方法首节点, 方法存在, now);
        case 枚举_本能方法ID::安全_检查权限边界:
            return 确认检查权限边界规格(方法首节点, 方法存在, now);
        case 枚举_本能方法ID::安全_检查风险状态:
            return 确认检查风险状态规格(方法首节点, 方法存在, now);
        case 枚举_本能方法ID::安全_执行降级保护:
            return 确认执行降级保护规格(方法首节点, 方法存在, now);
        default:
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        const bool 条件结果阶段成功 = 规格已确认
            ...
            && 目标条件结果节点.结果节点
            && 条件结果对提交成功;
        auto* 提交方法状态变化方法 = 条件结果阶段成功
            ? 确保方法域提交本能方法(
                实际方法首节点,
                枚举_本能方法ID::自我_提交方法状态变化,
                now)
            : nullptr;
        auto* 提交方法状态变化运行 = 提交方法状态变化方法
            ? 本轮运行虚拟存在(提交方法状态变化方法, 输出结果场景, now)
            : nullptr;
        auto 方法状态提交 = 条件结果阶段成功
            ? 提交目标方法I64特征_方法域动作(
                提交方法状态变化方法,
                提交方法状态变化运行,
                目标方法,
        ...
        if (提交方法状态变化方法) {
            auto* 提交输出运行 = 输出运行虚拟存在(
                ...
                输出结果场景,
                now);
            更新方法运行账(提交方法状态变化方法, 提交输出运行, 方法状态提交.成功, now);
        }
        const bool 成功 = 条件结果阶段成功 && 方法状态提交.成功;
        const auto* 失败原因 = !输入包
                ? 值_输入条件包绑定失败()
                : (!目标方法
                    ? 值_目标方法头节点缺失()
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        if (!绑定.匹配成功) {
            ...
            if (失败目标方法 && 失败目标为方法动作补齐) {
                // 即使定向学习自身的条件绑定失败，也要把本轮明确目标写出；
                // 后续派生需求必须围绕“目标方法.方法动作状态=1”，不能退化为空目标。
                写入目标方法I64特征派生需求(
                    实际方法首节点,
                    失败目标方法,
                    特征_方法动作状态(),
        ...
        const bool 目标特征已确认 = 目标特征 || 目标为方法动作补齐;
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        auto* 练习状态提交方法 = 成功
            ? 确保方法域提交本能方法(
                ...
                now)
            : nullptr;
        auto* 练习状态提交运行 = 练习状态提交方法
            ? 本轮运行虚拟存在(练习状态提交方法, 输出结果场景, now)
            : nullptr;
        auto 练习状态提交 = 成功
            ? 提交目标方法I64特征_方法域动作(
                练习状态提交方法,
        ...
        if (练习状态提交方法) {
            auto* 提交输出运行 = 输出运行虚拟存在(
                ...
                输出结果场景,
                now);
            更新方法运行账(练习状态提交方法, 提交输出运行, 练习状态提交.成功, now);
        }
        const bool 最终成功 = 成功 && 练习状态提交.成功;
        const auto* 最终失败原因 = 成功 && !练习状态提交.成功
            ? (练习状态提交.失败原因 ? 练习状态提交.失败原因 : 值_方法域动作动态创建失败())
            : 失败原因;
        ...
        auto 写结果 = [&](基础信息节点类* 宿主, const char* 宿主标签) noexcept {
            ...
            (void)写入基础节点I64(宿主, 特征_方法稳定状态(), 最终成功 && 练习后方法状态 >= 方法状态_好用() ? 1 : 0, now);
    ...
    struct 结构_对象作用输入 {
        ...
        任务类::节点类* 任务信息节点 = nullptr;
        I64 目标值 = 1;
    };

    inline bool 对象作用特征被允许(
        const 语素入口节点类* 特征类型,
        const 语素入口节点类* 默认目标特征,
        std::initializer_list<const 语素入口节点类*> 附加结果特征集) noexcept
    {
        if (语素入口主键相同(特征类型, 默认目标特征)) {
            return true;
        }
        for (const auto* 附加特征 : 附加结果特征集) {
            if (语素入口主键相同(特征类型, 附加特征)) {
                return true;
            }
        }
        return false;
    ...
    inline 结构_对象作用输入 读取对象作用输入(
        场景节点类* 输入参数场景,
        const 语素入口节点类* 默认目标特征,
        std::initializer_list<const 语素入口节点类*> 附加结果特征集) noexcept
    {
        结构_对象作用输入 输入{};
        输入.输入包 = reinterpret_cast<基础信息节点类*>(输入条件包根(输入参数场景));
        输入.目标特征类型 = 默认目标特征;
        if (!输入.输入包) {
            return 输入;
        }
        ...
        void* 指针 = nullptr;
        if (读取包指针(输入.输入包, 特征_目标宿主(), 指针) && 指针) {
            输入.目标宿主 = reinterpret_cast<基础信息节点类*>(指针);
        }
        指针 = nullptr;
        if (读取包指针(输入.输入包, 特征_目标特征类型(), 指针) && 指针) {
            auto* 输入目标特征 = reinterpret_cast<const 语素入口节点类*>(指针);
            if (对象作用特征被允许(
                    输入目标特征,
                    默认目标特征,
                    附加结果特征集)) {
        ...
        指针 = nullptr;
        if (读取包指针(输入.输入包, 特征_来源需求(), 指针) && 指针) {
            输入.来源需求 = reinterpret_cast<需求类::节点类*>(指针);
        }
        指针 = nullptr;
        if (读取包指针(输入.输入包, 特征_任务信息节点(), 指针) && 指针) {
            输入.任务信息节点 = reinterpret_cast<任务类::节点类*>(指针);
        }
        I64 目标值 = 1;
        if (读取基础节点I64(输入.输入包, 特征_目标执行后值(), 目标值)) {
            输入.目标值 = 目标值;
        }
        return 输入;
    }

    inline 场景节点类* 对象作用输出场景(
        方法类::节点类* 方法首节点,
        存在节点类* 方法存在,
        场景节点类* 输出结果场景,
        时间戳 now) noexcept
    {
        ...
        if (auto* 方法结果场景 = 方法类::取或创建_方法结果场景(
                方法首节点,
                方法存在,
                now)) {
            return 方法结果场景;
        ...
        时间戳 now) noexcept
    {
        ...
        I64 旧值 = 0;
        const bool 有旧值 = 读取基础节点I64(目标宿主, 目标特征类型, 旧值);
        auto* 目标特征 = 世界树.确保特征(目标宿主, 目标特征类型);
        if (!目标特征) {
            return nullptr;
        }

        (void)写入基础节点I64(目标宿主, 目标特征类型, 目标值, now);
        (void)世界树.状态().创建内部状态(
            输出场景,
            目标宿主,
            目标特征,
            特征值{ 有旧值 ? 旧值 : static_cast<I64>(0) },
            枚举_存在状态事件::创建,
            false,
            now);
        auto* 结果状态 = 世界树.状态().创建内部状态(
            ...
            !有旧值 || 旧值 != 目标值,
            now);
        auto* 场景宿主 = reinterpret_cast<基础信息节点类*>(输出场景);
        if (结果状态) {
            (void)写入基础节点指针(场景宿主, 目标特征类型, 结果状态, now);
            if (是否主结果) {
                (void)写入基础节点指针(场景宿主, 特征_实际结果状态(), 结果状态, now);
            }
        }
        return 结果状态;
    }

    inline bool 写入对象作用I64派生需求(
        基础信息节点类* 目标主体,
        const 语素入口节点类* 特征类型,
        I64 当前值,
        I64 目标值,
        ...
        时间戳 now) noexcept
    {
        ...
        auto* 目标状态 = 世界树.状态().创建内部状态(
            ...
            now);
        auto* 场景宿主 = reinterpret_cast<基础信息节点类*>(输出场景);
        (void)写入基础节点指针(场景宿主, 特征_派生需求(), 特征类型, now);
        if (当前状态) {
            (void)写入基础节点指针(场景宿主, 特征_派生需求当前状态(), 当前状态, now);
        }
        if (目标状态) {
            (void)写入基础节点指针(场景宿主, 特征_派生需求目标状态(), 目标状态, now);
        }
        return 当前状态 && 目标状态;
    }

    inline 存在节点类* 执行对象作用布尔方法(
        方法类::节点类* 方法首节点,
        场景节点类* 输入参数场景,
        场景节点类* 输出结果场景,
        枚举_本能方法ID 本能ID,
        const char* 调用方,
        bool (*确认规格)(方法类::节点类*, 存在节点类*, 时间戳),
        const 语素入口节点类* 默认目标特征,
        std::initializer_list<const 语素入口节点类*> 附加结果特征集,
        bool 记录服务前状态场景) noexcept
    {
        const auto now = 结构体_时间戳::当前_微秒();
        auto 环境 = 确保本能方法自身环境(
            方法首节点,
        ...
        bool 附加结果成功 = true;
        for (const auto* 附加特征 : 附加结果特征集) {
            if (语素入口主键相同(附加特征, 输入.目标特征类型)) {
                continue;
            }
            附加结果成功 = 写入对象作用I64结果状态(
                输入.目标宿主,
                附加特征,
                1,
                实际输出场景,
        ...
        const bool 成功 = 输入.输入包
            && 输入.目标宿主
            && 输入.目标特征类型
            && 主结果状态
            && 附加结果成功;
        const auto* 失败原因 = !输入.输入包
            ? 值_输入条件包绑定失败()
            : (!输入.目标宿主
                ? 值_目标实例特征缺失()
                : (!输入.目标特征类型
                    ? 值_目标特征缺失()
                    : (!主结果状态 ? 值_执行失败() : nullptr)));

        auto 写回执 = [&](基础信息节点类* 宿主) noexcept {
            if (!宿主) return;
            (void)写入方法执行情况(宿主, 成功 ? 值_执行成功() : 值_执行失败(), now);
            (void)写入基础节点I64(宿主, 特征_方法运行结果状态(), 成功 ? 1 : 0, now);
            (void)写入基础节点指针(宿主, 特征_目标宿主(), 输入.目标宿主, now);
            (void)写入基础节点指针(宿主, 特征_目标特征类型(), 输入.目标特征类型, now);
            (void)写入基础节点指针(宿主, 特征_实际结果状态(), 主结果状态, now);
            (void)写入基础节点指针(宿主, 特征_来源需求(), 输入.来源需求, now);
            (void)写入基础节点指针(宿主, 特征_任务信息节点(), 输入.任务信息节点, now);
            if (记录服务前状态场景 && 实际输出场景) {
                (void)写入基础节点指针(宿主, 特征_服务前状态场景(), 实际输出场景, now);
            }
            if (失败原因) {
                (void)写入基础节点指针(宿主, 特征_失败原因(), 失败原因, now);
            }
        };
        写回执(reinterpret_cast<基础信息节点类*>(环境.本轮运行虚拟存在));
        写回执(reinterpret_cast<基础信息节点类*>(实际输出场景));
        ...
        日志 << "对象作用本能方法执行"
            ...
            << " | 成功=" << (成功 ? 1 : 0)
            << " | 目标宿主=" << 指针日志文本(输入.目标宿主)
            << " | 目标特征=" << (输入.目标特征类型 ? 输入.目标特征类型->获取主键() : std::string{})
            << " | 目标值=" << 输入.目标值
            << " | 结果状态=" << 指针日志文本(主结果状态);
        if (失败原因) {
            日志 << " | 失败原因=" << 失败原因->获取主键();
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        const bool 有基础服务目标证据 =
            ...
            && 读取基础节点I64(
                ...
                已有目标明确值)
            && 已有目标明确值 > 0;
        const bool 需要沟通交流 = !有基础服务目标证据;
        auto* 主结果状态 = 输入.输入包 && 输入.目标宿主 && 输入.目标特征类型
            ? 写入对象作用I64结果状态(
                输入.目标宿主,
                输入.目标特征类型,
                需要沟通交流 ? 1 : 0,
        ...
        if (需要沟通交流 && 输入.输入包 && 输入.目标宿主 && 实际输出场景) {
            (void)写入对象作用I64派生需求(
                输入.目标宿主,
                特征_沟通交流需求明确状态(),
                0,
    ...
    // 写入目标方法的动作句柄，并输出可验收的“方法动作状态=1”事实；方法可用
    // 仍由条件结果对和后续方法状态提交链决定。
    inline 存在节点类* 提交方法可执行入口(
        方法类::节点类* 方法首节点,
        场景节点类* 输入参数场景,
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        const bool 输入任务状态完成 =
            输入任务状态 == static_cast<I64>(枚举_任务状态::完成);
        const bool 任务已完成 =
            任务
            && 任务存在匹配
            && 输入任务状态完成
