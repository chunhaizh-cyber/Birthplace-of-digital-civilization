...
namespace {
    ...
    inline const 词性节点类* 特征_学习目标类型() noexcept
    {
        static const 词性节点类* s_词 = 语素集.添加词性词("学习目标类型", "名词");
        return s_词;
    }

    inline const 词性节点类* 特征_学习尝试结果() noexcept
    {
        static const 词性节点类* s_词 = 语素集.添加词性词("学习尝试结果", "名词");
        return s_词;
    }

    inline const 词性节点类* 特征_学习尝试状态() noexcept
    {
        return 自我特征定义类::类型_自我_尝试学习状态();
    }
    ...
    inline const 词性节点类* 特征_学习类别() noexcept
    {
        static const 词性节点类* s_词 = 语素集.添加词性词("学习类别", "名词");
        return s_词;
    }
    ...
    inline const 词性节点类* 特征_学习结构状态() noexcept
    {
        static const 词性节点类* s_词 = 语素集.添加词性词("学习结构状态", "名词");
        return s_词;
    }

    inline const 词性节点类* 特征_变化路径() noexcept
    {
        static const 词性节点类* s_词 = 语素集.添加词性词("变化路径", "名词");
        return s_词;
    }

    inline const 词性节点类* 特征_影响方向() noexcept
    {
        static const 词性节点类* s_词 = 语素集.添加词性词("影响方向", "名词");
        return s_词;
    }
    ...
    inline const 词性节点类* 值_动作主体缺失() noexcept
    {
        static const 词性节点类* s_词 = 语素集.添加词性词("动作主体缺失", "名词");
        return s_词;
    }
        ...
        时间戳 now) noexcept
    {
        ...
        ok = 声明形参规格(方法首节点, 特征_改变量(), 值类型_I64(), 值_来源_输入条件包(), false, now) && ok;
        ...
        const 词性节点类* 兼容特征类型) noexcept
    {
        ...
        if (auto* 节点 = 解析一个(主特征类型)) return 节点;
        return 解析一个(兼容特征类型);
    }
...
export namespace 自我动作实现模块 {
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        auto* 输出运行 = 输出配对参数结果(
            ...
            方法配对结果,
            成功 ? 可执行输入场景 : nullptr,
            成功 ? 方法查找情况_配对成功 : 方法查找情况_配对失败,
            成功 ? 值_执行成功() : 值_参数绑定失败(),
            成功 ? nullptr : 值_必需参数缺失(),
            绑定数量,
            缺失数量,
            now);
        更新方法运行统计(实际方法首节点, 输出运行, 成功, now);
        return 输出运行;
    }

    inline 存在节点类* 输出简单本能摘要(
        方法类::节点类* 方法首节点,
        存在节点类* 运行存在,
        场景节点类* 输入参数场景,
        场景节点类* 输出结果场景,
        ...
        时间戳 now) noexcept
    {
        auto 写摘要 = [&](基础信息节点类* 宿主) noexcept {
            ...
            (void)写入方法执行情况(宿主, 执行情况, now);
            if (状态特征) {
                (void)写入基础节点I64(宿主, 状态特征, 状态值, now);
            }
            if (失败原因) {
                (void)写入基础节点指针(宿主, 特征_失败原因(), 失败原因, now);
            }
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        auto 写结果 = [&](基础信息节点类* 宿主) noexcept {
            ...
            (void)写入基础节点I64(宿主, 特征_配对状态(), 成功 ? 方法查找情况_配对成功 : 方法查找情况_配对失败, now);
        ...
        时间戳 now) noexcept
    {
        ...
        if (auto* 结果根 = 方法规格根(方法首节点, 特征_结果表(), now)) {
            auto* 宿主 = reinterpret_cast<基础信息节点类*>(结果根);
            ok = 取或创建子特征(宿主, 特征_学习类别()) && ok;
            ok = 取或创建子特征(宿主, 特征_学习尝试结果()) && ok;
            ok = 取或创建子特征(宿主, 特征_学习结构状态()) && ok;
    ...
    inline bool 方法具有本能执行入口(方法类::节点类* 方法首节点) noexcept
    {
        return 方法首节点
            && 方法首节点->主信息.动作句柄.类型 == 枚举_动作句柄类型::本能函数ID
            && 方法首节点->主信息.动作句柄.本能ID != 0;
    }

    inline 基础信息节点类* 读取学习证据节点(基础信息节点类* 输入包) noexcept
    {
        if (!输入包) return nullptr;
        const 词性节点类* 特征集[] = {
            特征_来源因果信息(),
    ...
    inline 基础信息节点类* 读取学习目标特征节点(基础信息节点类* 输入包) noexcept
    {
        if (!输入包) return nullptr;
        const 词性节点类* 特征集[] = {
            特征_目标抽象特征(),
        ...
        特征节点类*& 限制包) noexcept
    {
        ...
        (void)写入基础节点I64(宿主, 特征_学习结构状态(), 1, now);
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        if (条件结果对) {
            ...
            (void)写入基础节点I64(对宿主, 特征_学习类别(), 1, now);
            (void)写入基础节点I64(对宿主, 特征_学习结构状态(), 1, now);
            if (因果证据) {
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        auto* 目标方法 = 读取目标方法头节点(输入包);
        auto* 目标方法存在 = 目标方法 ? 方法虚拟存在(目标方法, now) : nullptr;
        auto* 目标特征 = 读取学习目标特征节点(输入包);
        auto* 观察证据 = 读取学习证据节点(输入包);
        ...
        if (条件结果对) {
            auto* 对宿主 = reinterpret_cast<基础信息节点类*>(条件结果对);
            (void)写入基础节点I64(对宿主, 特征_学习类别(), 2, now);
            (void)写入基础节点I64(对宿主, 特征_学习结构状态(), 1, now);
        ...
        auto 写结果 = [&](基础信息节点类* 宿主) noexcept {
            ...
            const auto* 执行情况 = 成功
                ...
                : ((!输入包 || !目标方法 || !目标特征)
                    ...
                    : 值_执行失败());
            (void)写入方法执行情况(宿主, 执行情况, now);
            (void)写入基础节点I64(宿主, 特征_学习类别(), 2, now);
            (void)写入基础节点I64(宿主, 特征_学习尝试状态(), 成功 ? 1 : 0, now);
            (void)写入基础节点I64(宿主, 特征_学习结构状态(), 已补结构 ? 1 : 0, now);
            (void)写入基础节点指针(宿主, 特征_目标方法头节点(), 目标方法, now);
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        const auto 目标本能ID = 有入口
            ? static_cast<枚举_本能方法ID>(目标方法->主信息.动作句柄.本能ID)
            : 枚举_本能方法ID::未定义;
        const bool 可试运行 = 有入口
            && 目标方法 != 实际方法首节点
            ...
            (void)写入方法执行情况(
                ...
                成功 ? 值_执行成功()
                    : ((!输入包 || !目标方法 || 缺失参数数量 > 0)
                        ...
                        : 值_执行失败()),
                now);
            (void)写入基础节点I64(宿主, 特征_学习类别(), 3, now);
            (void)写入基础节点I64(宿主, 特征_学习尝试状态(), 成功 ? 1 : 0, now);
            (void)写入基础节点I64(宿主, 特征_学习结构状态(), 成功 ? 2 : (已补结构 ? 1 : 0), now);
            (void)写入基础节点I64(宿主, 特征_试运行次数(), 目标方法运行结果 ? 1 : 0, now);
            (void)写入基础节点I64(宿主, 特征_成功次数(), 目标执行成功 ? 1 : 0, now);
            (void)写入基础节点I64(宿主, 特征_失败次数(), (目标方法运行结果 && !目标执行成功) ? 1 : 0, now);
