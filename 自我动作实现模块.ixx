...
export namespace 自我动作实现模块 {
    ...
    {
        return 全局_语素入口同一(失败原因, 值_执行失败())
            || 全局_语素入口同一(失败原因, 值_方法配对结果创建失败())
            || 全局_语素入口同一(失败原因, 值_可执行输入参数场景创建失败())
            || 全局_语素入口同一(失败原因, 值_方法域动作动态创建失败())
            || 全局_语素入口同一(失败原因, 服务模块::值_服务字段绑定写入失败());
    }
    ...
    inline bool 参数值类型为I64(基础信息节点类* 形参节点) noexcept
    {
        ...
        return 全局_语素入口同一(值类型, 值类型_I64())
            || 全局_语素入口同一(值类型, 值类型_类型I64());
    }
        ...
        I64& 上界) noexcept
    {
        ...
        if (全局_语素入口同一(参数特征类型, 特征_练习方向())
            || 全局_语素入口同一(参数特征类型, 特征_改变方向())) {
            下界 = -1;
            上界 = 1;
            return true;
        }
        ...
        时间戳 now) noexcept
    {
        ...
        for (auto* 子特征 : 世界树.特征().获取子特征(回执宿主)) {
            ...
            if (!主信息 || !全局_语素入口同一(主信息->类型, 特征_候选方法头节点())) {
                continue;
            }
