...
export namespace 自我动作实现模块 {
        ...
        I64& 输出候选实参数量) noexcept
    {
        ...
        const bool 参数闭合 = 练习输入参数闭合(输入包);
        输出练习模式 = 参数闭合 ? 1 : 2;
        输出候选实参数量 = 已生成数量 + 1;

        auto* 宿主 = reinterpret_cast<基础信息节点类*>(实参表);
        (void)写入基础节点I64(宿主, 特征_练习模式(), 输出练习模式, now);
        (void)写入基础节点I64(宿主, 特征_实参生成策略(), 参数闭合 ? 1 : 2, now);
        (void)写入基础节点I64(宿主, 特征_参数生成序号(), 输出候选实参数量, now);
        (void)写入基础节点I64(宿主, 特征_参数新颖性状态(), 1, now);

        if (输入包) {
            (void)复制包指针(输入包, 特征_目标特征类型(), 宿主, 特征_目标特征类型(), now);
            (void)复制包指针(输入包, 特征_目标抽象特征(), 宿主, 特征_目标抽象特征(), now);
            (void)复制包指针(输入包, 特征_目标特征(), 宿主, 特征_目标特征(), now);
            (void)复制包I64(输入包, 特征_目标执行后值(), 宿主, 特征_目标执行后值(), now);
        }
        ...
        (void)写入基础节点I64(宿主, 特征_执行差值(), 执行差值, now);
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        if (目标方法存在) {
            ...
            已补结构 = 取或创建子特征(宿主, 特征_方法能力包()) != nullptr;
            已补结构 = 取或创建子特征(宿主, 特征_形参表()) != nullptr || 已补结构;
            已补结构 = 取或创建子特征(宿主, 特征_结果表()) != nullptr || 已补结构;
            条件结果对 = 取或创建子特征(宿主, 特征_条件结果对());
            已补结构 = 条件结果对 != nullptr || 已补结构;
            已补结构 = 取或创建子特征(宿主, 特征_试运行限制包()) != nullptr || 已补结构;
            候选实参表 = 生成练习候选实参表(
                宿主,
                输入包,
                now,
        ...
        const bool 有入口 = 目标方法
            && 目标方法->主信息.动作句柄.类型 == 枚举_动作句柄类型::本能函数ID
            && 目标方法->主信息.动作句柄.本能ID != 0;
        const bool 成功 = 规格已确认 && 目标方法 && 已补结构;
        const auto* 失败原因 = !目标方法
            ? 值_目标方法头节点缺失()
            : (!有入口 ? 值_缺少执行入口() : nullptr);
        auto 写结果 = [&](基础信息节点类* 宿主) noexcept {
            if (!宿主) return;
            (void)写入方法执行情况(宿主, 成功 ? 值_执行成功() : 值_参数绑定失败(), now);
            (void)写入基础节点I64(宿主, 特征_学习尝试状态(), 成功 ? 1 : 0, now);
            (void)写入基础节点I64(宿主, 特征_试运行次数(), 0, now);
            (void)写入基础节点I64(宿主, 特征_成功次数(), 0, now);
            (void)写入基础节点I64(宿主, 特征_失败次数(), 0, now);
