...
namespace {
        ...
        规格确认函数 确认规格) noexcept
    {
        ...
        auto 输出慢环境日志 = [&](const char* 结果) noexcept {
            if (!调用方 || std::strcmp(调用方, "自我_提交任务状态变化") != 0) {
                return;
            }
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        if (状态有效) {
            ...
            if (状态变化 || 阶段变化) {
                ...
                if (任务状态提交诊断日志启用()) {
                    std::ostringstream 输出;
                    输出 << "任务状态提交/无有效状态阶段变化跳过动作动态"
                        << " | 任务=" << 任务->获取主键()
                        << " | 状态=" << static_cast<int>(新状态)
                        << " | 阶段=" << static_cast<int>(旧阶段)
                        << " | 建议阶段=" << static_cast<int>(新阶段);
                    项目运行日志(输出.str());
                }
            }
        }
