...
namespace {
    ...
    inline const 语素入口节点类* 特征_服务对象状态变化数() noexcept
    {
        static const 语素入口节点类* s_词 = 语素集.添加信息入口词("服务对象状态变化数", 枚举_信息入口类型::特征模板入口);
        return s_词;
    }

    inline const 语素入口节点类* 特征_服务对象状态改善证据() noexcept
    {
        static const 语素入口节点类* s_词 = 语素集.添加信息入口词("服务对象状态改善证据", 枚举_信息入口类型::特征模板入口);
        return s_词;
    }
    ...
    inline const 语素入口节点类* 特征_服务对象当前状态明确状态() noexcept
    {
        static const 语素入口节点类* s_词 = 语素集.添加信息入口词("服务对象当前状态明确状态", 枚举_信息入口类型::特征模板入口);
        return s_词;
    }
...
export namespace 自我动作实现模块 {
        ...
        时间戳 now) noexcept
    {
        ...
        ok = 声明形参规格(方法首节点, 特征_服务对象状态变化数(), 值类型_I64(), 值_来源_输入条件包(), false, now) && ok;
        ok = 声明形参规格(方法首节点, 特征_服务对象状态改善证据(), 值类型_I64(), 值_来源_输入条件包(), false, now) && ok;
        ...
        ok = 声明条件特征(方法首节点, 特征_服务对象状态变化数(), now) && ok;
        ok = 声明条件特征(方法首节点, 特征_服务对象状态改善证据(), now) && ok;
        ...
        if (auto* 回执根 = 方法规格根(方法首节点, 特征_执行回执表(), now)) {
            ...
            ok = 取或创建子特征(宿主, 特征_实际结果状态()) && ok;
            ok = 取或创建子特征(宿主, 特征_服务对象状态变化数()) && ok;
            ok = 取或创建子特征(宿主, 特征_服务对象状态改善证据()) && ok;
        ...
        时间戳 now) noexcept
    {
        ...
        case 枚举_本能方法ID::服务_读取服务对象当前状态:
            return 确认读取服务对象当前状态规格(方法首节点, 方法存在, now);
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        (void)读取基础节点I64(输入条件, 特征_服务对象状态变化数(), 服务对象状态变化数);
        (void)读取基础节点I64(输入条件, 特征_服务对象状态改善证据(), 服务对象状态改善证据);
        ...
        if (成功 && 计划服务增量 > 0 && !服务结算证据通过) {
            ...
            证据日志 << "自我动作/结算叶子任务价值服务结算证据不足"
                ...
                << " | 原计划服务增量=" << 计划服务增量
                << " | 服务对象状态变化数=" << 服务对象状态变化数
                << " | 改善证据=" << (服务对象状态改善证据 > 0 ? "有" : "缺")
                << " | 可验证=" << (服务结果可验证 > 0 ? "是" : "否")
                << " | 可归因=" << (服务结果可归因于自我动作 > 0 ? "是" : "否")
                << " | 说明=服务对象改善/验证/归因证据不足，服务增量按0结算";
            项目运行警告日志(证据日志.str());
        }
        ...
        if (运行宿主) {
            ...
            (void)写入基础节点I64(运行宿主, 特征_服务对象状态变化数(), 服务对象状态变化数, now);
            (void)写入基础节点I64(
                运行宿主,
                特征_服务对象状态改善证据(),
                服务对象状态改善证据 > 0 ? 1 : 0,
