...
namespace {
    ...
    // 功能：服务所在模块的内部辅助流程。
    std::string 私有_词标题(const 语素入口节点类* 词)
    {
    ...
    // 功能：服务所在模块的内部辅助流程。
    const 语素入口节点类* 私有_推导动态名称类型(const 动态节点主信息类& 主信息) noexcept
    {
        if (主信息.来源动作名) {
            return 主信息.来源动作名;
        }
        ...
        return 语素集.添加信息入口词("动态", 枚举_信息入口类型::动态模板入口);
    }
...
// 功能：按函数名执行对应处理。
枚举_动态业务语义类型 动态类::推导业务语义类型(const 动态节点主信息类& 主信息)
{
    return 私有_推导动态业务语义类型(主信息);
}
    ...
    const 动态创建后处理选项& 后处理选项)
{
    ...
    动态名称补齐耗时微秒 = 私有_动态阶段耗时微秒(
        动态名称补齐开始,
        私有_动态阶段时间点());
    const auto 业务语义刷新开始 = 私有_动态阶段时间点();
    私有_刷新动态业务语义类型(主信息);
    业务语义刷新耗时微秒 = 私有_动态阶段耗时微秒(
        业务语义刷新开始,
        私有_动态阶段时间点());
...
bool 动态类::设置业务语义类型(动态节点类* 节点, 枚举_动态业务语义类型 类型)
{
    ...
    const auto 推导类型 = 推导业务语义类型(*主信息);
    主信息->业务语义类型 = 推导类型 != 枚举_动态业务语义类型::未定义
        ? 推导类型
        : 类型;
    return true;
}
