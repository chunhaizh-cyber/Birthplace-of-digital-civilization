...
namespace {
    ...
    // 功能：按动态类别取得对应类型入口。
    const 语素入口节点类* 私有_动态类别入口(枚举_动态因果生成类别 类别) noexcept
    {
        switch (类别) {
        case 枚举_动态因果生成类别::主动动作致变:
            return 语素集.添加信息入口词("主动动作动态", 枚举_信息入口类型::动态模板入口);
        case 枚举_动态因果生成类别::被动动作致变:
    ...
    // 功能：返回动态到因果生成分类的日志文本。
    const char* 私有_动态因果生成类别文本(枚举_动态因果生成类别 类别) noexcept
    {
        switch (类别) {
    ...
    // 功能：按动态结构字段推导动态因果生成类别。
    枚举_动态因果生成类别 私有_推导动态因果生成类别(const 动态节点主信息类& 主信息) noexcept
    {
        if (主信息.来源动作.指针) {
            return 枚举_动态因果生成类别::主动动作致变;
        }
        if (主信息.来源动作名
            || 主信息.来源动作相位 != 枚举_动作事件相位::未定义
    ...
    {
        return 私有_动态类别入口(私有_推导动态因果生成类别(主信息));
    }
...
// 功能：按动态结构字段推导动态因果生成类别。
枚举_动态因果生成类别 动态类::推导因果生成类别(const 动态节点主信息类& 主信息) noexcept
{
    return 私有_推导动态因果生成类别(主信息);
}
...
// 功能：返回动态因果生成分类的日志文本。
const char* 动态类::动态因果生成类别文本(枚举_动态因果生成类别 类别) noexcept
{
    return 私有_动态因果生成类别文本(类别);
}
