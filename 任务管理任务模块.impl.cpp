...
namespace {
    ...
    {
        auto* 管理任务 = 私有_校验自我任务树节点(
            reinterpret_cast<任务节点*>(自我对象.获取任务管理任务()));
        auto* 传入任务头 = 私有_归一任务管理动作任务头(
            reinterpret_cast<任务节点*>(上下文.任务节点 ? 上下文.任务节点 : 上下文.任务));
        ...
        T执行体&& 执行体) noexcept
    {
        return 取本能动作集().注册(
            ...
            [&自我对象, 本能ID, 执行体 = std::forward<T执行体>(执行体)](
                结构_本能动作执行上下文& 上下文) mutable {
                结构_本能动作步骤结果 动作结果{};
                动作结果.动作ID = 本能ID;
                动作结果.结果存在集场景 = 上下文.输出场景 ? 上下文.输出场景 : 上下文.场景;
                动作结果.结果事件集场景 = 动作结果.结果存在集场景;
                ...
                私有_同步治理运行视图(治理结果);
                动作结果.成功码 = 成功 ? 0 : 本能动作错误_执行失败;
                动作结果.质量_Q10000 = 成功 ? 10000 : 0;
                动作结果.需求当前状态 = 治理结果.最新状态;
                动作结果.需求目标状态 = 治理结果.最新目标状态;
                动作结果.备注 = 私有_任务管理本能动作备注(本能ID, 治理结果);
                return 动作结果;
            },
            私有_治理本能方法文本(本能ID));
    }
