...
namespace {
    ...
    bool 私有_P0_是否属于升阶同向候选(const 任务节点* 节点) noexcept;
        ...
        结构_原语判定结果& 输出) noexcept
    {
        ...
        case 枚举_原语类型::冲突判定: {
            ...
            输出.是否冲突 = 存在显式冲突;
            输出.是否满足 = 存在显式冲突;
            输出.满足度_Q10000 = 存在显式冲突 ? 10000 : 3000;
            输出.置信度_Q10000 = 存在显式冲突 ? 9000 : 7000;
            输出.原因说明 = 存在显式冲突
                ? "当前已进入纠偏/禁止继续类控制边界"
                : "当前未发现显式反向控制信号";
            输出.下一步建议 = 存在显式冲突 ? "转入纠偏或重筹办" : "维持当前选择链";
            return true;
        }
        case 枚举_原语类型::因果稳定: {
            输出.主域 = "验证";
            输出.小域 = "因果";
            输出.是否满足 = 影子验证稳定;
            输出.满足度_Q10000 = 影子验证稳定 ? 8000 : 2000;
            输出.置信度_Q10000 = 影子验证稳定 ? 7000 : 5000;
            输出.原因说明 = 影子验证稳定
                ? "当前影子验证已给出稳定/通过类信号"
                : "当前尚无稳定因果验证信号";
            输出.下一步建议 = 影子验证稳定 ? "可结合结果符合进入收束" : "继续保留验证";
            return true;
        }
        case 枚举_原语类型::升阶判定: {
            输出.主域 = "细化";
            输出.小域 = "升阶";
            const auto 同向候选数 = 私有_P0_统计升阶同向候选数(请求);
            const auto 需求轴 =
                请求.当前需求轴键.empty()
                ? std::string("未定义需求轴")
                : 请求.当前需求轴键;
            const bool 需升阶 = 同向候选数 >= 2 && !存在显式冲突;
            输出.需升阶 = 需升阶;
            输出.是否满足 = 需升阶;
            输出.满足度_Q10000 = 需升阶 ? 10000 : 0;
            输出.置信度_Q10000 = 需升阶 ? 6500 : 6000;
            输出.原因说明 = 需升阶
                ? "需求轴[" + 需求轴 + "]下已出现多个同向二次特征候选，需继续排序谁更接近需求"
                : (
                    同向候选数 == 1
                    ? "需求轴[" + 需求轴 + "]下当前仅有单个同向候选，暂不触发高阶排序"
                    : "需求轴[" + 需求轴 + "]下当前尚未形成可比较的同向候选组"
                );
            输出.下一步建议 = 需升阶 ? "触发更高阶候选排序" : "继续沿当前层判定";
            return true;
        }
        default:
            return false;
        ...
        结构_原语判定汇总& 输出) noexcept
    {
        ...
        for (const auto& 结果 : 结果集) {
            ...
            case 枚举_原语类型::因果稳定:
                ...
                break;
            case 枚举_原语类型::升阶判定:
                输出.需要升阶 = 结果.需升阶 || 结果.是否满足;
                if (输出.升阶原因.empty() && 输出.需要升阶) {
                    输出.升阶原因 = 结果.原因说明;
                }
                break;
            default:
    ...
    bool 私有_稳定等待态复盘学习(
        const 任务节点* 宿主任务,
        枚举_任务管理缺口类型 当前缺口类型,
        枚举_任务管理下一步去向 当前下一步去向,
        枚举_任务管理方法来源 当前方法来源,
        const 方法节点* 当前方法,
        bool 宿主目标结果待推进) noexcept;
        ...
        枚举_动作事件相位 来源相位) noexcept
    {
        ...
        auto* 因果节点 = 世界树.创建因果实例(场景, 因果主信息);
        if (调试M10回填) {
            项目运行日志("任务管理/M10回填 | 主体桥接指针-因果实例创建完成");
        }
        if (因果节点) {
            (void)世界树.因果().绑定动作主体语义(因果节点, 任务虚拟存在, 特征节点, 动作语义键);
            const bool 已从动态回带主结果 =
    ...
    void 私有_P0_刷新请求原语锚点(结构_任务管理请求& 请求) noexcept
    {
        ...
        const auto 追加候选 = [&](任务节点* 节点, const char* 角色) noexcept {
            if (!节点) {
                return;
            }
            ...
            auto 已有 = std::find_if(
                请求.当前升阶候选组.begin(),
                请求.当前升阶候选组.end(),
                [&](const 结构_原语候选项& 项) noexcept { return 项.节点 == 节点; });
            if (已有 != 请求.当前升阶候选组.end()) {
                if (角色 && *角色) {
                    const std::string 角色文本 = 角色;
                    if (已有->候选角色.find(角色文本) == std::string::npos) {
                        if (!已有->候选角色.empty()) {
                            已有->候选角色 += "/";
                        }
                        已有->候选角色 += 角色文本;
                ...
                已有->是否同向 = 已有->是否同向 || 私有_P0_是否属于升阶同向候选(节点);
                if (已有->候选主键.empty()) {
                    已有->候选主键 = 节点->获取主键();
                }
                return;
    ...
    {
        if (!请求.当前升阶候选组.empty()) {
            std::vector<const 任务节点*> 候选{};
            候选.reserve(请求.当前升阶候选组.size());
            for (const auto& 项 : 请求.当前升阶候选组) {
                if (!项.是否同向 || !项.节点) {
                    continue;
                }
                if (std::find(候选.begin(), 候选.end(), 项.节点) == 候选.end()) {
                    候选.emplace_back(项.节点);
                }
            }
            return 候选.size();
        }
        ...
        const auto 追加候选 = [&](const 任务节点* 节点) noexcept {
            if (!私有_P0_是否属于升阶同向候选(节点)) {
                return;
            }
            for (const auto* 已有 : 候选) {
                if (已有 == 节点) {
                    return;
                }
            }
        ...
        const 结构_任务管理结果& 结果) noexcept
    {
        ...
        const auto* 当前方法 = 私有_P0_当前方法(结果);
        const bool 稳定等待态复盘学习 = 私有_稳定等待态复盘学习(
            宿主任务,
            当前缺口类型,
            当前下一步去向,
            当前方法来源,
        ...
        时间戳 now) noexcept
    {
        ...
        输入.去重键 = 学习子任务->获取主键();
        输入.学习子任务 = 学习子任务;
        输入.学习方法 = 当前选中学习方法 ? 当前选中学习方法 : 学习任务兜底方法;
        输入.学习任务兜底方法 = 学习任务兜底方法;
        输入.当前选中学习方法 = 当前选中学习方法 ? 当前选中学习方法 : 学习任务兜底方法;
        输入.来源宿主任务 = 宿主任务;
        输入.来源步骤 = 当前步骤;
        输入.来源结果 = 最近结果;
        输入.来源方法 = 当前方法;
        ...
        输入.仅需求位方法头 = 本能补齐学习 ? false : 私有_学习方法仅需求位(输入.学习方法);
        输入.当前安全值 = 自我对象.获取安全值();
        输入.当前服务值 = 自我对象.获取服务值();
        ...
        bool 允许业务派生) noexcept
    {
        ...
        auto* 当前轮学习方法 = 学习承接决策.是否触发学习
            ? 私有_写回学习方法骨架(
                ...
                调用点 + "/学习方法候选")
            : nullptr;
        auto* 学习方法 = 当前轮学习方法 ? 当前轮学习方法 : 学习任务兜底方法;
    ...
    *输出 = {};
    ...
    输出->当前学习承接类型 = 输出->已触发学习承接
        ? (私有_稳定等待态复盘学习(
            ...
            决策.当前方法来源,
            请求.上下文.当前方法,
            请求.宿主任务 ? 私有_任务节点目标结果待推进(请求.宿主任务) : false)
            ? 枚举_任务管理学习承接类型::广泛学习
            : 枚举_任务管理学习承接类型::定向学习)
        : 枚举_任务管理学习承接类型::未定义;
    输出->当前学习为本能补齐 =
    ...
    任务运行包::结构_一步治理任务包* 输出) noexcept
{
    ...
    if (请求.上下文.当前方法) {
        任务运行包::结构_方法候选项 方法候选{};
        方法候选.方法主键 = 私有_运行包主键_按指针(请求.上下文.当前方法);
        方法候选.置信度 = 私有_映射运行包方法置信度(决策.当前方法来源);
        方法候选.命中原因 = 私有_方法来源文本(决策.当前方法来源);
        任务包.候选方法集.push_back(方法候选);
        任务包.选定方法 = 方法候选;
    }
