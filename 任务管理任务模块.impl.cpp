...
namespace {
    ...
    enum class 枚举_任务管理本体阶段 : std::uint8_t {
        受理请求 = 0,
        宿主锚点确认 = 1,
        绑定宿主方法 = 2,
        执行入口闸门 = 3,
    ...
    struct 结构_任务管理本体阶段记录 {
        枚举_任务管理本体阶段 阶段 = 枚举_任务管理本体阶段::受理请求;
        bool 成功 = false;
    };
    ...
    struct 结构_M2治理阶段状态 {
        ...
        bool 已受上位输入约束 = false;
        bool 桥接补层允许继续 = false;
        std::vector<结构_任务管理本体阶段记录> 本体阶段轨迹{};
    };
    ...
    {
        return 语素集.添加词性词("任务虚拟存在_" + 私有_任务标题(节点), "专有名词");
    }
    ...
    {
        return 语素集.添加词性词("任务管理治理步骤_" + 私有_步骤名称前缀文本(结果), "名词");
    }
    ...
    {
        return 语素集.添加词性词("任务管理治理结果_" + 私有_结果名称前缀文本(结果), "名词");
    }
    ...
    std::string 私有_任务管理本体阶段文本(枚举_任务管理本体阶段 阶段) noexcept
    {
        switch (阶段) {
        case 枚举_任务管理本体阶段::受理请求: return "受理请求";
        case 枚举_任务管理本体阶段::宿主锚点确认: return "宿主锚点确认";
        case 枚举_任务管理本体阶段::绑定宿主方法: return "绑定宿主方法";
        case 枚举_任务管理本体阶段::执行入口闸门: return "执行入口闸门";
        case 枚举_任务管理本体阶段::执行后段治理: return "执行后段治理";
        case 枚举_任务管理本体阶段::学习承接与业务派生: return "学习承接与业务派生";
        case 枚举_任务管理本体阶段::写回结果: return "写回结果";
        case 枚举_任务管理本体阶段::组装一步结果: return "组装一步结果";
        default: return "未定义阶段";
        }
    }

    枚举_任务状态 私有_任务管理本体阶段结果状态(
        枚举_任务管理本体阶段 阶段,
        const 结构_任务管理结果& 结果,
        bool 阶段成功) noexcept
    {
        ...
        switch (阶段) {
        case 枚举_任务管理本体阶段::执行入口闸门:
        case 枚举_任务管理本体阶段::执行后段治理:
        case 枚举_任务管理本体阶段::学习承接与业务派生:
        case 枚举_任务管理本体阶段::写回结果:
        case 枚举_任务管理本体阶段::组装一步结果:
            if (结果.宿主任务新状态 == 枚举_任务状态::失败) {
                return 枚举_任务状态::失败;
            }
            if (私有_P0_当前下一步去向(结果) == 枚举_任务管理下一步去向::保持等待
    ...
    const 词性节点类* 私有_任务管理本体阶段步骤类型词(枚举_任务管理本体阶段 阶段) noexcept
    {
        return 语素集.添加词性词(
            "任务管理阶段步骤_" + 私有_任务管理本体阶段文本(阶段),
            "名词");
    }

    const 词性节点类* 私有_任务管理本体阶段结果类型词(枚举_任务管理本体阶段 阶段) noexcept
    {
        return 语素集.添加词性词(
            "任务管理阶段结果_" + 私有_任务管理本体阶段文本(阶段),
            "名词");
    }

    const 词性节点类* 私有_任务管理本体阶段步骤名称词(
        枚举_任务管理本体阶段 阶段,
        const 任务节点* 步骤节点,
        const 结构_任务管理结果& 结果) noexcept
    {
        std::ostringstream 输出;
        输出 << "任务管理步骤_" << 私有_任务管理本体阶段文本(阶段)
            << "_" << 私有_任务标题(私有_P0_当前宿主任务(结果));
        if (步骤节点) {
            输出 << "_第" << 步骤节点->主信息.步骤序号 << "步";
        }
        return 语素集.添加词性词(输出.str(), "专有名词");
    }

    const 词性节点类* 私有_任务管理本体阶段结果名称词(
        枚举_任务管理本体阶段 阶段,
        const 任务节点* 步骤节点,
        const 结构_任务管理结果& 结果) noexcept
    {
        std::ostringstream 输出;
        输出 << "任务管理结果_" << 私有_任务管理本体阶段文本(阶段)
            << "_" << 私有_任务标题(私有_P0_当前宿主任务(结果))
            << "_" << 私有_去向文本(私有_P0_当前下一步去向(结果));
        if (步骤节点) {
            输出 << "_第" << 步骤节点->主信息.步骤序号 << "步";
        ...
        const std::string& 调用点) noexcept
    {
        ...
        auto* 结果节点 = 临时树.创建任务结果节点(
            ...
            now,
            调用点 + "/追加任务管理阶段结果");
        const auto 阶段结果状态 = 私有_任务管理本体阶段结果状态(阶段, 结果, 阶段成功);

        auto& 步骤主信息 = 步骤节点->主信息;
        步骤主信息.名称 = 私有_任务管理本体阶段步骤名称词(阶段, 步骤节点, 结果);
        步骤主信息.类型 = 私有_任务管理本体阶段步骤类型词(阶段);
        步骤主信息.节点种类 = 枚举_任务节点种类::步骤节点;
        步骤主信息.状态 = 阶段结果状态 == 枚举_任务状态::失败
            ? 枚举_任务状态::失败
            : 枚举_任务状态::完成;
        if (步骤主信息.启动时间 == 0) {
            步骤主信息.启动时间 = now;
        ...
        私有_写入引用(步骤主信息.来源需求, 结果.上下文.当前主需求);
        ...
        if (结果节点) {
            auto& 结果主信息 = 结果节点->主信息;
            结果主信息.名称 = 私有_任务管理本体阶段结果名称词(阶段, 步骤节点, 结果);
            结果主信息.类型 = 私有_任务管理本体阶段结果类型词(阶段);
            ...
            结果主信息.置信度_Q10000 = 阶段结果状态 == 枚举_任务状态::失败 ? 0 : 10000;
            私有_写入引用(结果主信息.来源需求, 结果.上下文.当前主需求);
    ...
    void 私有_记录任务管理本体阶段(
        ...
        bool 成功) noexcept
    {
        阶段状态.本体阶段轨迹.push_back({ 阶段, 成功 });
    }

    void 私有_为任务管理任务筹办阶段步骤链(
        自我类& 自我对象,
        结构_M2治理阶段状态& 阶段状态,
        const 结构_任务管理结果& 结果,
        时间戳 now,
        const std::string& 调用点) noexcept
    {
        if (!阶段状态.当前管理任务 || 阶段状态.本体阶段轨迹.empty()) {
            return;
        }

        for (const auto& 记录 : 阶段状态.本体阶段轨迹) {
            (void)私有_追加任务管理本体阶段步骤(
                自我对象,
                阶段状态.当前管理任务,
                记录.阶段,
                结果,
        ...
        const std::string& 调用点) noexcept
    {
        ...
        auto* 前置步骤 = 私有_前置任务挂接步骤(结果, 宿主任务);
        auto* 挂接父节点 = 前置步骤 ? 前置步骤 : 宿主任务;

        auto* 现有子任务 = 私有_查找直接子节点_按谓词<任务节点>(挂接父节点, [&](const 任务节点* 节点) noexcept {
            return 节点
                && 节点->主信息.节点种类 == 枚举_任务节点种类::头结点
                && 私有_词性相同(节点->主信息.名称, 私有_补条件子任务名称词(宿主任务, 结果))
                && !私有_任务状态已终结(节点->主信息.状态);
        });
        if (!现有子任务 && 前置步骤 && 挂接父节点 != 宿主任务) {
            现有子任务 = 私有_查找直接子节点_按谓词<任务节点>(宿主任务, [&](const 任务节点* 节点) noexcept {
                return 节点
                    && 节点->主信息.节点种类 == 枚举_任务节点种类::头结点
                    && 私有_词性相同(节点->主信息.名称, 私有_补条件子任务名称词(宿主任务, 结果))
                    && !私有_任务状态已终结(节点->主信息.状态);
        ...
        私有_写入引用(主信息.来源需求, 结果.上下文.当前主需求);
        私有_写入引用(主信息.父任务, 挂接父节点);
        私有_写入引用(主信息.父任务头结点, 宿主任务);
        if (前置步骤) {
            私有_写入引用(主信息.来源父步骤节点, 前置步骤);
        }
        私有_写入引用(主信息.场景, 场景);

        auto* 子任务头 = 私有_独立树添加子节点<任务类>(挂接父节点, 主信息);
        if (!子任务头) {
            return nullptr;
        }

        私有_登记步骤前置任务(前置步骤, 子任务头);
        结果.当前子任务节点 = 子任务头;
        结果.已创建补条件子任务 = true;
        return 子任务头;
    }
    ...
    结构_任务管理学习推进结果* 输出) noexcept
{
    ...
    if (学习调度模块::选择一条可执行学习任务(&调度项, &调度快照)) {
        ...
        结果.已写回学习方法结构 = 执行结果.已写回学习方法结构;
        结果.已补动作骨架 = 执行结果.已补动作骨架;
        结果.已补条件节点 = 执行结果.已补条件节点;
        结果.已补结果节点 = 执行结果.已补结果节点;
        结果.学习执行摘要 = 执行结果.摘要;
