...
namespace {
        ...
        时间戳 now) noexcept
    {
        ...
        结果.记录状态 = 被动识别结果.已运行
            ? 观察内部治理记录状态_已生成
            : 观察内部治理记录状态_条件不足;
        if (!目标宿主 || !被动识别结果.已运行 || !被动识别结果.材料可交付观察方法) {
            return 结果;
        }
        if (被动识别结果.同一性状态 == 外设观察存在自我存在同一性状态_冲突
            || 被动识别结果.同一性状态 == 外设观察存在自我存在同一性状态_证据不足
            || 被动识别结果.归属状态 == 外设观察存在自我归属状态_冲突
            || 被动识别结果.归属状态 == 外设观察存在自我归属状态_证据不足) {
            return 结果;
        }
...
export namespace 自我动作实现模块::外设模块 {
        ...
        const 语素入口节点类* 动作语义覆盖 = nullptr) noexcept
    {
        ...
        auto* 动态 = 世界树.创建方法动作动态(
            ...
            输入参数场景,
            输出结果场景,
            动作致变端点完整 ? 初始状态 : nullptr,
            动作致变端点完整 ? 结果状态 : nullptr);
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        const bool 被动识别可进入观察写入 =
            被动识别结果.已运行
            && 被动识别结果.材料可交付观察方法
            && 被动识别结果.同一性状态 != 外设观察存在自我存在同一性状态_冲突
            && 被动识别结果.同一性状态 != 外设观察存在自我存在同一性状态_证据不足
            && 被动识别结果.归属状态 != 外设观察存在自我归属状态_冲突
            && 被动识别结果.归属状态 != 外设观察存在自我归属状态_证据不足;
        auto* 动态 = 写入双目相机I64事实与动态(
