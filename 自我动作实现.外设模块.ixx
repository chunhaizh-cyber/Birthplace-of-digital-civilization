...
export namespace 自我动作实现模块::外设模块 {
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        const bool 成功 = 目标宿主 != nullptr
            && !当前场景存在集合.empty()
            && 关键特征变化观察状态 == 安全明确状态_已明确;
        const I64 构造状态 = 成功 ? 安全明确状态_已明确 : 安全明确状态_未明确;
        const I64 候选数量 = 成功 ? static_cast<I64>(当前场景存在集合.size()) : 0;
        const I64 低置信数量 = 候选数量;
        const I64 归因置信度 = 0;
        ...
        auto 写摘要 = [&](基础信息节点类* 宿主) noexcept {
            ...
            (void)写入基础节点I64(宿主, 特征_安全因果候选数量(), 候选数量, now);
            (void)写入基础节点I64(宿主, 特征_安全因果候选低置信数量(), 低置信数量, now);
            (void)写入基础节点I64(宿主, 特征_安全因果候选归因置信度(), 归因置信度, now);
        };
        写摘要(reinterpret_cast<基础信息节点类*>(环境.本轮运行虚拟存在));
        写摘要(reinterpret_cast<基础信息节点类*>(实际输出场景));
        写摘要(目标宿主);
        ...
        I64 主结果值) noexcept
    {
            ...
            now);
        {
            ...
            输出 << 方法名
                ...
                << " | 内部世界子候选验证状态=" << (上下文.子候选 ? 内部世界子候选验证状态_待验证 : 内部世界子候选验证状态_未验证)
                ...
                << " | 局部彩色图句柄=" << 上下文.彩色局部图句柄
                << " | 局部深度图句柄=" << 上下文.深度局部图句柄
                << " | 不确认内部世界子存在=" << (阶段 < 枚举_内部世界动作阶段::确认子存在 ? 1 : 0)
                << " | 不写基础风险明确=1"
                << " | 不结算安全值=1"
