        ...
        for (auto* 已知存在 : 已确认观察存在集) {
            ...
            for (const auto& 簇 : 报告.观察像素簇集合) {
                const I64 基础分数 = 分区簇与预测ROI匹配分数(
                    ...
                    项.预测最大Y,
                    已知Z);
                const I64 轮廓分数 = 外设簇与已确认观察存在VecI64轮廓匹配评分(簇, 已知存在);
...
export namespace 自我动作实现模块::外设模块 {
        ...
        时间戳 now) noexcept
    {
        ...
        ok = 声明条件特征(方法首节点, 特征_目标特征当前值(), false, now) && ok;
        if (auto* 结果根 = 方法规格根(方法首节点, 特征_结果表(), now)) {
            auto* 宿主 = reinterpret_cast<基础信息节点类*>(结果根);
        ...
        时间戳 now) noexcept
    {
        ...
        ok = 声明条件特征(方法首节点, 特征_外设观察存在稳定复现状态(), false, now) && ok;
        ok = 声明条件特征(方法首节点, 特征_外设观察存在观察完成状态(), false, now) && ok;
        ok = 声明条件特征(方法首节点, 特征_外设观察存在可复验状态(), false, now) && ok;
        if (auto* 结果根 = 方法规格根(方法首节点, 特征_结果表(), now)) {
            auto* 宿主 = reinterpret_cast<基础信息节点类*>(结果根);
        ...
        时间戳 now) noexcept
    {
        ...
        ok = 声明条件特征(方法首节点, 特征_外设观察存在自我归属状态(), false, now) && ok;
        ok = 声明条件特征(方法首节点, 特征_扫描可启动状态(), false, now) && ok;
        if (auto* 结果根 = 方法规格根(方法首节点, 特征_结果表(), now)) {
            auto* 宿主 = reinterpret_cast<基础信息节点类*>(结果根);
            ok = 取或创建子特征(宿主, 特征_当前观察存在()) && ok;
            ok = 取或创建子特征(宿主, 特征_历史观察存在()) && ok;
            ok = 取或创建子特征(宿主, 特征_外设观察存在自我归属状态()) && ok;
            ok = 取或创建子特征(宿主, 特征_扫描可启动状态()) && ok;
            ...
            ok = 取或创建子特征(宿主, 特征_扫描基准特征状态()) && ok;
            ok = 取或创建子特征(宿主, 特征_已归属存在当前特征_基准特征差异状态()) && ok;
            ok = 取或创建子特征(宿主, 特征_已归属存在当前特征_基准特征差异值()) && ok;
            ok = 取或创建子特征(宿主, 特征_轮廓变化量()) && ok;
            ok = 取或创建子特征(宿主, 特征_最近扫描时间()) && ok;
            ok = 取或创建子特征(宿主, 特征_扫描稳定计数()) && ok;
            ok = 取或创建子特征(宿主, 特征_下一轮扫描基准观察存在()) && ok;
            ok = 取或创建子特征(宿主, 特征_外设观察存在可复验状态()) && ok;
            ok = 取或创建子特征(宿主, 特征_补观察缺口状态()) && ok;
            ok = 取或创建子特征(宿主, 特征_补观察缺失原因()) && ok;
            ok = 取或创建子特征(宿主, 特征_补观察需求建议()) && ok;
        ...
        时间戳 now) noexcept
    {
        ...
        ok = 声明条件特征(方法首节点, 特征_目标存在(), false, now) && ok;
        ok = 声明条件特征(方法首节点, 特征_外设观察存在自我归属状态(), false, now) && ok;
        ok = 声明条件特征(方法首节点, 特征_目标观察窗口状态(), false, now) && ok;
        ok = 声明条件特征(方法首节点, 特征_跟踪基准特征状态(), false, now) && ok;
        if (auto* 结果根 = 方法规格根(方法首节点, 特征_结果表(), now)) {
            auto* 宿主 = reinterpret_cast<基础信息节点类*>(结果根);
            ok = 取或创建子特征(宿主, 特征_目标存在()) && ok;
            ok = 取或创建子特征(宿主, 特征_外设观察存在自我归属状态()) && ok;
        ...
        时间戳 now) noexcept
    {
        ...
        ok = 声明条件特征(方法首节点, 特征_外设观察存在候选确认状态(), false, now) && ok;
        ...
        时间戳 now) noexcept
    {
        ...
        ok = 声明形参规格(方法首节点, 特征_目标宿主(), 值类型_场景指针(), 值_来源_输入条件包(), false, now) && ok;
        ok = 声明形参规格(方法首节点, 特征_当前方法运行存在(), 值类型_场景指针(), 值_来源_输入条件包(), false, now) && ok;
        ok = 声明条件特征(方法首节点, 特征_观察存在发现状态(), false, now) && ok;
        ok = 声明条件特征(方法首节点, 特征_当前观察存在(), false, now) && ok;
        if (auto* 结果根 = 方法规格根(方法首节点, 特征_结果表(), now)) {
            auto* 宿主 = reinterpret_cast<基础信息节点类*>(结果根);
        ...
        时间戳 now) noexcept
    {
        ...
        ok = 声明形参规格(方法首节点, 特征_目标宿主(), 值类型_场景指针(), 值_来源_输入条件包(), false, now) && ok;
        ok = 声明形参规格(方法首节点, 特征_当前方法运行存在(), 值类型_场景指针(), 值_来源_输入条件包(), false, now) && ok;
        ok = 声明条件特征(方法首节点, 特征_已识别区域扫描入账状态(), false, now) && ok;
        ok = 声明条件特征(方法首节点, 特征_观察存在变化结果(), false, now) && ok;
        ok = 声明条件特征(方法首节点, 特征_变化状态(), false, now) && ok;
        if (auto* 结果根 = 方法规格根(方法首节点, 特征_结果表(), now)) {
            auto* 宿主 = reinterpret_cast<基础信息节点类*>(结果根);
        ...
        时间戳 now) noexcept
    {
        ...
        ok = 声明形参规格(方法首节点, 特征_目标宿主(), 值类型_场景指针(), 值_来源_输入条件包(), false, now) && ok;
        ok = 声明形参规格(方法首节点, 特征_当前方法运行存在(), 值类型_场景指针(), 值_来源_输入条件包(), false, now) && ok;
        ok = 声明条件特征(方法首节点, 特征_目标存在跟踪状态(), false, now) && ok;
        ok = 声明条件特征(方法首节点, 特征_目标存在(), false, now) && ok;
        if (auto* 结果根 = 方法规格根(方法首节点, 特征_结果表(), now)) {
            auto* 宿主 = reinterpret_cast<基础信息节点类*>(结果根);
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        if (观察存在可进入识别 && 报告可选.has_value() && 观察材料可进入识别) {
        ...
        if (!观察存在可进入识别) {
            结果.失败原因 = 值_执行缺口_当前方法条件不满足();
            结果.消息 = "识别条件不足:外设观察存在未稳定复现或不可复验";
            方案.确认状态 = 外设观察存在候选确认状态_证据不足;
        } else if (!报告可选.has_value()) {
            结果.失败原因 = 值_执行缺口_当前方法条件不满足();
        ...
        if (!成功) {
            if (!观察存在可进入识别) {
                const 语素入口节点类* 缺口特征 = 特征_外设观察存在稳定复现状态();
                I64 当前状态值 = 观察稳定复现状态;
                I64 目标状态值 = 外设观察存在稳定复现状态_稳定;
                if (观察稳定复现状态已读
                    && 观察稳定复现状态 == 外设观察存在稳定复现状态_稳定
                    && (!观察完成状态已读 || 观察完成状态 != 外设观察存在观察完成状态_已完成)) {
                    缺口特征 = 特征_外设观察存在观察完成状态();
                    当前状态值 = 观察完成状态;
                    目标状态值 = 外设观察存在观察完成状态_已完成;
                } else if (观察稳定复现状态已读
                    && 观察完成状态已读
                    && 观察稳定复现状态 == 外设观察存在稳定复现状态_稳定
                    && 观察完成状态 == 外设观察存在观察完成状态_已完成
                    && (!观察可复验状态已读 || 观察可复验状态 != 外设观察存在可复验状态_可复验)) {
                    缺口特征 = 特征_外设观察存在可复验状态();
                    当前状态值 = 观察可复验状态;
                    目标状态值 = 外设观察存在可复验状态_可复验;
                }
                写入宿主I64特征派生需求(
                    环境.方法首节点,
                    目标宿主,
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        auto* 实际输出场景 = 双目相机实际输出场景(环境.方法首节点, 输出结果场景, now);
        清理执行输出派生需求标记(实际输出场景, "自我_扫描已识别区域");
        auto* 相机存在 = 解析双目相机目标外设(输入参数场景, 实际输出场景);
        auto* 目标宿主 = 解析双目相机目标宿主(
            输入参数场景,
            reinterpret_cast<基础信息节点类*>(实际输出场景));
        auto* 当前存在 = 解析当前观察存在(输入参数场景, 目标宿主, 实际输出场景);
        auto* 历史存在 = 解析历史观察存在(输入参数场景, 目标宿主, 实际输出场景, 当前存在);
        auto* 当前存在节点 = reinterpret_cast<基础信息节点类*>(当前存在);
        const auto 扫描报告可选 = 读取最新外设观察报告(
            "双目相机/D455深度相机",
            枚举_外设观察报告类型::扫描变化报告);
        const auto 扫描材料质量 = 判定自我侧观察材料质量(扫描报告可选);
        const bool 扫描材料可复验 = 扫描材料质量.可进入验证方法;
        const bool 扫描材料可生成变化候选 = 扫描材料质量.可生成扫描变化候选;
        const I64 扫描报告ID = 扫描报告可选.has_value()
            ? static_cast<I64>(扫描报告可选->报告ID)
            : 0;
        ...
        const bool 有当前存在 = 当前存在 != nullptr;
        const bool 有历史存在 = 历史存在 != nullptr;
        ...
        const bool 扫描闸门成立 =
            ...
            && 扫描可启动状态已读
            && 扫描可启动状态 == 1;
        const bool 成功 = 有当前存在 && 归属闸门成立 && 扫描闸门成立 && 扫描材料可复验;
        const auto* 失败原因 = 成功 ? nullptr : 值_执行缺口_当前方法条件不满足();
        I64 发现状态 = 观察存在发现状态_证据不足;
        ...
        I64 最近扫描时间 = 成功 ? now : 0;
        I64 扫描稳定计数 = 0;
        bool 扫描稳定计数已生成 = false;
        存在节点类* 下一轮扫描基准 = 成功 ? 当前存在 : nullptr;

        if (成功 && 有历史存在 && 扫描材料可生成变化候选) {
            计算观察存在匹配关系(上下文, now);
            计算观察存在变化结果(上下文, now);
            计算观察变化事件(上下文, now);
            扫描入账状态 = 已知存在当前帧扫描状态_已生成;
            发现状态 = 观察存在发现状态_未发现;
            有实际变化 = 上下文.变化状态 != 变化状态_未变化;
            I64 既有稳定计数 = 0;
            bool 稳定计数已读 = 读取提交入口I64(
                输入参数场景,
                实际输出场景,
                当前存在节点,
                特征_扫描稳定计数(),
            ...
            扫描稳定计数已生成 = true;
            if (上下文.变化状态 == 变化状态_未变化) {
                const I64 安全既有稳定计数 = std::clamp<I64>(既有稳定计数, 0, 999999999);
                扫描稳定计数 = 安全既有稳定计数 + 1;
            }
            需要复验缺口 =
                上下文.历史对应状态 == 历史对应状态_冲突
                || 上下文.变化状态 == 变化状态_不确定;
        } else if (成功 && 有历史存在) {
            扫描入账状态 = 已知存在当前帧扫描状态_待重识别;
            发现状态 = 观察存在发现状态_证据不足;
            需要复验缺口 = true;
        } else if (成功) {
            I64 关系键 = 0;
            if (当前存在节点) {
                (void)读取基础节点I64(当前存在节点, 特征_外设观察像素簇ID(), 关系键);
                if (关系键 == 0) {
                    (void)读取基础节点I64(当前存在节点, 特征_来源空间候选编号(), 关系键);
                }
            }
            if (关系键 == 0) {
                关系键 = static_cast<I64>(std::hash<std::string>{}(当前存在->获取主键()) & 0x7fffffff);
            }
            发现事实 = 取或创建观察存在发现事实(上下文.宿主, 关系键);
            发现状态 = 观察存在发现状态_已发现;
            扫描入账状态 = 已知存在当前帧扫描状态_已生成;
        }

        const I64 当前特征状态 = 成功 ? 存在当前特征状态_已更新 : 存在当前特征状态_证据不足;
        const I64 扫描可复验实际状态 = 需要复验缺口
            ? 外设观察存在可复验状态_待复验
            : (扫描材料可复验 ? 外设观察存在可复验状态_可复验 : 外设观察存在可复验状态_证据不足);
        const I64 补观察缺口状态 = 需要复验缺口 ? 1 : 0;

        const auto* 动态特征 = 有历史存在
            ? 特征_已识别区域扫描入账状态()
            : 特征_观察存在发现状态();
        const I64 动态值 = 有历史存在 ? 扫描入账状态 : 发现状态;
        auto* 动态 = 写入双目相机I64事实与动态(
            环境.方法首节点,
            实际输出场景,
            上下文.宿主,
        ...
        if (成功 && 有历史存在 && 上下文.匹配关系) {
            const auto 轮廓二次特征 = 比较存在轮廓特征(
                当前存在,
                历史存在,
                reinterpret_cast<基础信息节点类*>(上下文.匹配关系),
                实际输出场景,
                ...
                "自我_扫描已识别区域");
            if (轮廓二次特征.比较特征数量 > 0) {
                上下文.轮廓比较特征数量 = 轮廓二次特征.比较特征数量;
                上下文.轮廓可比较特征数量 = 轮廓二次特征.可比较特征数量;
                上下文.轮廓比较二次特征写入数量 = 轮廓二次特征.二次特征写入数量;
                上下文.平面轮廓匹配评分 = 轮廓二次特征.平面轮廓匹配评分;
                上下文.空间极值轮廓匹配评分 = 轮廓二次特征.空间极值轮廓匹配评分;
            }
        }

        auto 写扫描结果 = [&](基础信息节点类* 宿主) noexcept {
            if (!宿主) return;
            (void)写入方法执行情况(宿主, 成功 ? 值_执行成功() : 值_执行失败(), now);
            if (当前存在) {
            ...
            if (历史存在) {
                (void)写入基础节点指针(宿主, 特征_历史观察存在(), 历史存在, now);
            }
            (void)写入基础节点I64(宿主, 特征_外设观察报告ID(), 扫描报告ID, now);
            (void)写入基础节点I64(宿主, 特征_外设观察存在自我归属状态(), 归属状态, now);
            (void)写入基础节点I64(宿主, 特征_扫描可启动状态(), 扫描可启动状态, now);
            if (发现事实) {
            ...
            (void)写入基础节点I64(宿主, 特征_已识别区域扫描入账状态(), 扫描入账状态, now);
            (void)写入基础节点I64(宿主, 特征_存在当前特征状态(), 当前特征状态, now);
            (void)写入基础节点I64(宿主, 特征_扫描基准特征状态(), 上下文.扫描基准特征状态, now);
            (void)写入基础节点I64(宿主, 特征_已归属存在当前特征_基准特征差异状态(), 上下文.基准差异状态, now);
            (void)写入基础节点I64(宿主, 特征_已归属存在当前特征_基准特征差异值(), 上下文.基准差异值, now);
            (void)写入基础节点I64(宿主, 特征_轮廓变化量(), 上下文.轮廓变化量, now);
            (void)写入基础节点I64(宿主, 特征_外设观察存在可复验状态(), 扫描可复验实际状态, now);
            (void)写入基础节点I64(宿主, 特征_补观察缺口状态(), 补观察缺口状态, now);
            if (最近扫描时间 > 0) {
                (void)写入基础节点I64(宿主, 特征_最近扫描时间(), 最近扫描时间, now);
            }
            if (扫描稳定计数已生成) {
                (void)写入基础节点I64(宿主, 特征_扫描稳定计数(), 扫描稳定计数, now);
            }
            if (下一轮扫描基准) {
                (void)写入基础节点指针(宿主, 特征_下一轮扫描基准观察存在(), 下一轮扫描基准, now);
            }
            if (需要复验缺口) {
                (void)写入基础节点I64(宿主, 特征_外设观察存在可复验状态(), 外设观察存在可复验状态_待复验, now);
                (void)写入基础节点I64(宿主, 特征_补观察缺口状态(), 1, now);
                (void)写入基础节点I64(宿主, 特征_补观察缺失原因(), 补观察缺失原因_边界不稳定, now);
                (void)写入基础节点I64(
                    宿主,
                    特征_补观察需求建议(),
                    补观察建议_重新取得观察特征帧 | 补观察建议_缩小ROI重新观察,
                    now);
            }
            if (动态) {
                (void)写入基础节点指针(宿主, 特征_动作动态(), 动态, now);
        ...
        写扫描结果(reinterpret_cast<基础信息节点类*>(环境.本轮运行虚拟存在));
        写扫描结果(reinterpret_cast<基础信息节点类*>(实际输出场景));
        写扫描结果(目标宿主);
        写扫描结果(reinterpret_cast<基础信息节点类*>(当前存在));
        写扫描结果(reinterpret_cast<基础信息节点类*>(发现事实));
        写扫描结果(reinterpret_cast<基础信息节点类*>(上下文.变化结果));
        (void)写入目标方法I64实际结果状态(
            环境.方法首节点,
            环境.方法首节点,
            特征_观察存在发现状态(),
            ...
            now);
        {
            ...
            输出 << "已识别区域扫描正式结果"
                ...
                << " | 历史存在=" << (历史存在 ? 历史存在->获取主键() : std::string{})
                << " | 归属状态=" << 归属状态
                << " | 归属状态已读=" << (归属状态已读 ? 1 : 0)
                << " | 扫描可启动状态=" << 扫描可启动状态
                << " | 扫描可启动状态已读=" << (扫描可启动状态已读 ? 1 : 0)
                << " | 扫描报告ID=" << 扫描报告ID
                << " | 扫描材料等级=" << 观察材料等级文本(扫描材料质量.等级)
                << " | 扫描材料可复验=" << (扫描材料可复验 ? 1 : 0)
                << " | 可生成扫描变化候选=" << (扫描材料可生成变化候选 ? 1 : 0)
                << " | 扫描材料质量依据=" << 扫描材料质量.依据
                << " | 扫描闸门成立=" << (成功 ? 1 : 0)
                << " | 发现状态=" << 发现状态
                << " | 扫描入账状态=" << 扫描入账状态
                << " | 扫描基准特征状态=" << 上下文.扫描基准特征状态
                << " | 基准差异状态=" << 上下文.基准差异状态
                << " | 基准差异值=" << 上下文.基准差异值
                << " | 轮廓变化量=" << 上下文.轮廓变化量
                << " | VecI64轮廓比较特征数=" << 上下文.轮廓比较特征数量
                << " | VecI64轮廓可比较数=" << 上下文.轮廓可比较特征数量
                << " | VecI64轮廓二次特征写入数=" << 上下文.轮廓比较二次特征写入数量
                << " | 平面轮廓匹配评分=" << 上下文.平面轮廓匹配评分
                << " | 空间极值轮廓匹配评分=" << 上下文.空间极值轮廓匹配评分
                << " | 最近扫描时间=" << 最近扫描时间
                << " | 扫描稳定计数=" << 扫描稳定计数
                << " | 扫描稳定计数已生成=" << (扫描稳定计数已生成 ? 1 : 0)
                << " | 下一轮扫描基准=" << (下一轮扫描基准 ? 下一轮扫描基准->获取主键() : std::string{})
                << " | 有实际变化=" << (有实际变化 ? 1 : 0)
                << " | 扫描冲突复验缺口=" << (需要复验缺口 ? 1 : 0)
                << " | 变化结果=" << (上下文.变化结果 ? 上下文.变化结果->获取主键() : std::string{})
                << " | 不组织外设提交包=1"
                << " | 首次扫描不写变化动态=" << (!有历史存在 && 有当前存在 ? 1 : 0)
                << " | 无变化不写变化动态=" << (有历史存在 && !有实际变化 ? 1 : 0)
                << " | 不写安全值=1"
                << " | 动作动态=" << 基础节点日志文本(reinterpret_cast<基础信息节点类*>(动态));
            项目运行日志(输出.str());
        }
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        const auto 报告可选 = 读取最新外设观察报告(
            "双目相机/D455深度相机",
            枚举_外设观察报告类型::逐簇识别报告);
        const auto 提交质量判定 = 判定自我侧观察材料质量(报告可选);
        const bool 提交来源可组织正式对象 = 提交质量判定.可进入提交入口组织;
        const auto 已知集合 = 读取当前场景已确认存在集合(
            目标宿主,
            输入参数场景,
            实际输出场景);
        const auto 方案 = 报告可选.has_value() && 提交来源可组织正式对象
        ...
        if (!成功) {
            ...
            结果.消息 = 提交来源可组织正式对象
                ...
                : "提交确认观察存在事实条件不足:观察材料质量门未允许提交入口组织正式对象";
        } else {
            结果.消息 = "提交确认观察存在事实完成";
        }
            ...
            "外设观察存在事实提交状态");
        {
            ...
            输出 << "提交_确认观察存在事实"
                ...
                << " | 提交主体类型=已确认观察存在"
                << " | 来源报告只作材料=1"
                << " | 观察材料等级=" << 观察材料等级文本(提交质量判定.等级)
                << " | 可进入提交入口组织=" << (提交来源可组织正式对象 ? 1 : 0)
                << " | 提交质量依据=" << 提交质量判定.依据
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        const bool 对应来源可组织正式对象 = 对应来源质量判定.可进入提交入口组织;
        const bool 缺正式外设报告 = 报告ID <= 0 || !对应来源报告可选.has_value();
        const bool 来源材料不可组织 = 缺正式外设报告 || !对应来源可组织正式对象;
        ...
        auto 写入稳定子集桥 = [&]() noexcept {
            ...
            if (!事实宿主 || !对应事实 || !映射表 || !报告 || !可写对应事实 || 扫描可启动状态 <= 0) {
                桥.首个失败说明 = "缺事实宿主、对应事实、映射表、来源报告或扫描可启动状态";
                return 桥;
            }
            const auto 已知集合 = 读取当前场景已确认存在集合(事实宿主, 输入参数场景, 实际输出场景);
            const auto 方案 = 构造外设观察存在确认方案(*报告, true, 已知集合);
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        const auto 发现提交质量判定 = 判定自我侧观察材料质量(发现来源报告可选);
        const bool 发现来源可复验 = 发现提交质量判定.可进入验证方法;
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        const auto 扫描提交质量判定 = 判定自我侧观察材料质量(扫描来源报告可选);
        const bool 扫描变化来源可提交 = 扫描提交质量判定.可生成扫描变化候选;
            ...
            now);
        {
            ...
            输出 << "观察存在特征值变化事实提交"
                ...
                << " | 提交主体类型=观察存在特征值变化事实"
                << " | 来源报告只作材料=1"
                << " | 观察材料等级=" << 观察材料等级文本(扫描提交质量判定.等级)
                << " | 可生成扫描变化候选=" << (扫描变化来源可提交 ? 1 : 0)
                << " | 提交质量依据=" << 扫描提交质量判定.依据
                << " | 不写需求满足=1"
                << " | 不写安全值=1"
                << " | 动作动态=" << 基础节点日志文本(reinterpret_cast<基础信息节点类*>(动态))
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        const auto 跟踪提交质量判定 = 判定自我侧观察材料质量(跟踪来源报告可选);
        const bool 跟踪来源可提交 = 跟踪提交质量判定.可生成跟踪动态候选;

        const bool 跟踪正式结果可读 =
            (上游目标跟踪状态已读 && 上游目标跟踪状态 == 目标存在跟踪状态_已更新)
            || (上游观察窗口状态已读 && 上游观察窗口状态 == 目标观察窗口状态_已更新);
        const bool 成功 = 目标存在 && 跟踪正式结果可读 && 跟踪来源可提交;
        const auto* 失败原因 = 成功 ? nullptr : 值_执行缺口_当前方法条件不满足();
        const I64 提交状态 = 成功
            ? 指定存在跟踪事实提交状态_已提交
            : 指定存在跟踪事实提交状态_条件不足;
            ...
            now);
        {
            ...
            输出 << "指定存在跟踪事实提交"
                ...
                << " | 提交主体类型=指定存在跟踪事实"
                << " | 来源报告只作材料=1"
                << " | 观察材料等级=" << 观察材料等级文本(跟踪提交质量判定.等级)
                << " | 可生成跟踪动态候选=" << (跟踪来源可提交 ? 1 : 0)
                << " | 提交质量依据=" << 跟踪提交质量判定.依据
                << " | 缺目标不回退当前观察存在=1"
                << " | 不写需求满足=1"
                << " | 不写安全值=1"
        ...
        auto 写分区项集合 = [&](基础信息节点类* 宿主) noexcept {
            ...
            for (const auto& 项 : 分区.项集合) {
                ...
                (void)写入基础节点I64(
                    节点,
                    特征_轮廓匹配分数(),
                    项.VecI64轮廓匹配分数 > 0 ? 项.VecI64轮廓匹配分数 : 项.匹配分数,
                    now);
