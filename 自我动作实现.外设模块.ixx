...
namespace {
    ...
    struct 结构_观察变化上下文 {
        ...
        I64 基准差异值 = 0;
        I64 目标特征值域状态 = 目标存在特征值域状态_未读取;
        I64 值域冲突状态 = 已归属存在特征值域冲突状态_证据不足;
    };
    ...
    inline void 计算观察存在变化结果(结构_观察变化上下文& 上下文, 时间戳 now) noexcept
    {
        ...
        if (上下文.历史对应状态 == 历史对应状态_已匹配 && 当前.有效 && 历史.有效) {
            ...
            上下文.基准差异值 = 最大3(
                上下文.位置变化量,
                上下文.范围变化量,
                std::max<I64>(上下文.尺寸变化量, 上下文.距离变化量));
            上下文.基准差异状态 = 已归属存在基准差异状态_已生成;
            上下文.目标特征值域状态 = 目标存在特征值域状态_值域内;
            上下文.值域冲突状态 = 已归属存在特征值域冲突状态_无冲突;
            ...
            上下文.基准差异值 = 最大3(
                ...
                上下文.尺寸差);
            上下文.目标特征值域状态 = 当前.有效 && 历史.有效
                ? 目标存在特征值域状态_冲突
                : 目标存在特征值域状态_证据不足;
            上下文.值域冲突状态 = 当前.有效 && 历史.有效
                ? 已归属存在特征值域冲突状态_有冲突
                : 已归属存在特征值域冲突状态_证据不足;
            上下文.变化状态 = 变化状态_不确定;
        }
        ...
        (void)写入基础节点I64(节点, 特征_目标存在特征值域状态(), 上下文.目标特征值域状态, now);
        (void)写入基础节点I64(节点, 特征_已归属存在特征值域冲突状态(), 上下文.值域冲突状态, now);
...
export namespace 自我动作实现模块::外设模块 {
        ...
        时间戳 now) noexcept
    {
        ...
        if (auto* 结果根 = 方法规格根(方法首节点, 特征_结果表(), now)) {
            ...
            ok = 取或创建子特征(宿主, 特征_目标存在特征值域状态()) && ok;
            ok = 取或创建子特征(宿主, 特征_已归属存在特征值域冲突状态()) && ok;
            ok = 取或创建子特征(宿主, 特征_已归属存在特征值域冲突动态()) && ok;
            ok = 取或创建子特征(宿主, 特征_外设观察存在可复验状态()) && ok;
            ok = 取或创建子特征(宿主, 特征_补观察缺口状态()) && ok;
            ok = 取或创建子特征(宿主, 特征_补观察缺失原因()) && ok;
            ok = 取或创建子特征(宿主, 特征_补观察需求建议()) && ok;
        ...
        ok = 确保方法结果能力I64(
            方法首节点,
            特征_已归属存在特征值域冲突状态(),
            已归属存在特征值域冲突状态_无冲突) && ok;
        ok = 确保方法结果能力I64(
            方法首节点,
            特征_已归属存在特征值域冲突状态(),
            已归属存在特征值域冲突状态_有冲突) && ok;
        ok = 确保方法结果能力I64(
            方法首节点,
            特征_外设观察存在可复验状态(),
            外设观察存在可复验状态_待复验) && ok;
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        const bool 观察稳定复现状态已读 = 读取观察摘要I64(
            目标宿主,
            输入参数场景,
            实际输出场景,
            特征_外设观察存在稳定复现状态(),
            观察稳定复现状态);
        const bool 观察完成状态已读 = 读取观察摘要I64(
            目标宿主,
            输入参数场景,
            实际输出场景,
            特征_外设观察存在观察完成状态(),
            观察完成状态);
        const bool 观察可复验状态已读 = 读取观察摘要I64(
            目标宿主,
            输入参数场景,
            实际输出场景,
            特征_外设观察存在可复验状态(),
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        const bool 扫描闸门成立 =
            ...
            && 扫描可启动状态已读
            && 扫描可启动状态 == 1;
        const bool 成功 = 有当前存在 && 归属闸门成立 && 扫描闸门成立;
        ...
        if (成功 && 有历史存在) {
            ...
            需要复验缺口 =
                上下文.值域冲突状态 == 已归属存在特征值域冲突状态_有冲突
                || 上下文.目标特征值域状态 == 目标存在特征值域状态_冲突;
        } else if (成功) {
            I64 关系键 = 0;
            if (当前存在节点) {
        ...
        auto 写扫描结果 = [&](基础信息节点类* 宿主) noexcept {
            ...
            (void)写入基础节点I64(宿主, 特征_目标存在特征值域状态(), 上下文.目标特征值域状态, now);
            (void)写入基础节点I64(宿主, 特征_已归属存在特征值域冲突状态(), 上下文.值域冲突状态, now);
            if (需要复验缺口) {
            ...
            if (动态) {
                ...
                if (有历史存在 && 有实际变化) {
                    ...
                    if (上下文.值域冲突状态 == 已归属存在特征值域冲突状态_有冲突) {
                        (void)写入基础节点指针(宿主, 特征_已归属存在特征值域冲突动态(), 动态, now);
                    }
                } else {
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        const bool 可写对应事实 = 识别正式结果可读 && 计数可读 && 提交确认链可读;
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        const bool 成功 = 当前存在 && 扫描正式结果可读 && 变化结果已生成 && 有实际变化;
        const auto* 失败原因 = 成功 ? nullptr : 值_执行缺口_当前方法条件不满足();
        const I64 提交状态 = 成功
            ? 观察存在特征值变化事实提交状态_已提交
