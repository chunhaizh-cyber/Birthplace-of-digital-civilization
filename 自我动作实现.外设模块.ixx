...
export namespace 自我动作实现模块::外设模块 {
    ...
    // 功能：执行当前观察特征帧采集和写账，供主动入口与被动动作入口复用。
        ...
        const char* 诊断标签) noexcept
    {
        ...
        if (动作语义覆盖 == 词_被动动作()) {
            项目运行日志(
                "观察中间层被动动作"
                " | 触发入口=" + std::string(诊断标签 ? 诊断标签 : "未标注")
                + " | 动作=双目相机_取得当前观察特征帧"
                " | 成功=" + std::to_string(成功 ? 1 : 0)
                + " | 动作动态=" + 基础节点日志文本(reinterpret_cast<基础信息节点类*>(动态))
                + " | 输出运行=" + 基础节点日志文本(reinterpret_cast<基础信息节点类*>(输出运行)));
        }
        更新方法运行账(环境.方法首节点, 输出运行, 成功, now);
    ...
    // 功能：由主动观察动作内部触发中间层被动动作。
    inline 存在节点类* 被动动作_取得当前观察特征帧(
        方法类::节点类* 方法树锚点,
        场景节点类* 输入参数场景,
        场景节点类* 输出结果场景) noexcept
    {
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        auto* 目标宿主 = 解析双目相机目标宿主(
            ...
            reinterpret_cast<基础信息节点类*>(实际输出场景));
        结构_双目相机调用结果 结果{};
        std::vector<动态节点类*> 被动中间层动态{};
        if (!读取已落账空间候选集合(目标宿主, 输入参数场景, 实际输出场景, 结果)) {
            const auto 初次缺口消息 = 结果.消息;
            auto* 被动运行 = 被动动作_取得当前观察特征帧(
                环境.方法首节点,
                输入参数场景,
                实际输出场景);
            if (auto* 被动动态 = 读取运行结果动作动态(被动运行)) {
                被动中间层动态.push_back(被动动态);
            }
            结果 = {};
            (void)读取已落账空间候选集合(目标宿主, 输入参数场景, 实际输出场景, 结果);
            项目运行日志(
                ...
                " | 初次缺口=" + 初次缺口消息
                + " | 被动动态数量=" + std::to_string(被动中间层动态.size())
                + " | 重读成功=" + std::to_string(结果.成功 ? 1 : 0)
                + " | 重读消息=" + 结果.消息);
        }
        const auto 判断 = 计算空间候选独立性判断(结果);
        const auto 队列 = 生成空间候选观察队列(结果);
        ...
        auto* 动态 = 写入双目相机I64事实与动态(
            ...
            now);
        if (动态) {
            for (auto* 被动动态 : 被动中间层动态) {
                if (!被动动态 || 被动动态 == 动态) continue;
                (void)世界树.追加来源低层动态(动态, 被动动态);
            }
            if (!被动中间层动态.empty() && 被动中间层动态.front()) {
                (void)写入基础节点指针(
                    reinterpret_cast<基础信息节点类*>(动态),
                    特征_来源动作动态(),
                    被动中间层动态.front(),
                    now);
            }
