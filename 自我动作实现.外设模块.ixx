...
namespace {
        ...
        I64 评估值候选) noexcept
    {
        ...
        if (层级 <= 0 || !目标安全因果信息 || 证据方向 == 安全因果证据方向_未定义) {
    ...
    inline const 语素入口节点类* 特征_外设提交包ID() noexcept
    {
        static const 语素入口节点类* s_词 = 语素集.添加信息入口词("外设提交包ID", 枚举_信息入口类型::特征模板入口);
        return s_词;
    }

    inline const 语素入口节点类* 特征_外设提交包类型() noexcept
    {
        static const 语素入口节点类* s_词 = 语素集.添加信息入口词("外设提交包类型", 枚举_信息入口类型::特征模板入口);
        return s_词;
    }

    inline const 语素入口节点类* 特征_外设提交包组织状态() noexcept
    {
        static const 语素入口节点类* s_词 = 语素集.添加信息入口词("外设提交包组织状态", 枚举_信息入口类型::特征模板入口);
        return s_词;
    }

    inline const 语素入口节点类* 特征_外设提交包材料可回查状态() noexcept
    {
        static const 语素入口节点类* s_词 = 语素集.添加信息入口词("外设提交包材料可回查状态", 枚举_信息入口类型::特征模板入口);
        return s_词;
    }

    inline const 语素入口节点类* 特征_外设提交包过期状态() noexcept
    {
        static const 语素入口节点类* s_词 = 语素集.添加信息入口词("外设提交包过期状态", 枚举_信息入口类型::特征模板入口);
        return s_词;
    }

    inline const 语素入口节点类* 特征_外设提交包generation明确状态() noexcept
    {
        static const 语素入口节点类* s_词 = 语素集.添加信息入口词("外设提交包generation明确状态", 枚举_信息入口类型::特征模板入口);
        return s_词;
    }

    inline const 语素入口节点类* 特征_外设提交包可比较状态() noexcept
    {
        static const 语素入口节点类* s_词 = 语素集.添加信息入口词("外设提交包可比较状态", 枚举_信息入口类型::特征模板入口);
        return s_词;
    }

    inline const 语素入口节点类* 特征_外设提交包缺失材料数量() noexcept
    {
        static const 语素入口节点类* s_词 = 语素集.添加信息入口词("外设提交包缺失材料数量", 枚举_信息入口类型::特征模板入口);
        return s_词;
    }

    inline const 语素入口节点类* 特征_外设提交包质量问题数量() noexcept
    {
        static const 语素入口节点类* s_词 = 语素集.添加信息入口词("外设提交包质量问题数量", 枚举_信息入口类型::特征模板入口);
        return s_词;
    }

    inline const 语素入口节点类* 特征_外设提交包材料句柄数量() noexcept
    {
        static const 语素入口节点类* s_词 = 语素集.添加信息入口词("外设提交包材料句柄数量", 枚举_信息入口类型::特征模板入口);
        return s_词;
    }

    inline const 语素入口节点类* 特征_外设提交包特征值条目数量() noexcept
    {
        static const 语素入口节点类* s_词 = 语素集.添加信息入口词("外设提交包特征值条目数量", 枚举_信息入口类型::特征模板入口);
        return s_词;
    }

    inline const 语素入口节点类* 特征_外设提交包候选事实数量() noexcept
    {
        static const 语素入口节点类* s_词 = 语素集.添加信息入口词("外设提交包候选事实数量", 枚举_信息入口类型::特征模板入口);
        return s_词;
    }

    inline const 语素入口节点类* 特征_外设识别结果场景状态() noexcept
    {
        static const 语素入口节点类* s_词 = 语素集.添加信息入口词("外设识别结果场景状态", 枚举_信息入口类型::特征模板入口);
        return s_词;
    }
    ...
    inline const 语素入口节点类* 特征_外设跟踪结果场景状态() noexcept
    {
        static const 语素入口节点类* s_词 = 语素集.添加信息入口词("外设跟踪结果场景状态", 枚举_信息入口类型::特征模板入口);
        return s_词;
    }
    ...
    inline const 语素入口节点类* 特征_观察存在发现动态() noexcept
    {
        static const 语素入口节点类* s_词 = 语素集.添加信息入口词("观察存在发现动态", 枚举_信息入口类型::特征模板入口);
        return s_词;
    }
    ...
    inline const 语素入口节点类* 类型_外设提交包组织结果() noexcept
    {
        static const 语素入口节点类* s_词 = 语素集.添加信息入口词("外设提交包组织结果", 枚举_信息入口类型::存在概念入口);
        return s_词;
    }
...
export namespace 自我动作实现模块::外设模块 {
        ...
        时间戳 now) noexcept
    {
        ...
        ok = 声明条件特征(方法首节点, 特征_目标安全因果信息(), now) && ok;
        if (auto* 结果根 = 方法规格根(方法首节点, 特征_结果表(), now)) {
            auto* 宿主 = reinterpret_cast<基础信息节点类*>(结果根);
        ...
        时间戳 now) noexcept
    {
        ...
        ok = 声明形参规格(方法首节点, 特征_目标宿主(), 值类型_场景指针(), 值_来源_输入条件包(), false, now) && ok;
        ok = 声明形参规格(方法首节点, 特征_当前方法运行存在(), 值类型_场景指针(), 值_来源_输入条件包(), false, now) && ok;
        ok = 声明条件特征(方法首节点, 特征_自我边界参照状态(), false, now) && ok;
        ok = 声明条件特征(方法首节点, 特征_自我_场景绝对坐标明确状态(), false, now) && ok;
        ok = 声明条件特征(方法首节点, 特征_存在_场景绝对坐标明确状态(), false, now) && ok;
        if (auto* 结果根 = 方法规格根(方法首节点, 特征_结果表(), now)) {
            auto* 宿主 = reinterpret_cast<基础信息节点类*>(结果根);
    ...
    inline bool 确认外设提交包组织方法规格(
        方法类::节点类* 方法首节点,
        存在节点类* 方法存在,
        const 语素入口节点类* 主结果特征,
        时间戳 now) noexcept
    {
        if (!方法首节点 || !方法存在 || !主结果特征) return false;
        bool ok = true;
        ok = 声明形参规格(方法首节点, 特征_输入条件包(), 值类型_场景指针(), 值_来源_输入条件包(), false, now) && ok;
        ok = 声明形参规格(方法首节点, 特征_目标外设(), 值类型_场景指针(), 值_来源_输入条件包(), false, now) && ok;
        ok = 声明形参规格(方法首节点, 特征_目标宿主(), 值类型_场景指针(), 值_来源_输入条件包(), false, now) && ok;
        ok = 声明形参规格(方法首节点, 特征_当前方法运行存在(), 值类型_场景指针(), 值_来源_输入条件包(), false, now) && ok;
        ok = 声明条件特征(方法首节点, 特征_外设观察报告ID(), false, now) && ok;
        ok = 声明条件特征(方法首节点, 特征_外设观察材料可回查状态(), false, now) && ok;
        if (auto* 结果根 = 方法规格根(方法首节点, 特征_结果表(), now)) {
            auto* 宿主 = reinterpret_cast<基础信息节点类*>(结果根);
            ok = 取或创建子特征(宿主, 特征_外设观察报告ID()) && ok;
            ok = 取或创建子特征(宿主, 特征_外设提交包ID()) && ok;
            ok = 取或创建子特征(宿主, 特征_外设提交包类型()) && ok;
            ok = 取或创建子特征(宿主, 特征_外设提交包组织状态()) && ok;
            ok = 取或创建子特征(宿主, 特征_外设提交包材料可回查状态()) && ok;
            ok = 取或创建子特征(宿主, 特征_外设提交包过期状态()) && ok;
            ok = 取或创建子特征(宿主, 特征_外设提交包generation明确状态()) && ok;
            ok = 取或创建子特征(宿主, 特征_外设提交包可比较状态()) && ok;
            ok = 取或创建子特征(宿主, 特征_外设提交包缺失材料数量()) && ok;
            ok = 取或创建子特征(宿主, 特征_外设提交包质量问题数量()) && ok;
            ok = 取或创建子特征(宿主, 特征_外设提交包材料句柄数量()) && ok;
            ok = 取或创建子特征(宿主, 特征_外设提交包特征值条目数量()) && ok;
            ok = 取或创建子特征(宿主, 特征_外设提交包候选事实数量()) && ok;
            ok = 取或创建子特征(宿主, 主结果特征) && ok;
            ok = 取或创建子特征(宿主, 特征_动作动态()) && ok;
            ok = 取或创建子特征(宿主, 特征_失败原因()) && ok;
        }
        ok = 确保方法结果能力I64(
        ...
        时间戳 now) noexcept
    {
        ...
        ok = 声明形参规格(方法首节点, 特征_目标宿主(), 值类型_场景指针(), 值_来源_输入条件包(), false, now) && ok;
        ok = 声明形参规格(方法首节点, 特征_当前方法运行存在(), 值类型_场景指针(), 值_来源_输入条件包(), false, now) && ok;
        ok = 声明条件特征(方法首节点, 特征_外设识别结果场景状态(), false, now) && ok;
        ok = 声明条件特征(方法首节点, 特征_外设观察存在事实提交状态(), false, now) && ok;
        if (auto* 结果根 = 方法规格根(方法首节点, 特征_结果表(), now)) {
            auto* 宿主 = reinterpret_cast<基础信息节点类*>(结果根);
        ...
        时间戳 now) noexcept
    {
        ...
        ok = 声明形参规格(方法首节点, 特征_当前方法运行存在(), 值类型_场景指针(), 值_来源_输入条件包(), false, now) && ok;
        ok = 声明条件特征(方法首节点, 特征_外设扫描结果场景状态(), false, now) && ok;
        ok = 声明条件特征(方法首节点, 特征_观察存在变化结果(), false, now) && ok;
        if (auto* 结果根 = 方法规格根(方法首节点, 特征_结果表(), now)) {
            auto* 宿主 = reinterpret_cast<基础信息节点类*>(结果根);
        ...
        时间戳 now) noexcept
    {
        ...
        ok = 声明形参规格(方法首节点, 特征_目标宿主(), 值类型_场景指针(), 值_来源_输入条件包(), false, now) && ok;
        ok = 声明形参规格(方法首节点, 特征_当前方法运行存在(), 值类型_场景指针(), 值_来源_输入条件包(), false, now) && ok;
        ok = 声明条件特征(方法首节点, 特征_外设跟踪结果场景状态(), false, now) && ok;
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        const I64 风险安全层因素数量 = 1;
        const I64 风险安全层缺失因素数量 = 0;
        const I64 风险安全层候选 = 风险安全_场景影响部分候选;
        ...
        const I64 风险因素已入账掩码 =
            ...
            ? I64{1}
            : 0;
        const I64 风险因素证据搜索未完成数量 = 风险因素未搜索掩码;
        const I64 风险因素默认满足数量 = 风险因素默认满足掩码;
        const I64 风险安全层可聚合状态 =
            风险因素未搜索掩码 == 0
            && 风险因素搜索缺口掩码 == 0
            && (风险因素已入账掩码 + 风险因素默认满足掩码 + 风险因素负向证据掩码)
                >= 风险安全层因素数量
            ? 安全明确状态_已明确
            : 安全明确状态_未明确;
        const I64 风险安全层明确状态 =
        ...
        auto 写安全评估结果 = [&](基础信息节点类* 宿主) noexcept {
            ...
            (void)写入基础节点I64(宿主, 特征_风险安全层因素数量(), 风险安全层因素数量, now);
    ...
    struct 结构_外设提交包组织摘要 {
        I64 报告ID = 0;
        I64 包ID = 0;
        I64 包类型 = static_cast<I64>(枚举_外设提交包类型::未指定);
        I64 组织状态 = 外设提交包组织状态_未组织;
        I64 材料可回查状态 = 0;
        I64 过期状态 = 外设提交包过期状态_未知;
        I64 generation明确状态 = 0;
        I64 可比较状态 = 外设提交包可比较状态_不可比较;
        I64 缺失材料数量 = 0;
        I64 质量问题数量 = 0;
        I64 材料句柄数量 = 0;
        I64 特征值条目数量 = 0;
        I64 候选事实数量 = 0;
        I64 主结果状态 = 外设提交包组织状态_未组织;
        const 语素入口节点类* 失败原因 = nullptr;
        std::string 消息{};
    };

    inline I64 外设组织_转I64(std::uint64_t 值) noexcept
    {
    ...
    inline I64 外设组织_计数转I64(std::size_t 值) noexcept
    {
        return 值 > static_cast<std::size_t>((std::numeric_limits<I64>::max)())
            ? (std::numeric_limits<I64>::max)()
            : static_cast<I64>(值);
    }

    inline I64 外设组织_当前时间毫秒() noexcept
    {
        const auto 现在 = std::chrono::steady_clock::now().time_since_epoch();
        return static_cast<I64>(
            std::chrono::duration_cast<std::chrono::milliseconds>(现在).count());
    }

    inline std::optional<结构_外设观察报告队列项> 读取外设组织输入报告(
        场景节点类* 输入参数场景,
        枚举_外设观察报告类型 期望类型) noexcept
    {
        I64 输入报告ID = 0;
        if (读取输入参数I64(输入参数场景, 特征_外设观察报告ID(), 输入报告ID)
            && 输入报告ID > 0) {
            return 读取外设观察报告_按ID(static_cast<std::uint64_t>(输入报告ID));
        }
        return 读取最新外设观察报告("双目相机/D455深度相机", 期望类型);
    }

    inline bool 外设组织_材料全部可回查(
        const std::vector<std::string>& 材料句柄集) noexcept
    {
        if (材料句柄集.empty()) {
            return false;
        }
        for (const auto& 句柄 : 材料句柄集) {
            if (句柄.empty() || !解析D455观察材料句柄(句柄).成功) {
                return false;
            }
        }
        return true;
    ...
    inline I64 外设组织_可比较状态(
        const std::vector<结构_外设特征值条目>& 条目集) noexcept
    {
        for (const auto& 条目 : 条目集) {
            if (条目.可比较) {
                return 外设提交包可比较状态_可比较;
            }
        }
        return 外设提交包可比较状态_不可比较;
    }

    inline void 填充外设提交包基础摘要(
        结构_外设提交包组织摘要& 摘要,
        const 结构_外设提交包头& 包头,
        I64 特征值条目数量,
        I64 候选事实数量,
        ...
        bool 要求可比较) noexcept
    {
        摘要.报告ID = 外设组织_转I64(包头.报告ID);
        摘要.包ID = 外设组织_转I64(包头.包ID);
        摘要.包类型 = static_cast<I64>(包头.包类型);
        摘要.材料句柄数量 = 外设组织_计数转I64(包头.材料句柄集.size());
        摘要.缺失材料数量 = 外设组织_计数转I64(包头.缺失材料.size());
        摘要.质量问题数量 = 外设组织_计数转I64(包头.质量问题.size());
        摘要.generation明确状态 = 包头.generation.empty() ? 0 : 1;
        摘要.材料可回查状态 = 外设组织_材料全部可回查(包头.材料句柄集) ? 1 : 0;
        摘要.特征值条目数量 = 特征值条目数量;
        摘要.候选事实数量 = 候选事实数量;
        摘要.可比较状态 = 可比较状态;

        if (包头.有效截止时间毫秒 <= 0) {
            摘要.过期状态 = 外设提交包过期状态_未知;
        } else {
            摘要.过期状态 = 外设组织_当前时间毫秒() <= static_cast<I64>(包头.有效截止时间毫秒)
                ? 外设提交包过期状态_未过期
                : 外设提交包过期状态_已过期;
        }

        if (摘要.过期状态 == 外设提交包过期状态_已过期) {
            摘要.组织状态 = 外设提交包组织状态_已过期;
            摘要.失败原因 = 值_执行缺口_当前方法条件不满足();
            摘要.消息 = "外设提交包组织失败:提交包已过期";
        } else if (摘要.材料可回查状态 == 0
            || 摘要.generation明确状态 == 0
            || (要求可比较 && 摘要.可比较状态 == 外设提交包可比较状态_不可比较)
            || (特征值条目数量 <= 0 && 候选事实数量 <= 0)) {
            摘要.组织状态 = 外设提交包组织状态_证据不足;
            摘要.失败原因 = 值_执行缺口_当前方法条件不满足();
            摘要.消息 = "外设提交包组织证据不足:材料/generation/特征值条目不足";
        } else {
            摘要.组织状态 = 外设提交包组织状态_已组织;
            摘要.消息 = "外设提交包已组织为自我可用候选事实视图";
        }
        摘要.主结果状态 = 摘要.组织状态;
    }

    inline 结构_外设提交包组织摘要 组织外设识别提交包摘要(
        场景节点类* 输入参数场景) noexcept
    {
        结构_外设提交包组织摘要 摘要{};
        auto 报告可选 = 读取外设组织输入报告(
            输入参数场景,
            枚举_外设观察报告类型::逐簇识别报告);
        if (!报告可选.has_value()) {
            摘要.组织状态 = 外设提交包组织状态_证据不足;
            摘要.主结果状态 = 摘要.组织状态;
            摘要.失败原因 = 值_执行缺口_当前方法条件不满足();
            摘要.消息 = "外设识别提交包组织失败:缺少D455逐簇识别报告";
            return 摘要;
        }
        auto 包可选 = 构造外设识别提交包_由观察报告(*报告可选);
        if (!包可选.has_value()) {
            摘要.报告ID = 外设组织_转I64(报告可选->报告ID);
            摘要.组织状态 = 外设提交包组织状态_类型不匹配;
            摘要.主结果状态 = 摘要.组织状态;
            摘要.失败原因 = 值_执行缺口_当前方法条件不满足();
            摘要.消息 = "外设识别提交包组织失败:报告类型不是逐簇识别报告";
            return 摘要;
        }
        const auto& 包 = *包可选;
        const I64 候选事实数量 = 外设组织_计数转I64(
            包.新存在候选.size() + 包.已有存在匹配候选.size() + 包.未识别区域.size() + 包.冲突区域.size());
        填充外设提交包基础摘要(
            摘要,
            包.包头,
            外设组织_计数转I64(包.候选特征值.size()),
            候选事实数量,
            外设提交包可比较状态_不可比较,
            false);
        return 摘要;
    ...
    inline 结构_外设提交包组织摘要 组织外设扫描提交包摘要(
        场景节点类* 输入参数场景) noexcept
    {
        结构_外设提交包组织摘要 摘要{};
        auto 报告可选 = 读取外设组织输入报告(
        ...
        if (!报告可选.has_value()) {
            ...
            摘要.主结果状态 = 摘要.组织状态;
            摘要.失败原因 = 值_执行缺口_当前方法条件不满足();
            摘要.消息 = "外设扫描提交包组织失败:缺少D455扫描变化报告";
            return 摘要;
        }
        auto 包可选 = 构造外设扫描提交包_由观察报告(*报告可选);
        if (!包可选.has_value()) {
            摘要.报告ID = 外设组织_转I64(报告可选->报告ID);
            摘要.组织状态 = 外设提交包组织状态_类型不匹配;
            摘要.主结果状态 = 摘要.组织状态;
            摘要.失败原因 = 值_执行缺口_当前方法条件不满足();
            摘要.消息 = "外设扫描提交包组织失败:报告类型不是扫描变化报告";
            return 摘要;
        }
        const auto& 包 = *包可选;
        const I64 特征值条目数量 = 外设组织_计数转I64(
            包.当前特征值.size() + 包.特征变化值.size());
        const I64 候选事实数量 = 外设组织_计数转I64(
            包.已知存在结果.size() + 包.未解释变化区域.size() + 包.需重识别区域.size() + 包.冲突区域.size());
        const I64 可比较状态 = 外设组织_可比较状态(包.特征变化值);
        填充外设提交包基础摘要(
            摘要,
            包.包头,
            特征值条目数量,
    ...
    inline 结构_外设提交包组织摘要 组织外设跟踪提交包摘要(
        场景节点类* 输入参数场景) noexcept
    {
        结构_外设提交包组织摘要 摘要{};
        auto 报告可选 = 读取外设组织输入报告(
        ...
        if (!报告可选.has_value()) {
            ...
            摘要.主结果状态 = 摘要.组织状态;
            摘要.失败原因 = 值_执行缺口_当前方法条件不满足();
            摘要.消息 = "外设跟踪提交包组织失败:缺少D455跟踪报告";
            return 摘要;
        }
        auto 包可选 = 构造外设跟踪提交包_由观察报告(*报告可选);
        if (!包可选.has_value()) {
            摘要.报告ID = 外设组织_转I64(报告可选->报告ID);
            摘要.组织状态 = 外设提交包组织状态_类型不匹配;
            摘要.主结果状态 = 摘要.组织状态;
            摘要.失败原因 = 值_执行缺口_当前方法条件不满足();
            摘要.消息 = "外设跟踪提交包组织失败:报告类型不是跟踪报告";
            return 摘要;
        }
        const auto& 包 = *包可选;
        const I64 特征值条目数量 = 外设组织_计数转I64(
            包.目标特征值.size() + 包.目标特征变化值.size());
        const I64 候选事实数量 = 包.目标种子ID > 0 || 包.目标存在ID > 0 ? 1 : 0;
        const I64 可比较状态 = 外设组织_可比较状态(包.目标特征变化值);
        填充外设提交包基础摘要(
            摘要,
            包.包头,
            特征值条目数量,
    ...
    inline void 写入外设提交包组织摘要(
        基础信息节点类* 宿主,
        const 结构_外设提交包组织摘要& 摘要,
        const 语素入口节点类* 主结果特征,
        bool 成功,
        存在节点类* 相机存在,
        动态节点类* 动态,
    ...
    {
        if (!宿主 || !主结果特征) return;
        写入双目相机通用结果(宿主, 成功, 相机存在, 宿主, 动态, 摘要.失败原因, now);
        (void)写入基础节点I64(宿主, 特征_外设观察报告ID(), 摘要.报告ID, now);
        (void)写入基础节点I64(宿主, 特征_外设提交包ID(), 摘要.包ID, now);
        (void)写入基础节点I64(宿主, 特征_外设提交包类型(), 摘要.包类型, now);
        (void)写入基础节点I64(宿主, 特征_外设提交包组织状态(), 摘要.组织状态, now);
        (void)写入基础节点I64(宿主, 特征_外设提交包材料可回查状态(), 摘要.材料可回查状态, now);
        (void)写入基础节点I64(宿主, 特征_外设提交包过期状态(), 摘要.过期状态, now);
        (void)写入基础节点I64(宿主, 特征_外设提交包generation明确状态(), 摘要.generation明确状态, now);
        (void)写入基础节点I64(宿主, 特征_外设提交包可比较状态(), 摘要.可比较状态, now);
        (void)写入基础节点I64(宿主, 特征_外设提交包缺失材料数量(), 摘要.缺失材料数量, now);
        (void)写入基础节点I64(宿主, 特征_外设提交包质量问题数量(), 摘要.质量问题数量, now);
        (void)写入基础节点I64(宿主, 特征_外设提交包材料句柄数量(), 摘要.材料句柄数量, now);
        (void)写入基础节点I64(宿主, 特征_外设提交包特征值条目数量(), 摘要.特征值条目数量, now);
        (void)写入基础节点I64(宿主, 特征_外设提交包候选事实数量(), 摘要.候选事实数量, now);
        (void)写入基础节点I64(宿主, 主结果特征, 摘要.主结果状态, now);
    }

    inline 存在节点类* 执行外设提交包组织方法(
        方法类::节点类* 方法首节点,
        场景节点类* 输入参数场景,
        场景节点类* 输出结果场景,
        枚举_本能方法ID 方法ID,
        ...
        const 语素入口节点类* 主结果特征,
        const char* 主结果特征名,
        结构_外设提交包组织摘要 (*组织摘要函数)(场景节点类*) noexcept) noexcept
    {
        const auto now = 结构体_时间戳::当前_微秒();
        auto 环境 = 确保本能方法自身环境(
            方法首节点,
        ...
        auto* 实际输出场景 = 双目相机实际输出场景(环境.方法首节点, 输出结果场景, now);
        清理执行输出派生需求标记(实际输出场景, 方法名);
        auto* 相机存在 = 解析双目相机目标外设(输入参数场景, 实际输出场景);
        auto* 结果宿主 = reinterpret_cast<基础信息节点类*>(实际输出场景);
        auto 摘要 = 组织摘要函数 ? 组织摘要函数(输入参数场景) : 结构_外设提交包组织摘要{};
        if (!组织摘要函数) {
            摘要.组织状态 = 外设提交包组织状态_证据不足;
            摘要.主结果状态 = 摘要.组织状态;
            摘要.失败原因 = 值_执行缺口_当前方法条件不满足();
            摘要.消息 = "外设提交包组织失败:缺少组织函数";
        }
        const bool 成功 = 摘要.组织状态 == 外设提交包组织状态_已组织;
        ...
        auto* 结果存在 = 世界树.取或创建子存在_按类型并按特征I64(
            ...
            类型_外设提交包组织结果(),
            特征_外设提交包ID(),
            摘要.包ID > 0 ? 摘要.包ID : static_cast<I64>(方法ID));
        写入外设提交包组织摘要(
            reinterpret_cast<基础信息节点类*>(环境.本轮运行虚拟存在),
            摘要,
            主结果特征,
            ...
            主结果特征名);
        {
            ...
            输出 << "外设提交包组织结果"
                ...
                << " | 材料可回查=" << 摘要.材料可回查状态
                << " | 过期状态=" << 摘要.过期状态
                << " | generation明确=" << 摘要.generation明确状态
                << " | 可比较=" << 摘要.可比较状态
                << " | 缺失材料数量=" << 摘要.缺失材料数量
                << " | 质量问题数量=" << 摘要.质量问题数量
                << " | 材料句柄数量=" << 摘要.材料句柄数量
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        const bool 识别已组织 = 识别结果场景状态 == 外设提交包组织状态_已组织;
        const bool 提交确认链可读 =
            观察存在事实提交状态 == 观察存在事实提交状态_已提交
            || 观察存在事实提交状态 == 观察存在事实提交状态_部分提交
            || 已验证数量已读;
        const bool 计数可读 = 候选事实数量已读 && 候选事实数量 >= 0;
        const bool 可写对应事实 = 识别已组织 && 计数可读 && 提交确认链可读;

        const I64 有效可观测单位数量 = 计数可读 ? std::max<I64>(0, 候选事实数量) : 0;
        const I64 已稳定对应单位数量 = 已验证数量已读 ? std::max<I64>(0, 已验证观察存在数量) : 0;
        const I64 多重冲突数量 = 冲突数量已读 ? std::max<I64>(0, 冲突数量) : 0;
        const I64 未稳定对应单位数量 = std::max<I64>(
            0,
            有效可观测单位数量 - 已稳定对应单位数量 - 多重冲突数量);
        const I64 稳定对应存在数量 = 已稳定对应单位数量;
        const I64 计数事实取得状态 = 可写对应事实 ? 1 : 0;
        const I64 稳定对应事实取得状态 = 可写对应事实 && 稳定对应存在数量 > 0 ? 1 : 0;
        const I64 稳定数量明确状态 = 可写对应事实 ? 1 : 0;
        const I64 对应闭合状态 =
            可写对应事实
            && 有效可观测单位数量 > 0
            && 已稳定对应单位数量 == 有效可观测单位数量
            ...
            && 稳定对应存在数量 > 0
                ? 1
                : 0;
        const I64 扫描可启动状态 = 可写对应事实 && 稳定对应存在数量 >= 1 ? 1 : 0;
        const I64 缺口可解释状态 = 可写对应事实 || (计数可读 && (冲突数量已读 || 证据不足数量已读)) ? 1 : 0;
        I64 映射缺口原因掩码 = 0;
        if (!识别已组织) 映射缺口原因掩码 |= 1;
        if (!计数可读) 映射缺口原因掩码 |= 2;
        ...
        const I64 提交状态 = 可写对应事实
            ? 可观测单位存在对应事实提交状态_已提交
            : 可观测单位存在对应事实提交状态_条件不足;
        const bool 成功 = 可写对应事实;
        const auto* 失败原因 = 成功 ? nullptr : 值_执行缺口_当前方法条件不满足();

        auto* 对应事实 = 取或创建当前观察范围对应事实(事实宿主, 报告ID);
        auto* 映射表宿主 = 对应事实
            ? reinterpret_cast<基础信息节点类*>(对应事实)
            : 事实宿主;
        auto* 映射表 = 取或创建可观测单位映射表(映射表宿主, 报告ID);
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        const bool 扫描已组织 = 扫描结果场景状态 == 外设提交包组织状态_已组织;
        const bool 成功 = 当前存在 && 扫描已组织;
        const auto* 失败原因 = 成功 ? nullptr : 值_执行缺口_当前方法条件不满足();
        const I64 提交状态 = 成功
            ? 观察存在发现事实提交状态_已提交
            : 观察存在发现事实提交状态_条件不足;
        ...
        auto 写发现事实摘要 = [&](基础信息节点类* 宿主) noexcept {
            ...
            if (动态) {
                (void)写入基础节点指针(宿主, 特征_观察存在发现动态(), 动态, now);
                (void)写入基础节点指针(宿主, 特征_动作动态(), 动态, now);
            }
            if (失败原因) {
                (void)写入基础节点指针(宿主, 特征_失败原因(), 失败原因, now);
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        const bool 成功 = 当前存在 && 扫描已组织 && 变化结果已生成;
        const auto* 失败原因 = 成功 ? nullptr : 值_执行缺口_当前方法条件不满足();
        const I64 提交状态 = 成功
            ? 观察存在特征值变化事实提交状态_已提交
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        if (!目标存在) {
            目标存在 = 解析当前观察存在(输入参数场景, 目标宿主, 实际输出场景);
        }
        ...
        (void)读取提交入口I64(
            ...
            特征_外设跟踪结果场景状态(),
            跟踪结果场景状态);
        const bool 跟踪已组织 = 跟踪结果场景状态 == 外设提交包组织状态_已组织;
        const bool 成功 = 目标存在 && 跟踪已组织;
        const auto* 失败原因 = 成功 ? nullptr : 值_执行缺口_当前方法条件不满足();
        const I64 提交状态 = 成功
            ? 指定存在跟踪事实提交状态_已提交
            : 指定存在跟踪事实提交状态_条件不足;
