...
namespace {
    ...
    inline constexpr I64 安全评估刻度上限 = 10000;
...
export namespace 自我动作实现模块::外设模块 {
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        if (安全因果候选构造状态 != 安全明确状态_已明确) 不足原因 |= 安全评估不足原因_因果证据不足;
        if (不足原因 == 0) 不足原因 = 安全评估不足原因_因果证据不足;

        const bool 成功 = false;
        const I64 基础风险判断状态 = 基础观察可用
            ? 基础风险判断状态_证据不足
            : 基础风险判断状态_条件不足;
        const I64 明确状态 = 成功 ? 安全明确状态_已明确 : 安全明确状态_未明确;
        const I64 存在未知伤害可能性候选 = 安全评估刻度上限 / 2;
        const I64 直接伤害可能性候选 = 当前场景存在集合.empty() ? 0 : 存在未知伤害可能性候选;
        const I64 安全度候选 = 当前场景存在集合.empty()
            ? 0
            : 安全评估刻度上限 - 直接伤害可能性候选;
        const I64 评估置信度 = 0;
        const I64 未判断因子数量 =
        ...
        const I64 证据完整度 = 0;
        const I64 不确定度 = 安全评估刻度上限;
        auto* 最低安全度存在 = 当前场景存在集合.empty()
            ? static_cast<存在节点类*>(nullptr)
            : 当前场景存在集合.front();
        ...
        auto 写单个存在安全评估结果 = [&](基础信息节点类* 结果宿主) noexcept {
            ...
            for (auto* 存在 : 当前场景存在集合) {
                ...
                (void)写入基础节点指针(节点, 特征_被评估存在(), 存在, now);
                (void)写入基础节点I64(节点, 特征_存在直接伤害可能性候选(), 存在未知伤害可能性候选, now);
                (void)写入基础节点I64(节点, 特征_存在安全度候选(), 安全评估刻度上限 - 存在未知伤害可能性候选, now);
                (void)写入基础节点I64(节点, 特征_历史直接伤害归因次数(), 0, now);
                (void)写入基础节点I64(节点, 特征_最近直接伤害归因时间(), 0, now);
                (void)写入基础节点I64(节点, 特征_无伤害共处时长(), 0, now);
                (void)写入基础节点I64(节点, 特征_伤害可能性下界候选(), 0, now);
                (void)写入基础节点I64(节点, 特征_存在安全评估置信度(), 0, now);
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        if (!读取基础节点I64(目标宿主, 特征_外设观察像素簇候选已验证状态(), 上游验证状态)) {
            (void)读取基础节点I64(
                reinterpret_cast<基础信息节点类*>(实际输出场景),
                特征_外设观察像素簇候选已验证状态(),
                上游验证状态);
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        for (const auto& 项 : 方案.项集合) {
            ...
            原始事实写入状态 = 1;
            场景关联状态 = 1;
            已提交项集合.push_back({ 观察存在, 方案.报告ID, 项.来源候选簇ID });
        }
