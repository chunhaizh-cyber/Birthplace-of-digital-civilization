...
export namespace 自我动作实现模块::外设模块 {
        ...
        时间戳 now) noexcept
    {
        ...
        ok = 确保方法结果能力I64(方法首节点, 特征_观察内部治理记录状态(), 1) && ok;
        return ok;
    }
        ...
        场景节点类* 输出结果场景) noexcept
    {
        ...
        const bool 被动识别可形成观察结果 =
            ...
            && (被动识别结果.唯一已有存在
                ...
                || (被动识别结果.同一性状态 == 外设观察存在自我存在同一性状态_未命中已知存在
                    && 被动识别结果.归属状态 == 外设观察存在自我归属状态_待建立));
        const bool 成功 = 结果.成功 || 被动识别可形成观察结果;
        const I64 观察假设状态 = 假设.假设状态 > 0
            ? 假设.假设状态
            : (被动识别可形成观察结果 ? 观察存在假设状态_已生成 : 0);
        auto* 动态 = 写入双目相机I64事实与动态(
            环境.方法首节点,
            实际输出场景,
            目标宿主,
        ...
        if (成功 && !记录结果.成功) {
            记录结果.成功 = true;
            记录结果.失败原因 = 0;
            记录结果.消息 = "观察被动识别归属结果已写入";
        }
        auto 写结果 = [&](基础信息节点类* 宿主) noexcept {
            写入双目相机通用结果(宿主, 成功, 相机存在, 目标宿主, 动态, 记录结果.失败原因, now);
            写入双目相机观察存在假设摘要(宿主, 记录结果, 判断, 队列, 假设, now);
