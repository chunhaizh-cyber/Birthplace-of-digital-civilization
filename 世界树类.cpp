...
std::vector<结构_动态特征影响统计> 世界树类::评估动态特征影响(
    const std::vector<结构_动态特征影响样本>& 样本集合,
    std::uint64_t 最小样本数,
    std::uint64_t 最大反例数) const
{
    return 动态服务_.评估动态特征影响(样本集合, 最小样本数, 最大反例数);
}
...
{
    Vector3D 写入前坐标{};
    const bool 写入前坐标可读 = 读取存在场景绝对坐标(节点, 写入前坐标);
    ...
    if (auto* 所属场景 = 私有_取存在所在场景(基础信息链_, 节点)) {
        Vector3D 写入后坐标{};
        if (读取存在场景绝对坐标(节点, 写入后坐标)) {
            auto* 宿主 = reinterpret_cast<基础信息节点类*>(节点);
            auto 写入坐标轴状态 = [this, 所属场景, 宿主, 写入前坐标可读, now](
                const 语素入口节点类* 特征类型,
                double 写入前值,
                double 写入后值) {
                if (!宿主 || !特征类型) {
                ...
                const auto 前值 = static_cast<I64>(写入前值);
                const auto 后值 = static_cast<I64>(写入后值);
                if (写入前坐标可读 && 前值 == 后值) {
                    return;
                }
                auto* 状态特征 = 取或创建子特征_按类型(宿主, 特征类型);
                if (!状态特征) {
                    return;
                }
                (void)创建内部状态(
            ...
            写入坐标轴状态(私有_特征_存在场景绝对坐标X(), 写入前坐标.x, 写入后坐标.x);
            写入坐标轴状态(私有_特征_存在场景绝对坐标Y(), 写入前坐标.y, 写入后坐标.y);
            写入坐标轴状态(私有_特征_存在场景绝对坐标Z(), 写入前坐标.z, 写入后坐标.z);
        }
