    ...
    const 结构_世界树场景轮廓查找参数& 参数) const
{
    ...
    结构_场景三维体素轮廓查找参数 场景参数{};
    场景参数.三维体素特征类型 = 参数.三维体素特征类型;
    场景参数.查询层级 = 参数.查询层级;
    场景参数.最低相似度Q10000 = 参数.最低相似度Q10000;
    场景参数.默认存在最小体素边长_mm = 参数.默认存在最小体素边长_mm;
    场景参数.最大返回数量 = 参数.最大返回数量;

    const auto 场景结果集合 = 场景服务_.查找存在_按深度或彩图轮廓图(
        场景,
        局部视角集合,
        局部融合参数,
        场景参数,
        值池_,
        存在概念根);
    结果集合.reserve(场景结果集合.size());
    for (const auto& 场景结果 : 场景结果集合) {
        结构_世界树场景轮廓存在比较结果 项{};
        项.存在 = 场景结果.存在;
        项.三维体素特征 = 场景结果.三维体素特征;
        项.相似度Q10000 = 场景结果.相似度Q10000;
        项.比较结果 = 场景结果.比较结果;
