...
namespace {
    ...
    std::string 私有_世界树SQL建表脚本()
    {
        ...
        SQL << "SET NOCOUNT ON;\n"
            ...
            << "    ordinal_index int NOT NULL\n"
            << ");\n"
            << "IF COL_LENGTH(N'[鱼巢].[世界树节点关系]', N'owner_row_index') IS NULL\n"
            << "    ALTER TABLE [鱼巢].[世界树节点关系] ADD owner_row_index int NOT NULL CONSTRAINT DF_world_tree_node_relation_owner_row_index DEFAULT(0);\n"
            << "IF COL_LENGTH(N'[鱼巢].[世界树节点关系]', N'target_row_index') IS NULL\n"
            << "    ALTER TABLE [鱼巢].[世界树节点关系] ADD target_row_index int NULL;\n"
            << "IF COL_LENGTH(N'[鱼巢].[世界树根树关系]', N'owner_row_index') IS NULL\n"
            << "    ALTER TABLE [鱼巢].[世界树根树关系] ADD owner_row_index int NOT NULL CONSTRAINT DF_world_tree_root_tree_relation_owner_row_index DEFAULT(0);\n"
            << "IF COL_LENGTH(N'[鱼巢].[世界树根树关系]', N'target_row_index') IS NULL\n"
            << "    ALTER TABLE [鱼巢].[世界树根树关系] ADD target_row_index int NULL;\n"
            << "IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'IX_world_tree_node_key' AND object_id = OBJECT_ID(N'[鱼巢].[世界树节点]'))\n"
            << "    CREATE INDEX IX_world_tree_node_key ON [鱼巢].[世界树节点](snapshot_id, node_key, parent_key);\n"
            << "IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'IX_world_tree_node_parent' AND object_id = OBJECT_ID(N'[鱼巢].[世界树节点]'))\n"
            << "    CREATE INDEX IX_world_tree_node_parent ON [鱼巢].[世界树节点](snapshot_id, parent_key, node_key);\n"
            << "IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'IX_world_tree_node_path' AND object_id = OBJECT_ID(N'[鱼巢].[世界树节点]'))\n"
            << "    CREATE INDEX IX_world_tree_node_path ON [鱼巢].[世界树节点](snapshot_id, path_text);\n"
            << "IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'IX_world_tree_relation_owner' AND object_id = OBJECT_ID(N'[鱼巢].[世界树节点关系]'))\n"
            << "    CREATE INDEX IX_world_tree_relation_owner ON [鱼巢].[世界树节点关系](snapshot_id, owner_key, target_key);\n"
            ...
            << "    CREATE INDEX IX_world_tree_relation_owner_row ON [鱼巢].[世界树节点关系](snapshot_id, owner_row_index, target_row_index);\n"
            << "IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'IX_world_tree_root_tree_key' AND object_id = OBJECT_ID(N'[鱼巢].[世界树根树]'))\n"
            << "    CREATE INDEX IX_world_tree_root_tree_key ON [鱼巢].[世界树根树](snapshot_id, root_key);\n"
            ...
            << "    CREATE INDEX IX_world_tree_root_tree_node_key ON [鱼巢].[世界树根树节点](snapshot_id, root_key, node_key, parent_key);\n"
            ...
            << "    CREATE INDEX IX_world_tree_root_tree_node_row ON [鱼巢].[世界树根树节点](snapshot_id, root_key, row_index);\n"
            ...
            << "    CREATE INDEX IX_world_tree_root_tree_node_main_info_key ON [鱼巢].[世界树根树节点主信息](snapshot_id, root_key, node_key);\n"
            ...
            << "    CREATE INDEX IX_world_tree_root_tree_node_main_info_row ON [鱼巢].[世界树根树节点主信息](snapshot_id, root_key, node_row_index);\n"
            ...
            << "    CREATE INDEX IX_world_tree_root_tree_relation_owner ON [鱼巢].[世界树根树关系](snapshot_id, root_key, owner_key, target_key);\n"
            ...
            << "    CREATE INDEX IX_world_tree_root_tree_relation_owner_row ON [鱼巢].[世界树根树关系](snapshot_id, root_key, owner_row_index, target_row_index);\n";
        return SQL.str();
    }
    ...
    std::string 私有_世界树SQL视图脚本()
    {
        ...
        SQL << "EXEC(N'CREATE OR ALTER VIEW [鱼巢].[当前世界树节点] AS\n"
            << "SELECT n.*\n"
            << "FROM [鱼巢].[世界树节点] n\n"
            << "WHERE n.snapshot_id = (SELECT TOP (1) snapshot_id FROM [鱼巢].[世界树快照] ORDER BY captured_at DESC);');\n"
            << "EXEC(N'CREATE OR ALTER VIEW [鱼巢].[当前世界树关系] AS\n"
            << "SELECT r.*\n"
            << "FROM [鱼巢].[世界树节点关系] r\n"
            << "WHERE r.snapshot_id = (SELECT TOP (1) snapshot_id FROM [鱼巢].[世界树快照] ORDER BY captured_at DESC);');\n"
            ...
            << "    t.root_key,\n"
            << "    t.root_row_index,\n"
            << "    m.node_kind AS root_kind,\n"
            << "    m.display_text AS root_display_text,\n"
            << "    t.root_depth,\n"
            << "    t.root_sibling_index,\n"
            << "    t.root_direct_child_count,\n"
            << "    t.root_path_text,\n"
            << "    t.subtree_node_count,\n"
            << "    t.subtree_relation_count\n"
            << "FROM [鱼巢].[世界树根树] t\n"
            << "LEFT JOIN [鱼巢].[世界树根树节点主信息] m\n"
            << "    ON m.snapshot_id = t.snapshot_id\n"
            << "    AND m.root_key = t.root_key\n"
            << "    AND m.node_row_index = t.root_row_index\n"
            << "WHERE t.snapshot_id = (SELECT TOP (1) snapshot_id FROM [鱼巢].[世界树快照] ORDER BY captured_at DESC);');\n"
            << "EXEC(N'CREATE OR ALTER VIEW [鱼巢].[当前世界树根树节点主信息] AS\n"
            << "SELECT m.*\n"
            << "FROM [鱼巢].[世界树根树节点主信息] m\n"
            << "WHERE m.snapshot_id = (SELECT TOP (1) snapshot_id FROM [鱼巢].[世界树快照] ORDER BY captured_at DESC);');\n"
            << "EXEC(N'CREATE OR ALTER VIEW [鱼巢].[当前世界树根树节点] AS\n"
            << "SELECT\n"
            << "    n.id,\n"
            << "    n.snapshot_id,\n"
            << "    n.root_key,\n"
            << "    n.root_relative_depth,\n"
            << "    n.row_index,\n"
            << "    n.node_key,\n"
            << "    n.parent_key,\n"
            << "    n.depth,\n"
        ...
        const std::string& 来源原因)
    {
        ...
        SQL << "SET NOCOUNT ON;\n"
            << "SET XACT_ABORT ON;\n"
            << "BEGIN TRANSACTION;\n"
            << "IF COL_LENGTH(N'[鱼巢].[世界树根树]', N'root_kind') IS NOT NULL\n"
            << "    ALTER TABLE [鱼巢].[世界树根树] DROP COLUMN root_kind;\n"
            << "IF COL_LENGTH(N'[鱼巢].[世界树根树]', N'root_display_text') IS NOT NULL\n"
            << "    ALTER TABLE [鱼巢].[世界树根树] DROP COLUMN root_display_text;\n"
            << "IF COL_LENGTH(N'[鱼巢].[世界树根树节点]', N'node_kind') IS NOT NULL\n"
            << "    ALTER TABLE [鱼巢].[世界树根树节点] DROP COLUMN node_kind;\n"
            << "IF COL_LENGTH(N'[鱼巢].[世界树根树节点]', N'display_text') IS NOT NULL\n"
            << "    ALTER TABLE [鱼巢].[世界树根树节点] DROP COLUMN display_text;\n"
            << "IF COL_LENGTH(N'[鱼巢].[世界树根树节点]', N'main_type_value') IS NOT NULL\n"
            << "    ALTER TABLE [鱼巢].[世界树根树节点] DROP COLUMN main_type_value;\n"
            << "IF COL_LENGTH(N'[鱼巢].[世界树根树节点]', N'main_type_text') IS NOT NULL\n"
            << "    ALTER TABLE [鱼巢].[世界树根树节点] DROP COLUMN main_type_text;\n"
            << "IF COL_LENGTH(N'[鱼巢].[世界树根树节点]', N'name_key') IS NOT NULL\n"
            << "    ALTER TABLE [鱼巢].[世界树根树节点] DROP COLUMN name_key;\n"
            << "IF COL_LENGTH(N'[鱼巢].[世界树根树节点]', N'name_text') IS NOT NULL\n"
            << "    ALTER TABLE [鱼巢].[世界树根树节点] DROP COLUMN name_text;\n"
            << "IF COL_LENGTH(N'[鱼巢].[世界树根树节点]', N'type_key') IS NOT NULL\n"
            << "    ALTER TABLE [鱼巢].[世界树根树节点] DROP COLUMN type_key;\n"
            << "IF COL_LENGTH(N'[鱼巢].[世界树根树节点]', N'type_text') IS NOT NULL\n"
            << "    ALTER TABLE [鱼巢].[世界树根树节点] DROP COLUMN type_text;\n"
            << "IF COL_LENGTH(N'[鱼巢].[世界树根树节点]', N'value_kind') IS NOT NULL\n"
            << "    ALTER TABLE [鱼巢].[世界树根树节点] DROP COLUMN value_kind;\n"
            << "IF COL_LENGTH(N'[鱼巢].[世界树根树节点]', N'value_text') IS NOT NULL\n"
            << "    ALTER TABLE [鱼巢].[世界树根树节点] DROP COLUMN value_text;\n"
            << "IF COL_LENGTH(N'[鱼巢].[世界树根树节点]', N'auxiliary_text') IS NOT NULL\n"
            << "    ALTER TABLE [鱼巢].[世界树根树节点] DROP COLUMN auxiliary_text;\n"
            ...
            << "INSERT INTO [鱼巢].[世界树快照] (snapshot_id, captured_at, source_kind, source_reason, node_count, relation_count)\n"
        ...
        for (std::size_t 起始 = 0; 起始 < 节点集.size(); 起始 += 插入批量大小) {
            ...
            SQL << "INSERT INTO [鱼巢].[世界树节点] (snapshot_id, row_index, node_key, parent_key, depth, sibling_index, direct_child_count, path_text, node_kind, display_text, main_type_value, main_type_text, name_key, name_text, type_key, type_text, value_kind, value_text, auxiliary_text) VALUES\n";
        ...
        for (std::size_t 起始 = 0; 起始 < 关系集.size(); 起始 += 插入批量大小) {
            ...
            SQL << "INSERT INTO [鱼巢].[世界树节点关系] (snapshot_id, row_index, owner_row_index, target_row_index, owner_key, relation_name, target_kind, target_key, target_text, ordinal_index) VALUES\n";
            for (std::size_t 索引 = 起始; 索引 < 结束; ++索引) {
                const auto& 行 = 关系集[索引];
                SQL << "(@snapshot_id, "
                    << 行.行号 << ", "
        ...
        SQL << "WITH root_nodes AS (\n"
            ...
            << ")\n"
            << "INSERT INTO [鱼巢].[世界树根树节点] (snapshot_id, root_key, root_relative_depth, row_index, node_key, parent_key, depth, sibling_index, direct_child_count, path_text)\n"
            << "SELECT n.snapshot_id, r.root_key, n.depth - r.root_depth, n.row_index, n.node_key, n.parent_key, n.depth, n.sibling_index, n.direct_child_count, n.path_text\n"
            << "FROM root_nodes r\n"
            << "JOIN [鱼巢].[世界树节点] n ON n.snapshot_id = r.snapshot_id\n"
            ...
            << "    WHERE n.snapshot_id = @snapshot_id AND n.parent_key = N'WORLD_ROOT'\n"
            << ")\n"
            << "INSERT INTO [鱼巢].[世界树根树节点主信息] (snapshot_id, root_key, node_row_index, node_key, node_kind, display_text, main_type_value, main_type_text, name_key, name_text, type_key, type_text, value_kind, value_text, auxiliary_text)\n"
            << "SELECT n.snapshot_id, r.root_key, n.row_index, n.node_key, n.node_kind, n.display_text, n.main_type_value, n.main_type_text, n.name_key, n.name_text, n.type_key, n.type_text, n.value_kind, n.value_text, n.auxiliary_text\n"
            << "FROM root_nodes r\n"
            << "JOIN [鱼巢].[世界树节点] n ON n.snapshot_id = r.snapshot_id\n"
            << "    AND (n.path_text = r.root_path_text OR n.path_text LIKE r.root_path_text + N'/%');\n"
            << "INSERT INTO [鱼巢].[世界树根树关系] (snapshot_id, root_key, row_index, owner_row_index, target_row_index, owner_key, relation_name, target_kind, target_key, target_text, ordinal_index)\n"
            ...
            << "FROM [鱼巢].[世界树节点关系] r\n"
            << "JOIN [鱼巢].[世界树根树节点] n ON n.snapshot_id = r.snapshot_id AND n.row_index = r.owner_row_index\n"
            ...
            << "    GROUP BY snapshot_id, root_key\n"
            << ")\n"
            << "INSERT INTO [鱼巢].[世界树根树] (snapshot_id, root_key, root_row_index, root_depth, root_sibling_index, root_direct_child_count, root_path_text, subtree_node_count, subtree_relation_count)\n"
            << "SELECT root.snapshot_id, root.node_key, root.row_index, root.depth, root.sibling_index, root.direct_child_count, root.path_text,\n"
            ...
            << "        ON node.snapshot_id = info.snapshot_id\n"
            << "        AND node.root_key = info.root_key\n"
            << "        AND node.row_index = info.node_row_index\n"
            << "    WHERE info.snapshot_id = @snapshot_id AND node.row_index IS NULL\n"
            << ")\n"
            << "    THROW 51031, N'world_tree_root_tree_node_main_info node_row_index invalid', 1;\n";
...
存在节点类* 世界树类::取或创建子存在_按名称类型(
    ...
    const 语素入口节点类* 类型)
{
    return 存在服务_.取或创建子存在_按名称类型(父节点, 名称, 类型);
}
...
场景节点类* 世界树类::创建场景_按名称(
    ...
    枚举_世界类型 世界类型)
{
    return 场景服务_.创建场景_按名称(父节点, 名称, 世界类型);
}
...
状态节点类* 世界树类::创建状态(
    ...
    时间戳 收到时间)
{
    return 状态服务_.创建状态(场景, 主体, 特征, 状态值, 发生时间, 收到时间);
}
...
{
    return 状态服务_.创建内部状态(场景, 主体, 特征, 值, 是否变化, now, 后处理);
}
...
{
    return 状态服务_.绑定主体特征(节点, 主体, 特征);
}

// 功能：写入状态节点的状态值和可选时间戳。
bool 世界树类::写入状态值(状态节点类* 节点, const 特征值& 值, 时间戳 发生时间, 时间戳 收到时间)
{
    return 状态服务_.写入状态值(节点, 值, 发生时间, 收到时间);
}
