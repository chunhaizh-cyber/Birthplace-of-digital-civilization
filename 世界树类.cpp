...
namespace {
    ...
    std::string 私有_世界树SQL建表脚本()
    {
        ...
        SQL << "SET NOCOUNT ON;\n"
            ...
            << "    path_text nvarchar(1000) NULL\n"
            << ");\n"
            << "IF OBJECT_ID(N'fishnest.world_tree_root_tree_node_main_info', N'U') IS NULL\n"
            << "CREATE TABLE fishnest.world_tree_root_tree_node_main_info (\n"
            << "    id bigint IDENTITY(1,1) NOT NULL PRIMARY KEY,\n"
            << "    snapshot_id uniqueidentifier NOT NULL,\n"
            << "    root_key nvarchar(80) NOT NULL,\n"
            ...
            << "IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'IX_world_tree_root_tree_node_row' AND object_id = OBJECT_ID(N'fishnest.world_tree_root_tree_node'))\n"
            << "    CREATE INDEX IX_world_tree_root_tree_node_row ON fishnest.world_tree_root_tree_node(snapshot_id, root_key, row_index);\n"
            << "IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'IX_world_tree_root_tree_node_main_info_key' AND object_id = OBJECT_ID(N'fishnest.world_tree_root_tree_node_main_info'))\n"
            << "    CREATE INDEX IX_world_tree_root_tree_node_main_info_key ON fishnest.world_tree_root_tree_node_main_info(snapshot_id, root_key, node_key);\n"
            << "IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'IX_world_tree_root_tree_node_main_info_row' AND object_id = OBJECT_ID(N'fishnest.world_tree_root_tree_node_main_info'))\n"
            << "    CREATE INDEX IX_world_tree_root_tree_node_main_info_row ON fishnest.world_tree_root_tree_node_main_info(snapshot_id, root_key, node_row_index);\n"
    ...
    std::string 私有_世界树SQL视图脚本()
    {
        ...
        SQL << "EXEC(N'CREATE OR ALTER VIEW fishnest.v_current_world_tree_nodes AS\n"
            ...
            << "LEFT JOIN fishnest.world_tree_root_tree_node_main_info m\n"
            ...
            << "EXEC(N'CREATE OR ALTER VIEW fishnest.v_current_world_tree_root_tree_node_main_info AS\n"
            << "SELECT m.*\n"
            << "FROM fishnest.world_tree_root_tree_node_main_info m\n"
            << "WHERE m.snapshot_id = (SELECT TOP (1) snapshot_id FROM fishnest.world_tree_snapshot ORDER BY captured_at DESC);');\n"
        ...
        const std::string& 来源原因)
    {
        ...
        SQL << "SET NOCOUNT ON;\n"
            << "SET XACT_ABORT ON;\n"
            << "BEGIN TRANSACTION;\n"
            << "IF COL_LENGTH(N'fishnest.world_tree_root_tree', N'root_kind') IS NOT NULL\n"
            << "    ALTER TABLE fishnest.world_tree_root_tree DROP COLUMN root_kind;\n"
            << "IF COL_LENGTH(N'fishnest.world_tree_root_tree', N'root_display_text') IS NOT NULL\n"
            << "    ALTER TABLE fishnest.world_tree_root_tree DROP COLUMN root_display_text;\n"
            << "IF COL_LENGTH(N'fishnest.world_tree_root_tree_node', N'node_kind') IS NOT NULL\n"
            << "    ALTER TABLE fishnest.world_tree_root_tree_node DROP COLUMN node_kind;\n"
            << "IF COL_LENGTH(N'fishnest.world_tree_root_tree_node', N'display_text') IS NOT NULL\n"
            << "    ALTER TABLE fishnest.world_tree_root_tree_node DROP COLUMN display_text;\n"
            << "IF COL_LENGTH(N'fishnest.world_tree_root_tree_node', N'main_type_value') IS NOT NULL\n"
            << "    ALTER TABLE fishnest.world_tree_root_tree_node DROP COLUMN main_type_value;\n"
            << "IF COL_LENGTH(N'fishnest.world_tree_root_tree_node', N'main_type_text') IS NOT NULL\n"
            << "    ALTER TABLE fishnest.world_tree_root_tree_node DROP COLUMN main_type_text;\n"
            << "IF COL_LENGTH(N'fishnest.world_tree_root_tree_node', N'name_key') IS NOT NULL\n"
            << "    ALTER TABLE fishnest.world_tree_root_tree_node DROP COLUMN name_key;\n"
            << "IF COL_LENGTH(N'fishnest.world_tree_root_tree_node', N'name_text') IS NOT NULL\n"
            << "    ALTER TABLE fishnest.world_tree_root_tree_node DROP COLUMN name_text;\n"
            << "IF COL_LENGTH(N'fishnest.world_tree_root_tree_node', N'type_key') IS NOT NULL\n"
            << "    ALTER TABLE fishnest.world_tree_root_tree_node DROP COLUMN type_key;\n"
            << "IF COL_LENGTH(N'fishnest.world_tree_root_tree_node', N'type_text') IS NOT NULL\n"
            << "    ALTER TABLE fishnest.world_tree_root_tree_node DROP COLUMN type_text;\n"
            << "IF COL_LENGTH(N'fishnest.world_tree_root_tree_node', N'value_kind') IS NOT NULL\n"
            << "    ALTER TABLE fishnest.world_tree_root_tree_node DROP COLUMN value_kind;\n"
            << "IF COL_LENGTH(N'fishnest.world_tree_root_tree_node', N'value_text') IS NOT NULL\n"
            << "    ALTER TABLE fishnest.world_tree_root_tree_node DROP COLUMN value_text;\n"
            << "IF COL_LENGTH(N'fishnest.world_tree_root_tree_node', N'auxiliary_text') IS NOT NULL\n"
            << "    ALTER TABLE fishnest.world_tree_root_tree_node DROP COLUMN auxiliary_text;\n"
            << "DELETE FROM fishnest.world_tree_root_tree_relation;\n"
            << "DELETE FROM fishnest.world_tree_root_tree_node_main_info;\n"
        ...
        SQL << "WITH root_nodes AS (\n"
            ...
            << ")\n"
            << "INSERT INTO fishnest.world_tree_root_tree_node (snapshot_id, root_key, root_relative_depth, row_index, node_key, parent_key, depth, sibling_index, direct_child_count, path_text)\n"
            ...
            << "    WHERE n.snapshot_id = @snapshot_id AND n.parent_key = N'WORLD_ROOT'\n"
            << ")\n"
            << "INSERT INTO fishnest.world_tree_root_tree_node_main_info (snapshot_id, root_key, node_row_index, node_key, node_kind, display_text, main_type_value, main_type_text, name_key, name_text, type_key, type_text, value_kind, value_text, auxiliary_text)\n"
            ...
            << "    GROUP BY snapshot_id, root_key\n"
            << ")\n"
            << "INSERT INTO fishnest.world_tree_root_tree (snapshot_id, root_key, root_row_index, root_depth, root_sibling_index, root_direct_child_count, root_path_text, subtree_node_count, subtree_relation_count)\n"
            ...
            << "IF EXISTS (\n"
            << "    SELECT 1\n"
            << "    FROM fishnest.world_tree_root_tree_node_main_info info\n"
            << "    LEFT JOIN fishnest.world_tree_root_tree_node node\n"
...
std::vector<结构_世界树场景轮廓存在比较结果> 世界树类::查找场景存在_按深度或彩图轮廓图(
    const 场景节点类* 场景,
    const std::vector<结构_三维体素轮廓图视角>& 局部视角集合,
    const 结构_三维体素轮廓融合参数& 局部融合参数,
    const 结构_世界树场景轮廓查找参数& 参数) const
{
    std::vector<结构_世界树场景轮廓存在比较结果> 结果集合{};
    if (!场景 || !参数.三维体素特征类型) return 结果集合;
    ...
    for (auto* 候选存在 : 存在服务_.获取子存在(场景)) {
        if (!候选存在 || !是实例存在(候选存在)) continue;

        auto* 三维体素特征 = 特征服务_.查找子特征_按类型(候选存在, 参数.三维体素特征类型);
        VecU句柄 存在体素根句柄{};
        if (!特征服务_.读取三维体素特征值(三维体素特征, 存在体素根句柄)) continue;

        Vector3D 存在原点{};
        if (!读取存在场景绝对坐标(候选存在, 存在原点)) continue;
    ...
    std::stable_sort(
        结果集合.begin(),
        结果集合.end(),
        [](const 结构_世界树场景轮廓存在比较结果& 左,
           const 结构_世界树场景轮廓存在比较结果& 右) {
