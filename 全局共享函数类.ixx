...
// 功能：判断两个 I64 闭区间是否重叠，并可输出闭区间重叠长度。
export inline bool 闭区间重叠I64(
    I64 第一最小值,
    I64 第一最大值,
    I64 第二最小值,
    I64 第二最大值,
    I64* 输出重叠长度) noexcept
{
    const auto 重叠最小 = (std::max<I64>)(第一最小值, 第二最小值);
    const auto 重叠最大 = (std::min<I64>)(第一最大值, 第二最大值);
...
// 功能：返回两个 I64 闭区间对应端点绝对差中的较大值。
export inline I64 区间端点最大差I64(
    I64 第一最小值,
    I64 第一最大值,
    I64 第二最小值,
    I64 第二最大值) noexcept
...
// 功能：按两个 I64 闭区间跨度的一半计算容差，并受最小容差保护。
export inline I64 区间跨度容差I64(
    I64 第一最小值,
    I64 第一最大值,
    I64 第二最小值,
    I64 第二最大值,
    I64 最小容差) noexcept
{
    const auto 第一跨度 = 绝对差I64(第一最大值, 第一最小值);
    const auto 第二跨度 = 绝对差I64(第二最大值, 第二最小值);
    return (std::max<I64>)(
        最小容差,
