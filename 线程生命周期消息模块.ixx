...
// 文件头部规则注释模块：
// 1. 本模块只负责线程生命周期消息文件、中间件和控制面板线程信息表缓存。
// 2. 线程生命周期消息只表达线程运行状态，不作为方法动作动态来源。
// 3. 文本字段只用于消息文件、控制面板显示和日志诊断，不承载业务事实裁决。
// 4. 纯格式化 / 纯计算工具优先复用 全局共享函数类，领域结构和状态编排保留在本模块。
...
export constexpr std::uint8_t 线程生命周期状态值_未设置 = 255;
...
export struct 结构_线程生命周期消息 {
    ...
    std::string 所属线程池ID{};
    std::string 所属线程池名称{};
    std::uint8_t 旧生命周期状态值 = 线程生命周期状态值_未设置;
    std::string 旧生命周期状态{};
    std::uint8_t 新生命周期状态值 = 线程生命周期状态值_未设置;
    std::string 新生命周期状态{};
    std::string 旧运行状态{};
    std::string 新运行状态{};
    bool 是否堵塞 = false;
...
export struct 结构_控制面板线程信息项 {
    ...
    std::string 所属线程池ID{};
    std::string 所属线程池名称{};
    std::uint8_t 生命周期状态值 = 线程生命周期状态值_未设置;
    std::string 生命周期状态{};
    std::string 运行状态{};
    bool 是否堵塞 = false;
    bool 是否暂停 = false;
...
namespace {
    ...
    {
        static const std::string 空{};
        const auto it = 字段.find(索引项字段键(索引, 键));
        return it == 字段.end() ? 空 : it->second;
    }

    // 功能：解析输入文本、消息、场景或运行包。
    bool 私有_解析事件类型(
        const std::string& 代码文本,
        枚举_线程生命周期消息事件& 输出) noexcept
    {
        const auto 代码 = 私有_解析U64(代码文本);
        if (代码 <= static_cast<std::uint64_t>(枚举_线程生命周期消息事件::异常退出)
            && !代码文本.empty()) {
            输出 = static_cast<枚举_线程生命周期消息事件>(代码);
            return true;
        }
        return false;
    }
        ...
        结构_线程生命周期消息& 消息)
    {
        ...
        if (!私有_解析事件类型(私有_取字段(字段, "事件类型代码"), 消息.事件类型)) {
            return false;
        }
        ...
        if (字段.contains("旧生命周期状态值")) {
            消息.旧生命周期状态值 = static_cast<std::uint8_t>(
                私有_解析U64(私有_取字段(字段, "旧生命周期状态值")));
        }
        消息.旧生命周期状态 = 私有_取字段(字段, "旧生命周期状态");
        if (字段.contains("新生命周期状态值")) {
            消息.新生命周期状态值 = static_cast<std::uint8_t>(
                私有_解析U64(私有_取字段(字段, "新生命周期状态值")));
        }
        消息.新生命周期状态 = 私有_取字段(字段, "新生命周期状态");
        消息.旧运行状态 = 私有_取字段(字段, "旧运行状态");
        消息.新运行状态 = 私有_取字段(字段, "新运行状态");
        消息.是否堵塞 = 解析一或零布尔文本(私有_取字段(字段, "是否堵塞"));
        消息.是否暂停 = 解析一或零布尔文本(私有_取字段(字段, "是否暂停"));
        消息.是否健康 = !字段.contains("是否健康")
            || 解析一或零布尔文本(私有_取字段(字段, "是否健康"));
        消息.是否正常退出 = 解析一或零布尔文本(私有_取字段(字段, "是否正常退出"));
        ...
        const std::size_t 索引)
    {
        ...
        const auto 生命周期状态值键 = 索引项字段键(索引, "生命周期状态值");
        if (字段.contains(生命周期状态值键)) {
            项.生命周期状态值 = static_cast<std::uint8_t>(
                私有_解析U64(私有_取项字段(字段, 索引, "生命周期状态值")));
        }
        项.生命周期状态 = 私有_取项字段(字段, 索引, "生命周期状态");
        项.运行状态 = 私有_取项字段(字段, 索引, "运行状态");
        项.是否堵塞 = 解析一或零布尔文本(私有_取项字段(字段, 索引, "是否堵塞"));
        项.是否暂停 = 解析一或零布尔文本(私有_取项字段(字段, 索引, "是否暂停"));
        项.是否健康 = !字段.contains(索引项字段键(索引, "是否健康"))
            || 解析一或零布尔文本(私有_取项字段(字段, 索引, "是否健康"));
        项.是否已退出 = 解析一或零布尔文本(私有_取项字段(字段, 索引, "是否已退出"));
        项.是否故障 = 解析一或零布尔文本(私有_取项字段(字段, 索引, "是否故障"));
        项.创建消息已到 = 解析一或零布尔文本(私有_取项字段(字段, 索引, "创建消息已到"));
    ...
    void 私有_写线程信息表缓存_已持锁() noexcept
    {
        try {
...

            {
                ...
                写入转义键值字段行(输出, "版本", "1");
                写入转义键值字段行(输出, "项数", std::to_string(项集.size()));
                for (std::size_t i = 0; i < 项集.size(); ++i) {
                    const auto& 项 = *项集[i];
                    写入索引项转义键值字段行(输出, i, "首次消息ID", std::to_string(项.首次消息ID));
                    写入索引项转义键值字段行(输出, i, "最近消息ID", std::to_string(项.最近消息ID));
                    写入索引项转义键值字段行(输出, i, "创建时间", std::to_string(项.创建时间));
                    写入索引项转义键值字段行(输出, i, "最近更新时间", std::to_string(项.最近更新时间));
                    写入索引项转义键值字段行(输出, i, "退出时间", std::to_string(项.退出时间));
                    写入索引项转义键值字段行(输出, i, "线程逻辑ID", 项.线程逻辑ID);
                    写入索引项转义键值字段行(输出, i, "系统线程ID", std::to_string(项.系统线程ID));
                    写入索引项转义键值字段行(输出, i, "线程名称", 项.线程名称);
                    写入索引项转义键值字段行(输出, i, "线程用途", 项.线程用途);
                    写入索引项转义键值字段行(输出, i, "线程类别", 项.线程类别);
                    写入索引项转义键值字段行(输出, i, "所属模块", 项.所属模块);
                    写入索引项转义键值字段行(输出, i, "创建者逻辑ID", 项.创建者逻辑ID);
                    写入索引项转义键值字段行(输出, i, "创建者名称", 项.创建者名称);
                    写入索引项转义键值字段行(输出, i, "所属线程池ID", 项.所属线程池ID);
                    写入索引项转义键值字段行(输出, i, "所属线程池名称", 项.所属线程池名称);
                    写入索引项转义键值字段行(输出, i, "生命周期状态值", std::to_string(项.生命周期状态值));
                    写入索引项转义键值字段行(输出, i, "生命周期状态", 项.生命周期状态);
                    写入索引项转义键值字段行(输出, i, "运行状态", 项.运行状态);
                    写入索引项转义键值字段行(输出, i, "是否堵塞", 布尔文本_一或零(项.是否堵塞));
                    写入索引项转义键值字段行(输出, i, "是否暂停", 布尔文本_一或零(项.是否暂停));
                    写入索引项转义键值字段行(输出, i, "是否健康", 布尔文本_一或零(项.是否健康));
                    写入索引项转义键值字段行(输出, i, "是否已退出", 布尔文本_一或零(项.是否已退出));
                    写入索引项转义键值字段行(输出, i, "是否故障", 布尔文本_一或零(项.是否故障));
                    写入索引项转义键值字段行(输出, i, "创建消息已到", 布尔文本_一或零(项.创建消息已到));
                    写入索引项转义键值字段行(输出, i, "关联任务ID", std::to_string(项.关联任务ID));
                    写入索引项转义键值字段行(输出, i, "关联工作项ID", std::to_string(项.关联工作项ID));
                    写入索引项转义键值字段行(输出, i, "最近事件类型", 项.最近事件类型);
                    写入索引项转义键值字段行(输出, i, "最近事件键", 项.最近事件键);
                    写入索引项转义键值字段行(输出, i, "最近事件摘要", 项.最近事件摘要);
                    写入索引项转义键值字段行(输出, i, "版本", std::to_string(项.版本));
                    写入索引项转义键值字段行(输出, i, "迟到消息数", std::to_string(项.迟到消息数));
                }
            }
    ...
    void 私有_应用线程生命周期消息_已持锁(const 结构_线程生命周期消息& 消息)
    {
        ...
        if (消息.新生命周期状态值 != 线程生命周期状态值_未设置) {
            项.生命周期状态值 = 消息.新生命周期状态值;
        }
        ...
        if (消息.事件类型 == 枚举_线程生命周期消息事件::已退出
            ...
            项.退出时间 = 消息.发生时间;
            if (项.生命周期状态值 == 线程生命周期状态值_未设置) {
                项.生命周期状态值 = 消息.事件类型 == 枚举_线程生命周期消息事件::异常退出 ? 6 : 5;
            }
            if (项.生命周期状态.empty()) {
                项.生命周期状态 = "已退出";
            }
        ...
        const 结构_线程生命周期消息& 消息) noexcept
    {
        try {
            ...
            输出 << "\n";
            写入转义键值字段行(输出, "记录类型", "线程生命周期消息");
            写入转义键值字段行(输出, "归档源文件", 源路径.filename().string());
            写入转义键值字段行(输出, "归档时间", std::to_string(私有_当前时间_微秒()));
            写入转义键值字段行(输出, "原始消息开始", "1");
            输出 << 输入.rdbuf();
            输出 << "\n";
            写入转义键值字段行(输出, "记录结束", "线程生命周期消息");
            return true;
        } catch (const std::exception& 异常) {
            项目运行错误日志(
        ...
        std::size_t& 删除失败数)
    {
        ...
        for (const auto& 项 : std::filesystem::directory_iterator(目录)) {
            ...
            const auto 字段 = 私有_读取字段文件(路径);
            结构_线程生命周期消息 消息{};
            const bool 消息有效 = 私有_从字段构造消息(字段, 消息);
            if ((!消息有效 || 消息.消息ID == 0 || 消息.线程逻辑ID.empty())
...
export bool 发送线程生命周期消息(结构_线程生命周期消息 消息) noexcept
{
    ...
    try {
...

        {
            ...
            写入转义键值字段行(输出, "消息ID", std::to_string(消息.消息ID));
            写入转义键值字段行(输出, "事件类型代码", 私有_事件文件代码(消息.事件类型));
            写入转义键值字段行(输出, "事件类型", 线程生命周期消息事件文本(消息.事件类型));
            写入转义键值字段行(输出, "发生时间", std::to_string(消息.发生时间));
            写入转义键值字段行(输出, "线程逻辑ID", 消息.线程逻辑ID);
            写入转义键值字段行(输出, "系统线程ID", std::to_string(消息.系统线程ID));
            写入转义键值字段行(输出, "线程名称", 消息.线程名称);
            写入转义键值字段行(输出, "线程用途", 消息.线程用途);
            写入转义键值字段行(输出, "线程类别", 消息.线程类别);
            写入转义键值字段行(输出, "所属模块", 消息.所属模块);
            写入转义键值字段行(输出, "创建者逻辑ID", 消息.创建者逻辑ID);
            写入转义键值字段行(输出, "创建者名称", 消息.创建者名称);
            写入转义键值字段行(输出, "所属线程池ID", 消息.所属线程池ID);
            写入转义键值字段行(输出, "所属线程池名称", 消息.所属线程池名称);
            写入转义键值字段行(输出, "旧生命周期状态值", std::to_string(消息.旧生命周期状态值));
            写入转义键值字段行(输出, "旧生命周期状态", 消息.旧生命周期状态);
            写入转义键值字段行(输出, "新生命周期状态值", std::to_string(消息.新生命周期状态值));
            写入转义键值字段行(输出, "新生命周期状态", 消息.新生命周期状态);
            写入转义键值字段行(输出, "旧运行状态", 消息.旧运行状态);
            写入转义键值字段行(输出, "新运行状态", 消息.新运行状态);
            写入转义键值字段行(输出, "是否堵塞", 布尔文本_一或零(消息.是否堵塞));
            写入转义键值字段行(输出, "是否暂停", 布尔文本_一或零(消息.是否暂停));
            写入转义键值字段行(输出, "是否健康", 布尔文本_一或零(消息.是否健康));
            写入转义键值字段行(输出, "是否正常退出", 布尔文本_一或零(消息.是否正常退出));
            写入转义键值字段行(输出, "关联任务ID", std::to_string(消息.关联任务ID));
            写入转义键值字段行(输出, "关联工作项ID", std::to_string(消息.关联工作项ID));
            写入转义键值字段行(输出, "原因键", 消息.原因键);
            写入转义键值字段行(输出, "显示摘要", 消息.显示摘要);
        }
