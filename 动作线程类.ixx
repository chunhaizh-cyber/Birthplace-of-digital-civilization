    ...
    bool 从本能动作管理器注册(枚举_本能方法ID 动作ID)
    {
        ...
        注册本能(动作ID, [动作ID, 动作上下文 = std::move(动作上下文)](动作执行上下文& 上下文) mutable {
            auto* 方法首节点 = reinterpret_cast<方法类::节点类*>(
                上下文.方法根 ? 上下文.方法根 : 动作上下文.方法信息首节点);
            auto* 输入场景 = 上下文.输入场景 ? 上下文.输入场景 : 上下文.场景;
            auto* 输出场景 = 上下文.输出场景 ? 上下文.输出场景 : 输入场景;

            auto* 本能运行结果 = 取本能动作集().执行功能函数(
                动作ID,
                方法首节点,
                输入场景,
                输出场景);
            ...
            结构_动作步骤结果 输出{};
            输出.动作ID = 动作ID;
            输出.成功码 = 本能动作运行结果值(本能运行结果);
            const auto 运行闭环视图 = 读取本能方法运行闭环视图(本能运行结果, 输出场景);
