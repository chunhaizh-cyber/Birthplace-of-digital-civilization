...
// 功能：判断获取途径是否属于简化需求树准入的四类输出。
bool 需求类::需求获取途径是简化准入输出(
    枚举_需求获取途径 途径) noexcept
{
    switch (途径) {
    case 枚举_需求获取途径::自检原子缺口:
...
// 功能：按四类缺口选择需求获取途径，不创建或修改需求树。
需求类::枚举_需求获取途径 需求类::需求_按缺口类型选择获取途径(
    bool 根特征缺口,
    bool 任务筹办缺口,
    bool 执行期真实变化缺口,
    bool 可验证服务触发事实,
    bool& 缺口输入不足,
    bool& 缺口输入冲突) noexcept
{
    const int 命中数量 =
        (根特征缺口 ? 1 : 0)
        + (任务筹办缺口 ? 1 : 0)
    ...
    缺口输入冲突 = 命中数量 > 1;
    if (缺口输入不足 || 缺口输入冲突) {
        return 枚举_需求获取途径::未定义;
    }
    if (根特征缺口) {
        return 枚举_需求获取途径::自检原子缺口;
    }
    if (任务筹办缺口) {
        return 枚举_需求获取途径::任务筹办原子缺口;
    }
    if (执行期真实变化缺口) {
        return 枚举_需求获取途径::任务执行回执原子缺口;
    }
    return 枚举_需求获取途径::可验证服务触发事实;
}

// 功能：判断内部治理缺陷是否必须阻断普通需求入树。
bool 需求类::需求_内部治理缺陷禁止普通入树(
    bool 缺内部治理函数,
    bool 缺结构字段,
    bool 缺协议入口) noexcept
{
    return 缺内部治理函数 || 缺结构字段 || 缺协议入口;
}
...
// 功能：把父需求视角下的子需求关系角色转成人读文本，不参与机器判断。
const char* 需求类::父子需求关系角色文本(
    枚举_父子需求关系角色 角色) noexcept
{
    switch (角色) {
    case 枚举_父子需求关系角色::必须子需求:
        return "必须子需求";
    case 枚举_父子需求关系角色::可选路径子需求:
        return "可选路径子需求";
    case 枚举_父子需求关系角色::非必须子需求:
        return "非必须子需求";
    case 枚举_父子需求关系角色::未定义:
    default:
        return "未定义";
    }
}

// 功能：只读推导父需求视角下的子需求关系角色，不修改需求树。
需求类::枚举_父子需求关系角色 需求类::读取父子关系角色视图(
    const 节点类* 父需求,
    const 节点类* 子需求,
    const 语素入口节点类* 当前方法路径目标特征类型) noexcept
{
    if (!父需求 || !子需求
        || static_cast<const 节点类*>(子需求->父) != 父需求) {
        return 枚举_父子需求关系角色::未定义;
    }

    if (需求已截止(子需求) || !需求阻塞父任务执行(子需求)) {
        return 枚举_父子需求关系角色::非必须子需求;
    }

    const auto* 子目标特征类型 = 读取需求目标特征类型(子需求);
    if (当前方法路径目标特征类型
        && 子目标特征类型 == 当前方法路径目标特征类型) {
        return 枚举_父子需求关系角色::必须子需求;
    }

    const auto 父目标语义 = 需求目标语义视图(父需求);
    if (父目标语义.逻辑组织类型 == 枚举_逻辑组织需求类型::OR组
        || 父目标语义.逻辑组织类型 == 枚举_逻辑组织需求类型::方法路径组) {
        return 枚举_父子需求关系角色::可选路径子需求;
    }
...
// 功能：构造满足事件后的本轮承接收束更新指令，不直接修改需求树。
