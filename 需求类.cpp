...
namespace {
        ...
        std::vector<结构_需求树SQL行>& 行集)
    {
        ...
        auto 节点名称 = 语素集.安全获取词(需求类::读取需求目标特征类型(节点));
        if (深度 == 0) {
            节点名称 = "需求树";
        }
        ...
        const std::string& 主键) noexcept
    {
        ...
        return 世界树.取状态主信息(状态节点) ? 状态节点 : nullptr;
    }
        ...
        const 状态节点类* 状态) noexcept
    {
        ...
        return 世界树.取特征主信息(特征节点) ? 特征节点 : nullptr;
    }
    ...
    {
        const auto* 特征主信息 =
            世界树.取特征主信息(私有_解析状态特征节点_需求校验(状态));
        ...
        bool* 输出有子状态 = nullptr) noexcept
    {
        ...
        std::size_t 点状态数 = 0;
        for (auto* 子节点 : 世界树.获取子节点_按主信息类型<抽象特征主信息类>(抽象特征)) {
            auto* 子抽象 = static_cast<抽象特征节点类*>(子节点);
            const auto* 子主信息 = 世界树.取抽象特征主信息(子抽象);
        ...
        const 特征值& 目标值) noexcept
    {
        ...
        const auto* 抽象主信息 = 世界树.取抽象特征主信息(抽象特征);
        return 抽象主信息
            && 抽象主信息->是叶子特征()
            && 抽象主信息->值域.has_value()
        ...
        const 语素入口节点类* 目标特征类型) noexcept
    {
        ...
        auto* 因果节点 = 世界树.按主键解析因果节点(来源因果主键);
        if (!因果节点) {
            return 枚举_枚举目标生产者分级::未登记;
        }
        ...
        if (有方法模板) {
            return 方法类::方法首节点生产目标特征(因方法, 目标特征类型)
                ? 枚举_枚举目标生产者分级::专用方法生产
                : 枚举_枚举目标生产者分级::待补齐;
        }
    ...
    三向关系掩码 满足关系) noexcept
{
    ...
    const auto* 目标状态主信息 = 世界树.取状态主信息(目标状态);
    结果.目标值属于值域 = 目标状态主信息
...
二次特征节点类* 需求类::获取需求方向(节点类* 需求节点) const noexcept
{
    ...
    const auto* 初始状态主信息 = 世界树.取状态主信息(初始状态);
    const auto* 结果状态主信息 = 世界树.取状态主信息(结果状态);
    if (!初始状态主信息 || !结果状态主信息) {
        return nullptr;
    }
    ...
    if (!初始特征类型 || !结果特征类型 || !语素入口同一(初始特征类型, 结果特征类型)) {
        return nullptr;
    }

    const auto 比较方法结果 = 世界树.获取比较方法结果(
        初始特征类型,
        初始状态主信息->状态值,
        结果状态主信息->状态值,
        初始特征主信息,
    ...
    const auto* 二次特征主信息 = 世界树.取二次特征主信息(节点);
