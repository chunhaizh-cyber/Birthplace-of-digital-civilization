...
bool 私有_校验封口时间(
    const 自我线程运行包::结构_线程运行包头& 包头,
    std::string* 错误) noexcept
{
    if (包头.包状态 == 自我线程运行包::枚举_包状态::已封口 && 包头.封口时间 == 0) {
...
bool 私有_封口包(
    T包& 包,
    const 自我线程运行包::时间戳 封口时间,
    bool(*校验函数)(const T包&, std::string*) noexcept,
    std::string* 错误) noexcept
{
    if (包.包头.包状态 == 自我线程运行包::枚举_包状态::已封口) {
        return 校验函数(包, 错误);
    }
    if (!校验函数(包, 错误)) {
        return false;
    }
    包.包头.包状态 = 自我线程运行包::枚举_包状态::已封口;
...
bool 校验上下文包(const 结构_线程轮次上下文包& 包, std::string* 错误) noexcept
{
    if (!校验包头(包.包头, 错误)) {
        return false;
    }
    if (包.允许推进 && !包.主导需求.has_value()) {
        私有_写错误(错误, "允许推进的上下文包缺少主导需求");
        return false;
    }
    if (包.时间预算_ms < 0 || 包.资源预算 < 0 || 包.并发上限 < 0 || 包.最大授权等级 < 0) {
        私有_写错误(错误, "上下文包预算字段不能为负数");
        return false;
    }
    return true;
}
...
bool 封口上下文包(结构_线程轮次上下文包& 包, 时间戳 封口时间, std::string* 错误) noexcept
{
    return 私有_封口包(包, 封口时间, 校验上下文包, 错误);
}
