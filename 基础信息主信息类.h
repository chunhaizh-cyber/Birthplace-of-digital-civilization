...
public:
    ...
    std::int64_t 比较(基础信息基类* 对象, 枚举_比较字段 字段) const override {
        ...
        case 枚举_比较字段::名称:
            return reinterpret_cast<std::intptr_t>(名称) - reinterpret_cast<std::intptr_t>(右->名称);
        case 枚举_比较字段::基础信息基类_类型:
        case 枚举_比较字段::类型:
            return reinterpret_cast<std::intptr_t>(类型) - reinterpret_cast<std::intptr_t>(右->类型);
        default:
            return 语言词性 == 右->语言词性 ? 0
                : (reinterpret_cast<std::intptr_t>(语言词性) < reinterpret_cast<std::intptr_t>(右->语言词性) ? -1 : 1);
        }
    }
};
