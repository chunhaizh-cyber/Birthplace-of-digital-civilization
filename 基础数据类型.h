...
enum class 枚举_存在观测缓冲 : std::uint32_t {
    无 = 0,
    裁剪BGR = 1u << 0,
    裁剪掩膜 = 1u << 1,
    边缘图 = 1u << 2,
...
// 功能：按函数名执行对应处理。
inline 枚举_存在观测缓冲 operator|(枚举_存在观测缓冲 a, 枚举_存在观测缓冲 b) {
...
enum class 枚举_评估阶段 : std::uint8_t {
    未知 = 0,
    证据不足 = 1,
    可用 = 2,
    需缺口承接 = 3,
...
enum class 枚举_特征使用者 : std::uint8_t {
    未定义 = 0,
    下一步合成器,
    特征组合器,
    需求显现器,
...
enum class 枚举_特征使用方式 : std::uint8_t {
    未定义 = 0,
    决策输入,
    排序信号,
    过滤条件,
...
enum class 枚举_特征结果影响 : std::uint8_t {
    未定义 = 0,
    选择步骤改变,
    优先级改变,
    风险阻断动作,
