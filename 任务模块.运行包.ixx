...
struct 结构_线程运行包头 {
    ...
    const 语素入口节点类* 包状态抽象特征指针 = nullptr;
...
struct 结构_根需求项 {
    ...
    const 语素入口节点类* 需求大类抽象特征指针 = nullptr;
    I64 需求大类值 = static_cast<I64>(枚举_需求大类::服务);
    const 语素入口节点类* 服务子类抽象特征指针 = nullptr;
    I64 服务子类值 = static_cast<I64>(枚举_服务子类::无);
    const 语素入口节点类* 安全子类抽象特征指针 = nullptr;
...
struct 结构_方法沉淀候选项 {
    ...
    const 语素入口节点类* 方法沉淀样本类别抽象特征指针 = nullptr;
...
struct 结构_一步治理任务包 {
    ...
    const 语素入口节点类* 控制意图抽象特征指针 = nullptr;
...
struct 结构_回流结算包 {
    ...
    const 语素入口节点类* 结算结论抽象特征指针 = nullptr;
