...
namespace {
    ...
    const 词性节点类* 私有_任务虚拟存在名称词(const 任务节点* 节点) noexcept
    {
        ...
        return 语素集.添加词性词("任务虚拟存在_" + 后缀, "专有名词");
    }
    ...
    const 词性节点类* 私有_特征_任务已折算安全值() noexcept
    {
        static const 词性节点类* s_词 = 语素集.添加词性词("任务已折算安全值", "名词");
        return s_词;
    }

    const 词性节点类* 私有_特征_任务已折算服务值() noexcept
    {
        static const 词性节点类* s_词 = 语素集.添加词性词("任务已折算服务值", "名词");
        return s_词;
    }
    ...
    任务节点* 私有_查找最新步骤节点(任务节点* 任务头结点) noexcept
    {
        ...
        while (!待查.empty()) {
            ...
            if (当前 != 任务头结点 && 任务类::读取任务节点种类(当前) == 枚举_任务节点种类::步骤节点) {
    ...
    任务节点* 私有_查找步骤结果节点(任务节点* 步骤节点) noexcept
    {
        ...
        auto* 当前 = 起点;
        do {
            if (任务类::读取任务节点种类(当前) != 枚举_任务节点种类::结果节点) {
        ...
        const std::string& 调用点) noexcept
    {
        ...
        (void)任务类::写入任务节点种类(结果节点, 枚举_任务节点种类::结果节点, 场景, now);
        (void)任务类::写入任务状态(结果节点, 枚举_任务状态::未启动, 场景, now);
        (void)任务类::写入任务执行方法(
            结果节点,
    ...
    时间戳 now) noexcept
{
    ...
    auto* 任务虚拟存在 = 私有_创建并初始化任务虚拟存在(任务节点, 承载场景, "任务类::写入任务节点种类");
    return 任务虚拟存在
        && 世界树.写入特征_I64(
            reinterpret_cast<基础信息节点类*>(任务虚拟存在),
            自我特征定义类::类型_任务_节点种类(),
    ...
    const std::string& 调用点)
{
    ...
    if (任务类::读取任务节点种类(挂接父节点) == 枚举_任务节点种类::步骤节点) {
    ...
    if (任务类::读取任务节点种类(挂接父节点) != 枚举_任务节点种类::头结点
        && 任务类::读取任务节点种类(挂接父节点) != 枚举_任务节点种类::结果节点) {
        return nullptr;
    }
    ...
    const auto 初始状态 = 当前方法首节点 ? 枚举_任务状态::就绪 : 枚举_任务状态::筹办中;
    主信息.创建时间 = now;
    ...
    (void)任务类::写入任务节点种类(步骤节点, 枚举_任务节点种类::步骤节点, 场景, now);
    (void)任务类::写入任务状态(步骤节点, 初始状态, 场景, now);
    (void)任务类::写入任务执行方法(
        步骤节点,
        reinterpret_cast<方法节点类*>(当前方法),
        场景,
