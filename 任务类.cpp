...
// 功能：从指定来源读取数据或状态。
const 可解析引用<状态节点类>& 任务类::读取任务结果状态摘要_兼容(
    const 节点类* 任务节点) noexcept
{
    static const 可解析引用<状态节点类> 空引用{};
    auto* 头结点 = 私有_归一到任务头结点(const_cast<节点类*>(任务节点));
    return 头结点 ? 头结点->主信息.结果状态信息 : 空引用;
}

// 功能：解析输入文本、消息、场景或运行包。
状态节点类* 任务类::解析任务结果状态摘要_兼容(
    const 节点类* 任务节点) noexcept
{
    return 私有_解析状态引用(读取任务结果状态摘要_兼容(任务节点));
}

// 功能：把处理结果写入指定对象、场景或日志。
bool 任务类::写入任务结果状态摘要_兼容(
    节点类* 任务节点,
    ...
    场景节点类* 承载场景) noexcept
{
    ...
    if (!任务虚拟存在 && !头结点->主信息.任务虚拟存在.主键.empty()) {
        if (auto* 基础节点 = 世界树.基础信息().查找主键(头结点->主信息.任务虚拟存在.主键)) {
            if (世界树.存在().取存在主信息(static_cast<存在节点类*>(基础节点))) {
                任务虚拟存在 = static_cast<存在节点类*>(基础节点);
            }
        }
    }
    if (!任务虚拟存在) {
