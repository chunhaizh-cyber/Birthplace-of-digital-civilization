...
namespace {
    ...
    状态节点类* 私有_取需求目标状态(需求节点类* 来源需求) noexcept
    {
        ...
        if (来源需求->子节点数量 > 0) {
            auto* 首子 = static_cast<需求节点类*>(来源需求->子);
            auto* 当前 = 首子;
            std::size_t 保护计数 = 0;
            do {
                if (当前 && 当前->主信息.需求有效截止 == 0) {
                    if (auto* 状态 = 私有_解析状态引用(当前->主信息.需求状态)) {
                        return 状态;
                    }
                }
                当前 = 当前 ? static_cast<需求节点类*>(当前->下) : nullptr;
                ++保护计数;
    ...
    状态节点类* 私有_取需求当前状态(需求节点类* 来源需求) noexcept
    {
        ...
        if (来源需求->子节点数量 > 0) {
            ...
            do {
                if (当前 && 当前->主信息.需求有效截止 == 0) {
                    if (auto* 状态 = 私有_解析状态引用(当前->主信息.被需求当前状态)) {
                        return 状态;
                    }
                }
                当前 = 当前 ? static_cast<需求节点类*>(当前->下) : nullptr;
    ...
    状态节点类* 私有_取方法首个结果初始状态(方法节点* 方法首节点) noexcept
    {
        if (!方法首节点 || !方法首节点->子) {
            return nullptr;
        }

        auto* 首子节点 = static_cast<方法节点*>(方法首节点->子);
        auto* 当前 = 首子节点;
        do {
            if (当前 && 当前->主信息.节点种类() == 枚举_方法节点种类::方法结果节点) {
                if (const auto* 结果信息 = 当前->主信息.取结果节点信息()) {
                    if (auto* 初始状态 = 私有_取首个有效状态(结果信息->结果初始状态)) {
                        return 初始状态;
                    }
                }
            }
    ...
    const 词性节点类* 私有_特征_任务承接安全权重上限() noexcept
    {
        static const 词性节点类* s_词 = 语素集.添加词性词("任务承接安全权重上限", "名词");
        return s_词;
    }

    const 词性节点类* 私有_特征_任务承接服务权重上限() noexcept
    {
        static const 词性节点类* s_词 = 语素集.添加词性词("任务承接服务权重上限", "名词");
        return s_词;
    }

    const 词性节点类* 私有_特征_任务权重评分() noexcept
    {
        static const 词性节点类* s_词 = 语素集.添加词性词("任务权重评分", "名词");
        return s_词;
    }

    const 词性节点类* 私有_特征_任务权重版本号() noexcept
    {
        static const 词性节点类* s_词 = 语素集.添加词性词("任务权重版本号", "名词");
        return s_词;
    }
        ...
        std::int32_t 步骤序号) noexcept
    {
        ...
        if (当前方法 && 当前方法->主信息.首节点信息().动作名) {
            return 当前方法->主信息.首节点信息().动作名;
        }
        ...
        状态节点类* 显式目标状态) noexcept
    {
        ...
        if (当前方法) {
            if (auto* 初始状态 = 私有_取方法首个结果初始状态(当前方法)) {
                return 初始状态;
            }
        }
        ...
        if (所属任务头结点 && 所属任务头结点->主信息.对应需求.指针) {
            return 私有_取需求目标状态(所属任务头结点->主信息.对应需求.指针);
        }
    ...
    场景节点类* 承载场景)
{
    ...
    return 私有_创建并初始化任务虚拟存在(头结点, 承载场景);
}
