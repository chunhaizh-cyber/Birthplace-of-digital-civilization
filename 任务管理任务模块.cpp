    ...
    结构_任务管理请求* 输出) noexcept
{
    ...
    if (!上下文.最近上层反馈摘要.empty()) {
        输出->触发事件类型 = 枚举_任务管理触发事件类型::外部反馈;
    }
    else if (!上下文.最近反馈摘要.empty() && 上下文.最近结果) {
        输出->触发事件类型 = 上下文.最近反馈摘要.find("学习") != std::string::npos
            ? 枚举_任务管理触发事件类型::学习回流
            : 枚举_任务管理触发事件类型::执行回流;
    }
    else {
    ...
    结构_任务管理写回结果* 输出) noexcept
{
    ...
    输出->双面摘要 = {};
    输出->结构动作摘要 = "P0骨架阶段：写回结果先由旧治理流程回填；候选去向=" + 私有_去向文本(决策.当前下一步去向);
    return true;
}
    ...
    结构_任务管理本能能力缺口面* 输出) noexcept
{
    ...
    constexpr std::uint64_t 位_确保单承载存在 = 1ULL << 0;
    constexpr std::uint64_t 位_读取单特征当前值 = 1ULL << 1;
    constexpr std::uint64_t 位_写入单特征当前值 = 1ULL << 2;
    constexpr std::uint64_t 位_写入单挂点指针 = 1ULL << 3;
    constexpr std::uint64_t 位_记录动作开始 = 1ULL << 4;
    constexpr std::uint64_t 位_记录动作结束 = 1ULL << 5;
    constexpr std::uint64_t 位_状态差分取证 = 1ULL << 6;
    constexpr std::uint64_t 位_实例因果派生 = 1ULL << 7;
    constexpr std::uint64_t 位_轻量因果链组织 = 1ULL << 8;
    ...
    输出->已具备写入单特征当前值 = true;
    输出->已具备写入单挂点指针 = true;
    输出->已具备记录动作开始 = 需求位特征.最近开始动态 != nullptr;
    输出->已具备记录动作结束 = 需求位特征.最近结束动态 != nullptr;
    输出->已具备状态差分取证 =
    ...
    输出->已具备原语位图 = 0;
    if (输出->已具备确保单承载存在) 输出->已具备原语位图 |= 位_确保单承载存在;
    if (输出->已具备读取单特征当前值) 输出->已具备原语位图 |= 位_读取单特征当前值;
    if (输出->已具备写入单特征当前值) 输出->已具备原语位图 |= 位_写入单特征当前值;
    if (输出->已具备写入单挂点指针) 输出->已具备原语位图 |= 位_写入单挂点指针;
    if (输出->已具备记录动作开始) 输出->已具备原语位图 |= 位_记录动作开始;
    if (输出->已具备记录动作结束) 输出->已具备原语位图 |= 位_记录动作结束;
    if (输出->已具备状态差分取证) 输出->已具备原语位图 |= 位_状态差分取证;
    if (输出->已具备实例因果派生) 输出->已具备原语位图 |= 位_实例因果派生;
    if (输出->已具备轻量因果链组织) 输出->已具备原语位图 |= 位_轻量因果链组织;
    ...
    else if (!输出->已具备记录动作开始 || !输出->已具备记录动作结束) {
    ...
    结构_任务管理结果* 输出) noexcept
{
    ...
    std::string 锚点裁决摘要{};
    auto* 宿主任务 = 私有_选择宿主任务(自我对象, 结果.上下文, &锚点裁决摘要);
    结果.锚点裁决摘要 = 锚点裁决摘要;
    ...
    if (当前方法来源 != 枚举_任务管理方法来源::未定义) {
        结果.最近反馈摘要 += "；方法来源=" + 私有_方法来源文本(当前方法来源);
    }
    if (当前步骤位类型 != 枚举_任务管理步骤位类型::未定义) {
        结果.最近反馈摘要 += "；步骤位=" + 私有_步骤位类型文本(当前步骤位类型);
    }
