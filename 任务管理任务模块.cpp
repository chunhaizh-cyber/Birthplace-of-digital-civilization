...
namespace {
    ...
    void 私有_P0_刷新主体桥接字段(结构_任务管理结果& 结果) noexcept
    {
        ...
        }
        {
            ...
            请求摘要 << "宿主=" << 私有_任务标题(结果.上下文.当前宿主任务)
                ...
                << " | 原语位图=" << (结果.来源最小原语位图 == 0 ? std::string("空") : 私有_原语位图摘要(结果.来源最小原语位图))
    ...
    bool 私有_评估本能能力缺口(结构_任务管理结果& 结果) noexcept
    {
        结构_任务管理本能能力缺口面 当前缺口{};
        const bool 已评估 =
    ...
    bool 私有_桥接补层裁决(结构_任务管理结果& 结果) noexcept
    {
        ...
        摘要 << "桥接补层裁决"
            ...
            << " | 原语=" << (结果.当前本能能力缺口.原语账本摘要.empty() ? "空" : 结果.当前本能能力缺口.原语账本摘要)
            << " | 派生=" << (结果.当前本能能力缺口.派生规则摘要.empty() ? "空" : 结果.当前本能能力缺口.派生规则摘要);
        ...
        const 结构_任务管理结果& 结果) noexcept
    {
        ...
        摘要 << "学习承接="
            ...
            << " | 原语位图=" << (决策.来源最小原语位图 == 0 ? std::string("空") : 私有_原语位图摘要(决策.来源最小原语位图))
...
namespace {
        ...
        结构_M2治理阶段状态& 阶段状态) noexcept
    {
        ...
        if (阶段状态.宿主任务 && 阶段状态.宿主任务 != 结果.上下文.当前宿主任务) {
    ...
    结构_任务管理请求* 输出) noexcept
{
    ...
    结构_任务管理显式事件控制 显式事件{};
    const bool 命中显式事件 = 私有_尝试消费治理显式事件控制(&显式事件);
    结构_治理恢复快照 恢复快照{};
    const bool 命中恢复重建 = 私有_尝试消费治理恢复重建控制(&恢复快照);
    if (命中恢复重建) {
    ...
    输出->来源最小原语位图 = 命中显式事件 ? 显式事件.来源最小原语位图 : 0;
    输出->来源主观察特征 = 半白盒控制.启用 ? 半白盒控制.来源主观察特征 : 显式事件.来源主观察特征;
    输出->影子验证状态 = 半白盒控制.启用
        ? (半白盒控制.影子验证状态.empty() ? std::string("未触发") : 半白盒控制.影子验证状态)
        : (显式事件.影子验证状态.empty() ? std::string("未触发") : 显式事件.影子验证状态);
    输出->允许正式资产提交 = 半白盒控制.启用 ? 半白盒控制.允许正式资产提交 : 显式事件.允许正式资产提交;
    输出->触发事件摘要 = 命中显式事件 ? 显式事件.事件摘要 : std::string{};
    输出->当前恢复点类型.clear();
    输出->存在待消费学习回流 = false;
    输出->存在待消费外部反馈 = false;
    输出->恢复重建摘要.clear();
    ...
    if (命中恢复重建) {
        ...
        if (恢复快照.最近影子验证状态.empty()) {
            输出->影子验证状态 = "未触发";
        }
        else {
            输出->影子验证状态 = 恢复快照.最近影子验证状态;
        }
    ...
    if (命中显式事件 && !显式事件.事件摘要.empty()) {
        摘要 << " | mailbox=" << 显式事件.事件摘要;
    }
    else if (!输出->触发事件摘要.empty()) {
    ...
    结构_任务管理单步决策* 输出) noexcept
{
    ...
    摘要 << "功能域=" << 私有_功能域文本(输出->当前功能域)
        ...
        << " | 原语位图=" << (输出->来源最小原语位图 == 0 ? std::string("空") : 私有_原语位图摘要(输出->来源最小原语位图))
        << " | 主观察=" << (输出->来源主观察特征.empty() ? "空" : 输出->来源主观察特征)
        << " | 影子验证=" << (输出->影子验证状态.empty() ? "未触发" : 输出->影子验证状态)
        << " | 正式提交=" << (输出->允许正式资产提交 ? "允许" : "禁止");
    输出->决策摘要 = 摘要.str();
    ...
    结构_任务管理结果* 输出) noexcept
{
    ...
    const bool 已受上位输入约束 = 阶段状态.已受上位输入约束;
