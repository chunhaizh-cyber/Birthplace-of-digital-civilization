...
namespace {
        ...
        时间戳 now) noexcept
    {
        ...
        输入.当前下一步去向 = 当前下一步去向;
        输入.当前方法来源 = 当前方法来源;
        输入.来源方法需求位 = 私有_P0_当前方法需求位(结果);
        输入.来源本能能力缺口类型 = 私有_P0_当前本能缺口(结果);
        输入.回流目标方法位 = 私有_P0_当前方法需求位(结果);
        const bool 本能补齐学习 =
        ...
        输入.来源最小原语位图 = 结果.当前本能能力缺口.所需原语位图;
        输入.来源主观察特征 = 结果.当前本能能力缺口.主观察特征键;
        输入.回流目标摘要 =
    ...
    时间戳 now) noexcept
{
    ...
    上下文.当前主体ID = 私有_读取U64文本特征(
        自我对象,
        私有_管理特征_当前主体ID(),
        "任务管理任务模块::读取任务管理上下文/当前主体ID");
    上下文.当前分身ID = 私有_读取U64文本特征(
        自我对象,
        私有_管理特征_当前分身ID(),
        "任务管理任务模块::读取任务管理上下文/当前分身ID");
    上下文.当前方法需求位 = 私有_读取I64枚举特征<枚举_任务管理方法需求位>(
        自我对象,
        私有_管理特征_当前方法需求位(),
        "任务管理任务模块::读取任务管理上下文/当前方法需求位");
    上下文.当前本能能力缺口类型 = 私有_读取I64枚举特征<枚举_任务管理本能能力缺口类型>(
        自我对象,
        私有_管理特征_当前本能能力缺口类型(),
        "任务管理任务模块::读取任务管理上下文/当前本能能力缺口类型");
    上下文.当前输入条件主键 = 私有_读取反馈摘要特征(
        自我对象,
        私有_管理特征_当前输入条件主键(),
        "任务管理任务模块::读取任务管理上下文/当前输入条件主键");
    上下文.当前治理态型 = 私有_读取反馈摘要特征(
        自我对象,
        私有_管理特征_当前治理态型(),
    ...
    结构_任务管理本能能力缺口面* 输出) noexcept
{
    ...
    // 六原语描述的是当前工程具备的最小动作能力，不以本轮是否已产出证据为准。
    输出->已具备确保单承载存在 = true;
    输出->已具备读取单特征当前值 = true;
    输出->已具备写入单特征当前值 = true;
    输出->已具备写入单挂点指针 = true;
    ...
    摘要 << "需求位=" << 私有_方法需求位文本(需求位特征.当前需求位)
        ...
        << " | 原语=" << (输出->原语账本摘要.empty() ? "空" : 输出->原语账本摘要)
        << " | 派生=" << (输出->派生规则摘要.empty() ? "空" : 输出->派生规则摘要);
    输出->缺口摘要 = 摘要.str();
    return true;
}
    ...
    结构_任务管理结果* 输出) noexcept
{
    ...
    if (!已受上位输入约束) {
      ...
      if (结果.已确保任务管理主体虚拟存在) {
          ...
          结果.当前分身.当前主方法需求位 = 结果.当前需求位特征.当前需求位;
    ...
    auto* 学习子任务 = 私有_创建或复用学习子任务头(
        ...
        调用点 + "/学习子任务");
    const bool 本能补齐学习 =
        私有_本能缺口属于原语或治理补齐(私有_P0_当前本能缺口(结果));
    auto* 学习方法 = 本能补齐学习
        ? nullptr
        : 私有_写回学习方法骨架(
            自我对象,
    ...
    if ((结果.已创建学习子任务 || 结果.已复用学习子任务) && 本能补齐学习) {
        结果.最近反馈摘要 += "；当前学习通道=本能补齐/治理本能组合";
    }
    if (结果.已创建学习方法头) {
