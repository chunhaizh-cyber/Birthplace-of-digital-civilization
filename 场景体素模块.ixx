...
namespace detail {
    inline constexpr I64 结构化缺口大类_未定义 = 0;
    inline constexpr I64 结构化缺口大类_坐标基准缺口 = 4;
    inline constexpr I64 结构化缺口大类_场景体素先验缺口 = 7;

    inline constexpr I64 结构化阻断阶段_未定义 = 0;
    inline constexpr I64 结构化阻断阶段_坐标基准收束 = 3;
    inline constexpr I64 结构化阻断阶段_先验融合 = 7;
    inline constexpr I64 结构化阻断阶段_体素融合 = 12;

    inline constexpr I64 需求化建议_未评估 = 0;
    inline constexpr I64 需求化建议_等待复验 = 11;
    inline constexpr I64 需求化建议_转观察验证闭环 = 7;
    ...
    // 功能：把快照生成时间转换为当前年龄毫秒，只作为统计材料。
    inline I64 私有_快照年龄毫秒(时间戳 生成时间) noexcept
    {
        if (生成时间 <= 0) return 0;
        const auto 当前时间 = 结构体_时间戳::当前_微秒();
        if (当前时间 <= 生成时间) return 0;
        const auto 年龄微秒 = 当前时间 - 生成时间;
        return 年龄微秒 / 1000;
    }

    // 功能：判断场景体素缺口是否属于第一版快照严重缺口统计材料。
    inline bool 私有_场景体素严重缺口(枚举_场景体素缺口类型 类型) noexcept
    {
        switch (类型) {
        case 枚举_场景体素缺口类型::缺体素特征类型:
        case 枚举_场景体素缺口类型::缺存在场景绝对坐标:
        case 枚举_场景体素缺口类型::缺三维体素特征:
        case 枚举_场景体素缺口类型::缺三维体素句柄:
        case 枚举_场景体素缺口类型::缺坐标系版本:
        case 枚举_场景体素缺口类型::缺稳定体素版本:
        case 枚举_场景体素缺口类型::缺存在位姿版本:
        case 枚举_场景体素缺口类型::缺场景边界:
        case 枚举_场景体素缺口类型::坐标系版本失配:
        case 枚举_场景体素缺口类型::来源场景版本失配:
        case 枚举_场景体素缺口类型::解码超过预算:
        case 枚举_场景体素缺口类型::先验图层生成失败:
            return true;
        default:
            return false;
        }
...
// 功能：把场景体素缺口类型映射为 D5 结构化缺口大类 I64 值域。
I64 场景体素缺口_结构化缺口大类(枚举_场景体素缺口类型 类型) noexcept
{
    switch (类型) {
    case 枚举_场景体素缺口类型::无:
        return detail::结构化缺口大类_未定义;
    case 枚举_场景体素缺口类型::缺存在场景绝对坐标:
    case 枚举_场景体素缺口类型::缺坐标系版本:
    case 枚举_场景体素缺口类型::缺内部子体素局部原点:
    case 枚举_场景体素缺口类型::缺存在位姿版本:
    case 枚举_场景体素缺口类型::坐标系版本失配:
    case 枚举_场景体素缺口类型::来源场景版本失配:
        return detail::结构化缺口大类_坐标基准缺口;
    default:
        return detail::结构化缺口大类_场景体素先验缺口;
    }
}

// 功能：把场景体素缺口类型映射为 D5 结构化阻断阶段 I64 值域。
I64 场景体素缺口_结构化阻断阶段(枚举_场景体素缺口类型 类型) noexcept
{
    switch (类型) {
    case 枚举_场景体素缺口类型::无:
        return detail::结构化阻断阶段_未定义;
    case 枚举_场景体素缺口类型::缺存在场景绝对坐标:
    case 枚举_场景体素缺口类型::缺坐标系版本:
    case 枚举_场景体素缺口类型::缺内部子体素局部原点:
    case 枚举_场景体素缺口类型::缺存在位姿版本:
    case 枚举_场景体素缺口类型::坐标系版本失配:
    case 枚举_场景体素缺口类型::来源场景版本失配:
        return detail::结构化阻断阶段_坐标基准收束;
    case 枚举_场景体素缺口类型::缺体素特征类型:
    case 枚举_场景体素缺口类型::缺三维体素特征:
    case 枚举_场景体素缺口类型::缺三维体素句柄:
    case 枚举_场景体素缺口类型::缺稳定体素版本:
    case 枚举_场景体素缺口类型::缺体素块索引:
    case 枚举_场景体素缺口类型::解码超过预算:
        return detail::结构化阻断阶段_体素融合;
    default:
        return detail::结构化阻断阶段_先验融合;
    }
}

// 功能：把场景体素缺口类型映射为 D5 需求化处理建议 I64 值域，不直接创建需求。
I64 场景体素缺口_需求化处理建议(枚举_场景体素缺口类型 类型) noexcept
{
    switch (类型) {
    case 枚举_场景体素缺口类型::无:
        return detail::需求化建议_未评估;
    case 枚举_场景体素缺口类型::缺存在场景绝对坐标:
    case 枚举_场景体素缺口类型::缺坐标系版本:
    case 枚举_场景体素缺口类型::缺内部子体素局部原点:
    case 枚举_场景体素缺口类型::缺存在位姿版本:
    case 枚举_场景体素缺口类型::坐标系版本失配:
    case 枚举_场景体素缺口类型::来源场景版本失配:
        return detail::需求化建议_转观察验证闭环;
    default:
        return detail::需求化建议_等待复验;
    }
}
...
public:
        ...
        const 结构_场景体素同步参数& 参数)
    {
        ...
        auto 统计入选存在项 = [&](const 结构_场景体素存在项& 项) noexcept {
            if (项.场景AABB_mm.有效()) {
                递增I64计数(快照.统计.有场景AABB数量);
            }
            if (项.颜色事实状态 == static_cast<I64>(枚举_场景体素颜色事实状态::已读取父基础颜色事实)
                || 项.颜色事实状态 == static_cast<I64>(枚举_场景体素颜色事实状态::已读取子平均颜色事实)) {
                递增I64计数(快照.统计.有颜色事实数量);
            }
            if (项.显示角色 == static_cast<I64>(枚举_场景体素显示角色::内部世界子存在体素)) {
                递增I64计数(快照.统计.有内部世界子体素数量);
            }
        };
        ...
        if (!快照.先验感兴趣区域集合句柄.有效()) {
            追加缺口(枚举_场景体素缺口类型::缺先验投影索引, nullptr, nullptr);
        }
        if (!快照.预计深度范围图句柄.有效()) {
            追加缺口(枚举_场景体素缺口类型::缺预计深度范围图, nullptr, nullptr);
        }
        if (!快照.预计遮挡区域句柄.有效()) {
            追加缺口(枚举_场景体素缺口类型::缺可见性预计算, nullptr, nullptr);
        }
        if (!快照.已知空闲区句柄.有效()) {
            追加缺口(枚举_场景体素缺口类型::缺已知空闲区, nullptr, nullptr);
        }
        if (!快照.未知区句柄.有效()) {
            追加缺口(枚举_场景体素缺口类型::缺未知区, nullptr, nullptr);
        }
        if (!快照.自由空间冲突候选区句柄.有效()) {
            追加缺口(枚举_场景体素缺口类型::缺已知空闲区, nullptr, nullptr);
        }
        if (!快照.可见性预计算结果句柄.有效()) {
            追加缺口(枚举_场景体素缺口类型::缺可见性预计算, nullptr, nullptr);
        ...
        快照.统计.缺口数量 = static_cast<I64>(std::min<std::size_t>(
            快照.缺口集合.size(),
            static_cast<std::size_t>((std::numeric_limits<I64>::max)())));
        for (const auto& 缺口 : 快照.缺口集合) {
            if (detail::私有_场景体素严重缺口(缺口.类型)) {
                递增I64计数(快照.统计.严重缺口数量);
            }
        }
        快照.统计.快照年龄毫秒 = detail::私有_快照年龄毫秒(快照.生成时间);
        std::lock_guard<std::mutex> 锁(快照互斥_);
        if (快照.版本 >= 最新快照_.版本) {
            最新快照_ = std::move(快照);
        }
