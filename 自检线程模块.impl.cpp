...
namespace {
    ...
    const 语素入口节点类* 私有_特征_方法成熟度阶段_自检() noexcept
    {
        static const 语素入口节点类* s_词 =
            语素集.添加信息入口词("方法成熟度阶段", 枚举_信息入口类型::特征模板入口);
        return s_词;
    }

    constexpr I64 私有_方法成熟度_好用_自检() noexcept
    {
        return 3;
    }
    ...
    {
        I64 阶段 = 方法 ? 方法->主信息.首节点信息().成熟度阶段 : 0;
        if (方法虚拟存在
            && 世界树.读取特征_I64(
        ...
        时间戳 now) noexcept
    {
        ...
        auto* 特征类型 = 私有_特征_方法成熟度阶段_自检();
        if (!自检场景 || !特征类型) {
            return;
        }

        const auto 状态时间 = now != 0 ? now : 结构体_时间戳::当前_微秒();
        const I64 当前阶段 = 当前成熟度阶段覆盖 >= 0
            ? 当前成熟度阶段覆盖
            : 私有_读取方法成熟度阶段_自检(方法, 方法虚拟存在);
        auto* 当前状态 = 自我.创建自检I64原子状态(
            自检场景,
        ...
        候选.原子最初状态指针 = reinterpret_cast<std::uintptr_t>(当前状态);
        候选.原子最初状态主键 = 当前状态->获取主键();
        候选.原子当前状态指针 = reinterpret_cast<std::uintptr_t>(当前状态);
        候选.原子当前状态主键 = 当前状态->获取主键();
        ...
        时间戳 now = 0) noexcept
    {
        ...
        候选.证据摘要 = 观察摘要;
        if (方法) {
            私有_填充方法好用原子目标(候选, 方法, 当前成熟度阶段覆盖, now);
        }
