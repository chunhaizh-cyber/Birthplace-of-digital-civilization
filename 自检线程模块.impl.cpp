...
namespace {
    ...
    bool 私有_提交派生需求(
        const 结构_任务管理派生需求消息& 派生,
        const std::string& 调用点) noexcept
    {
        if (派生.父需求指针 == 0 && 派生.父需求主键.empty()) {
            项目自检无上级需求日志(
                调用点
                + " | 自检发现缺口未形成正式需求：缺少上级需求 | "
                + 构造任务管理派生需求消息文本(派生));
            return false;
        }
...
bool 自检线程类::启动(const std::string& 调用点)
{
    {
...
void 自检线程类::请求停止(const std::string& 调用点)
{
    {
        std::lock_guard<std::mutex> 锁(状态锁_);
...
void 自检线程类::等待停止(const std::string&)
{
    if (工作线程_.joinable()) {
        工作线程_.join();
    }
...
void 自检线程类::请求休眠期自检(const std::string& 原因)
{
    休眠期自检请求_.store(true);
    {
        std::lock_guard<std::mutex> 锁(状态锁_);
        记录事件_已加锁(结构体_时间戳::当前_微秒(), "休眠期自检请求 | 原因=" + 原因);
    }
    唤醒条件_.notify_all();
}
    ...
    if (本轮提交 < 配置_.每轮提交上限
        ...
        for (auto* 方法 : 私有_枚举方法首节点(配置_.单类扫描上限, &方法扫描游标_)) {
            ...
            const auto 派生 = 私有_构造自检派生需求(
                ...
                false,
                now);
            if (私有_提交派生需求(派生, "自检线程/运行中/方法稳定性")) {
                ++本轮提交;
                std::lock_guard<std::mutex> 锁(状态锁_);
                最近模式_ = 枚举_自检线程模式::运行中;
                ++累计提交需求数_;
    ...
    if (本轮提交 < 配置_.每轮提交上限
        ...
        for (auto* 方法 : 私有_枚举方法首节点(配置_.单类扫描上限, &方法扫描游标_)) {
            ...
            const auto 派生 = 私有_构造自检派生需求(
                ...
                false,
                now);
            if (私有_提交派生需求(派生, "自检线程/运行中/方法运行账")) {
                ++本轮提交;
                std::lock_guard<std::mutex> 锁(状态锁_);
                最近模式_ = 枚举_自检线程模式::运行中;
                ++累计提交需求数_;
...
void 自检线程类::执行休眠期自检_(时间戳 now)
{
    ...
    for (auto* 方法 : 私有_枚举方法首节点(配置_.单类扫描上限, &休眠方法扫描游标_)) {
        ...
        const auto 派生 = 私有_构造自检派生需求(
            ...
            false,
            now);
        if (私有_提交派生需求(派生, "自检线程/休眠期/方法虚拟存在")) {
            ++本轮提交;
            std::lock_guard<std::mutex> 锁(状态锁_);
            最近模式_ = 枚举_自检线程模式::休眠期;
            ++累计提交需求数_;
        ...
        if (自我.读取自我I64特征当前值(
            ...
            if (!私有_是否去重抑制中(去重账_, 去重键, now, 抑制微秒)) {
                ...
                const auto 派生 = 私有_构造自检派生需求(
                    ...
                    false,
                    now);
                if (私有_提交派生需求(派生, "自检线程/休眠期/可用方法数量镜像")) {
                    ++本轮提交;
                    std::lock_guard<std::mutex> 锁(状态锁_);
                    最近模式_ = 枚举_自检线程模式::休眠期;
                    ++累计提交需求数_;
...
bool 启动自检线程(const std::string& 调用点)
{
    return 获取全局自检线程().启动(调用点);
}

void 停止自检线程(const std::string& 调用点)
{
    auto& 线程 = 获取全局自检线程();
    线程.请求停止(调用点);
    线程.等待停止(调用点);
...
void 请求休眠期自检(const std::string& 原因)
{
    获取全局自检线程().请求休眠期自检(原因);
}
