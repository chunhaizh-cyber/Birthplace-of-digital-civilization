...
namespace {
    ...
    bool 私有_任务仍在处理缺口(const 任务节点* 任务) noexcept
    {
        ...
        if (任务类::读取任务当前阶段(任务) == 枚举_任务阶段::已完成) {
            return false;
        }
        switch (任务类::读取任务状态(任务)) {
        case 枚举_任务状态::完成:
        case 枚举_任务状态::失败:
        case 枚举_任务状态::取消:
        case 枚举_任务状态::超时:
        ...
        const 方法节点* 方法) noexcept
    {
        ...
        const bool 练习任务 =
            任务类::读取任务选中自我能力值(任务) == static_cast<I64>(枚举_本能方法ID::自我_练习)
            || 任务类::读取任务等待缺口唤醒(任务);
        const auto 当前阶段 = 任务类::读取任务当前阶段(任务);
        const bool 结构缺口任务 =
            当前阶段 == 枚举_任务阶段::筹办
            || 当前阶段 == 枚举_任务阶段::执行
            || 当前阶段 == 枚举_任务阶段::阻塞
            || 任务类::读取任务状态(任务) == 枚举_任务状态::筹办中
            || 任务类::读取任务状态(任务) == 枚举_任务状态::执行中
            || 任务类::读取任务状态(任务) == 枚举_任务状态::等待中;
        return 练习任务 || 结构缺口任务;
    }

    bool 私有_需求命中方法结构补齐积压(
        const 需求节点* 需求,
        const 方法节点* 方法) noexcept
    {
        if (!需求 || !方法) {
        ...
        return !需求->主信息.派生来源方法主键.empty()
            && 需求->主信息.派生来源方法主键 == 方法->获取主键();
    }

    bool 私有_存在方法结构补齐积压(const 方法节点* 方法) noexcept
    {
        if (!方法) {
            return false;
        }
        ...
        if (需求根节点) {
            ...
            for (auto* 当前 = reinterpret_cast<需求节点*>(需求根节点->链下);
                 当前 && 当前 != 需求根节点 && 需求保护 < 4096;
                 当前 = reinterpret_cast<需求节点*>(当前->链下), ++需求保护) {
                if (私有_需求命中方法结构补齐积压(当前, 方法)) {
                    return true;
                }
            }
        }
