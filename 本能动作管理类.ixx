...
inline constexpr std::int64_t 本能动作错误_缺方法首节点 = -20005;
...
inline const char* 本能动作运行结果语义_按码(std::int64_t 运行码) noexcept
{
    ...
    case 本能动作错误_缺方法首节点: return "方法首节点缺失";
    ...
    时间戳 now = 结构体_时间戳::当前_微秒()) noexcept
{
    ...
    auto* 抽象特征 = 取或创建本能动作运行结果抽象特征(语义);
    if (!宿主 || !抽象特征) {
        return nullptr;
    }
    auto* 实例 = 世界树.取或创建实例特征_按抽象特征(宿主, 抽象特征);
    if (!实例) {
        return nullptr;
    }
    (void)世界树.写入特征_I64(实例, static_cast<I64>(运行码), now);
    (void)世界树.解析实例特征命中抽象特征(实例);
    return 实例;
}

特征节点类* 创建本能动作运行结果实例_按码(
    方法类::节点类* 方法首节点,
    场景节点类* 输出结果场景,
    std::int64_t 运行码,
    时间戳 now = 结构体_时间戳::当前_微秒()) noexcept
...
std::int64_t 本能动作运行结果值(const 特征节点类* 运行结果实例) noexcept
{
    if (!运行结果实例) {
        return 本能动作运行结果_未知;
    }
    I64 输出值 = 本能动作运行结果_未知;
    return 世界树.读取特征_I64(运行结果实例, 输出值)
        ? static_cast<std::int64_t>(输出值)
        : 本能动作运行结果_未知;
}

bool 本能动作运行成功(const 特征节点类* 运行结果实例) noexcept
{
    return 本能动作运行结果值(运行结果实例) == 本能动作运行结果_成功;
}

std::string 本能动作运行结果文本(const 特征节点类* 运行结果实例)
{
    const auto* 主信息 = 运行结果实例 ? 世界树.特征().取特征主信息(运行结果实例) : nullptr;
    return 主信息 && 主信息->类型 ? 语素集.获取词(主信息->类型) : std::string("运行结果未知");
}
...
public:
        ...
        场景节点类* 输出结果场景 = nullptr) const noexcept
    {
        ...
        if (动作句柄.有效()) {
            ...
            const auto 解析ID = 查询ID(方法->主信息.动作名->获取主键());
            if (解析ID == 枚举_本能动作ID::未定义 || 解析ID != 动作ID) {
                return 设置结果(本能动作错误_缺动作入口);
            }
        }

        auto* 条件场景 = 方法->主信息.条件场景.指针;
        const bool 需要输入参数 = 条件场景 || !方法->主信息.条件判定索引.empty();
        if (!需要输入参数) {
