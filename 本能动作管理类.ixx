...
namespace 本能动作管理模块_detail {
        ...
        const std::string& 调用点) noexcept
    {
        ...
        (void)写I64(方法, 特征类型, 当前值 + 增量, now, 调用点);
    }
        ...
        const std::string& 调用点) noexcept
    {
        ...
        (void)写I64(方法, 特征_运行阶段(), static_cast<I64>(阶段), now, 调用点 + "/运行阶段");
        if (const auto* 状态特征 = 阶段状态特征(阶段)) {
            (void)写I64(方法, 状态特征, static_cast<I64>(状态), now, 调用点 + "/阶段状态");
        }
        if (const auto* 耗时特征 = 阶段耗时特征(阶段)) {
            const auto 耗时 = 开始 != 0 && now >= 开始 ? static_cast<I64>(now - 开始) : 0;
            (void)写I64(方法, 耗时特征, 耗时, now, 调用点 + "/阶段耗时");
        }
    }
    ...
    inline std::string 条件方向签名(
        const 结构_本能动作执行上下文& 上下文,
        const 结构_本能动作步骤结果& 结果) noexcept
    {
        try {
            std::string 输出 = "本能条件方向|动作=" + std::to_string(static_cast<std::uint32_t>(结果.动作ID));
            auto* 条件场景 = 上下文.输入场景 ? 上下文.输入场景 : 上下文.场景;
            if (条件场景) {
                const auto 条件特征 = 世界树.二次特征().获取子二次特征(
                    reinterpret_cast<const 基础信息节点类*>(条件场景));
                if (!条件特征.empty()) {
                    输出 += "|特征=";
                    std::size_t 已写 = 0;
                    for (auto* 特征 : 条件特征) {
                        ...
                        const auto* 主信息 = 世界树.二次特征().取二次特征主信息(特征);
                        const auto 片段 = 主信息 && !主信息->度量签名_链键.empty()
                            ? 主信息->度量签名_链键
                            : 节点主键(reinterpret_cast<const 基础信息节点类*>(特征));
                        if (片段.empty()) {
                            continue;
                        }
                        if (已写++ > 0) {
                    ...
                    if (已写 > 0) {
                        return 方向安全片段(std::move(输出));
                    }
                }
                const auto 场景键 = 节点主键(reinterpret_cast<const 基础信息节点类*>(条件场景));
                if (!场景键.empty()) {
                    输出 += "|场景=" + 场景键;
                    return 方向安全片段(std::move(输出));
                }
            ...
            if (上下文.目标存在) {
                输出 += "|目标=" + 节点主键(reinterpret_cast<const 基础信息节点类*>(上下文.目标存在));
            } else if (上下文.任务节点 || 上下文.任务) {
                输出 += "|任务=" + 指针键(上下文.任务节点 ? 上下文.任务节点 : 上下文.任务);
            } else {
                输出 += "|空条件";
            }
            return 方向安全片段(std::move(输出));
    ...
    inline std::string 结果方向签名(const 结构_本能动作步骤结果& 结果) noexcept
    {
        try {
            std::string 输出 = "本能结果方向|动作=" + std::to_string(static_cast<std::uint32_t>(结果.动作ID));
            if (!结果.需求方向说明.empty()) {
                输出 += "|方向=" + 结果.需求方向说明;
            } else if (结果.需求方向) {
                输出 += "|方向=" + 二次特征方向文本(结果.需求方向);
            } else if (结果.需求当前状态 || 结果.需求目标状态) {
                输出 += "|状态="
                    + 节点主键(reinterpret_cast<const 基础信息节点类*>(结果.需求当前状态))
                    + "->"
                    + 节点主键(reinterpret_cast<const 基础信息节点类*>(结果.需求目标状态));
            } else {
                输出 += "|成功码=" + std::to_string(static_cast<long long>(结果.成功码));
            }
            return 方向安全片段(std::move(输出));
        }
        catch (...) {
        ...
        const std::string& 调用点) noexcept
    {
        ...
        auto* 方法虚拟存在 = 结果.方法虚拟存在
            ? 结果.方法虚拟存在
            : 确保方法虚拟存在(方法, 结束, 调用点 + "/虚拟存在");
        if (!方法虚拟存在) {
            return;
        }

        const auto 条件方向 = 条件方向签名(上下文, 结果);
        const auto 结果方向 = 结果方向签名(结果);
        auto* 条件场景 = 取或创建方法样本场景(方法虚拟存在, "本能执行条件样本", 条件方向);
        auto* 结果场景 = 取或创建方法样本场景(方法虚拟存在, "本能执行结果样本", 结果方向);

        auto* 条件特征 = 取或创建方向二次特征(
            条件场景,
            二次特征主信息类::枚举_形态::条件,
            条件方向,
            "本能执行条件方向",
        ...
        if (条件特征) {
            条件索引.push_back(引用(条件特征));
        }
        std::vector<可解析引用<二次特征节点类>> 结果索引{};
        if (结果特征) {
            结果索引.push_back(引用(结果特征));
        }
        ...
        if (条件节点) {
            条件节点->主信息.最近执行时间 = 开始;
            条件节点->主信息.最近完成时间 = 结束;
            追加状态引用(条件节点->主信息.结果初始状态, 结果.需求当前状态);
        }
        if (结果节点) {
            结果节点->主信息.最近执行时间 = 开始;
            结果节点->主信息.最近完成时间 = 结束;
            追加状态引用(结果节点->主信息.结果初始状态, 结果.需求目标状态);
        }
    }
    ...
    inline void 准备运行记账(
        ...
        const std::string& 调用点) noexcept
    {
        auto* 方法虚拟存在 = 确保方法虚拟存在(方法, 开始, 调用点);
        if (方法 && !上下文.输出场景) {
            上下文.输出场景 = 方法类::取或创建_方法结果场景(
                方法,
                nullptr,
                开始,
                调用点 + "/结果场景");
        ...
        if (!上下文.输入场景) {
            上下文.输入场景 = 上下文.场景 ? 上下文.场景 : 上下文.输出场景;
        }
        if (!上下文.输出场景) {
            上下文.输出场景 = 上下文.场景 ? 上下文.场景 : 上下文.输入场景;
        }
        if (上下文.请求ID == 0) {
            上下文.请求ID = 下一个请求ID();
        }
        ...
        增量I64(方法, 特征_运行次数(), 1, 开始, 调用点 + "/运行次数");
        (void)写I64(方法, 特征_运行状态(), static_cast<I64>(枚举_本能运行状态::运行中), 开始, 调用点 + "/运行状态");
        (void)写I64(方法, 特征_运行阶段(), static_cast<I64>(枚举_本能运行阶段::确保), 开始, 调用点 + "/运行阶段");
        (void)写I64(方法, 特征_最近请求ID(), static_cast<I64>(上下文.请求ID), 开始, 调用点 + "/请求ID");
        (void)写I64(方法, 特征_最近开始时间(), static_cast<I64>(开始), 开始, 调用点 + "/开始时间");
        (void)写指针(方法, 特征_最近结果场景(), 上下文.输出场景, 开始, 调用点 + "/结果场景");
        (void)写指针(方法, 特征_最近输入场景(), 上下文.输入场景, 开始, 调用点 + "/输入场景");
        (void)写指针(方法, 特征_最近目标存在(), 上下文.目标存在, 开始, 调用点 + "/目标存在");
        (void)写指针(方法, 特征_最近宿主任务(), 上下文.任务节点 ? 上下文.任务节点 : 上下文.任务, 开始, 调用点 + "/宿主任务");
        记录阶段(方法, 枚举_本能运行阶段::确保, 枚举_本能运行状态::成功, 开始, 结构体_时间戳::当前_微秒(), 调用点 + "/确保");
        记录阶段(方法, 枚举_本能运行阶段::解析绑定, 枚举_本能运行状态::成功, 结构体_时间戳::当前_微秒(), 结构体_时间戳::当前_微秒(), 调用点 + "/解析绑定");
        记录阶段(方法, 枚举_本能运行阶段::执行, 枚举_本能运行状态::运行中, 结构体_时间戳::当前_微秒(), 0, 调用点 + "/执行开始");
    }
        ...
        const std::string& 调用点) noexcept
    {
        ...
        auto* 方法虚拟存在 = 确保方法虚拟存在(方法, 结束, 调用点);
        结果.方法虚拟存在 = 方法虚拟存在;
        记录功能结果到结果场景(上下文, 结果, 结束);
        沉淀本能执行样本到方法树(方法, 上下文, 结果, 开始, 结束, 调用点 + "/方法树样本");
        if (!方法 || !方法虚拟存在) {
            return;
        }

        const auto 总耗时 = 开始 != 0 && 结束 >= 开始 ? static_cast<I64>(结束 - 开始) : 0;
        const bool 成功 = 结束状态 == 枚举_本能运行状态::成功;
        结果.成功 = 成功;
        结果.置信度 = 成功 ? 10000 : 0;
        记录阶段(
            方法,
            枚举_本能运行阶段::执行,
            成功 ? 枚举_本能运行状态::成功 : 结束状态,
            开始,
            结束,
            调用点 + "/执行完成");
        记录阶段(
        ...
        (void)写I64(方法, 特征_运行状态(), static_cast<I64>(结束状态), 结束, 调用点 + "/运行状态");
        (void)写I64(方法, 特征_运行阶段(), static_cast<I64>(枚举_本能运行阶段::完成), 结束, 调用点 + "/运行阶段");
        (void)写I64(方法, 特征_最近结束时间(), static_cast<I64>(结束), 结束, 调用点 + "/结束时间");
        (void)写I64(方法, 特征_最近耗时(), 总耗时, 结束, 调用点 + "/耗时");
        (void)写I64(方法, 特征_最近错误码(), static_cast<I64>(结果.成功码), 结束, 调用点 + "/错误码");
        (void)写I64(方法, 特征_最近调用桥接码(), static_cast<I64>(结果.成功码), 结束, 调用点 + "/桥接码");
        (void)写I64(方法, 特征_最近质量(), static_cast<I64>(结果.质量_Q10000), 结束, 调用点 + "/质量");
        (void)写I64(方法, 特征_最近成本Q(), static_cast<I64>(结果.成本_Q10000), 结束, 调用点 + "/成本Q");
        (void)写I64(方法, 特征_最近成本Us(), static_cast<I64>(结果.成本_us), 结束, 调用点 + "/成本us");
        (void)写I64(方法, 特征_最近失败阶段(), static_cast<I64>(成功 ? 枚举_本能运行阶段::未定义 : 失败阶段), 结束, 调用点 + "/失败阶段");
        (void)写指针(方法, 特征_最近结果场景(), 结果.结果存在集场景, 结束, 调用点 + "/结果场景");
        增量I64(方法, 特征_累计成本Us(), static_cast<I64>(结果.成本_us), 结束, 调用点 + "/累计成本");
        if (成功) {
            增量I64(方法, 特征_成功次数(), 1, 结束, 调用点 + "/成功次数");
            增量I64(方法, 特征_累计成功耗时Us(), 总耗时, 结束, 调用点 + "/累计成功耗时");
            (void)写I64(方法, 特征_连续失败次数(), 0, 结束, 调用点 + "/连续失败次数");
            (void)写I64(方法, 特征_最近成功时间(), static_cast<I64>(结束), 结束, 调用点 + "/最近成功时间");
        } else {
            增量I64(方法, 特征_失败次数(), 1, 结束, 调用点 + "/失败次数");
            增量I64(方法, 特征_连续失败次数(), 1, 结束, 调用点 + "/连续失败次数");
            (void)写I64(方法, 特征_最近失败时间(), static_cast<I64>(结束), 结束, 调用点 + "/最近失败时间");
        }
        if (结束状态 == 枚举_本能运行状态::异常) {
            增量I64(方法, 特征_异常次数(), 1, 结束, 调用点 + "/异常次数");
        }
        if (结果.成功码 == 本能方法结果_任务缺条件需自我线程) {
            增量I64(方法, 特征_缺条件结束次数(), 1, 结束, 调用点 + "/缺条件次数");
        }
        if (结果.成功码 == 本能方法结果_任务缺方法需任务管理任务
            || 结果.成功码 == 本能方法结果_任务筹办已激活学习任务
            || 结果.成功码 == 本能方法结果_任务执行一步已激活学习任务) {
            增量I64(方法, 特征_缺方法结束次数(), 1, 结束, 调用点 + "/缺方法次数");
        }
        if (结果.结果存在集场景) {
            增量I64(方法, 特征_结果场景变更次数(), 1, 结束, 调用点 + "/结果场景变更");
        }
    }
}
...
public:
        ...
        结构_本能动作步骤结果* 输出 = nullptr) const noexcept
    {
        ...
        if (!已注册动作.has_value() || !已注册动作->调用点) {
            ...
            结果.成功码 = 本能动作错误_未注册;
            结果.备注 = "本能动作未注册";
            本能动作管理模块_detail::完成运行记账(
                方法,
                上下文,
                结果,
                开始,
                本能动作管理模块_detail::枚举_本能运行状态::失败,
                本能动作管理模块_detail::枚举_本能运行阶段::确保,
                "本能动作管理类::运行/未注册");
            if (输出) {
                *输出 = std::move(结果);
            }
        ...
        try {
            ...
            本能动作管理模块_detail::完成运行记账(
                ...
                开始,
                结果.成功码 == 0
                    ? 本能动作管理模块_detail::枚举_本能运行状态::成功
                    : 本能动作管理模块_detail::枚举_本能运行状态::失败,
                本能动作管理模块_detail::枚举_本能运行阶段::执行,
                "本能动作管理类::运行/完成");
