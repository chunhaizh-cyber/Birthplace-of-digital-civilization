    ...
    const 场景节点类* 显式输出结果场景 = nullptr) noexcept
{
    ...
    视图.方法执行成功 =
        本能动作语素入口同一(视图.方法执行情况, 本能动作执行成功词())
        || (!视图.方法执行情况已读
            && 视图.调试运行码已读
...
public:
        ...
        场景节点类* 输出结果场景 = nullptr) const noexcept
    {
        ...
        auto 记录6150检查失败 = [&](
            ...
            if (动作ID != 枚举_本能动作ID::提交_当前观察范围可观测单位存在对应事实) {
                return;
            }
