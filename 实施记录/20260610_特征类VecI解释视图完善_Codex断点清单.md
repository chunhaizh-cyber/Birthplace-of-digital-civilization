...
状态：P0-P6 已闭合；P7 已接入识别 / 当前帧分区 raw 簇 VecI64 轮廓评分，扫描 / 跟踪存在-存在轮廓比较和二次特征写入；P8 已完成差异检查、禁止字段扫描、Debug x64 构建和真实自检窗口。普通自检未自然命中 D455 识别 / 扫描 / 跟踪轮廓比较样本，真实业务命中待自然样本。
```
...
预计验证：
    ...
    Debug x64 构建通过。
    普通自检窗口退出码 0；若没有自然 D455 轮廓样本，只能证明路径可构建和无新增运行错误，不能宣称识别 / 扫描 / 跟踪真实业务通过。
禁止宣称：
...
预期：
    退出码为 0。
    若普通自检窗口没有自然形成 D455 已验证观察像素簇，本轮不会命中 `外设观察/VecI64轮廓写入汇总`。
    未命中真实轮廓样本时，只能证明程序真实运行窗口不崩溃，P6 行为命中仍待真实 D455 样本。
...
真实自检窗口：
    ...
    `外设观察/VecI64轮廓写入汇总` 扫描退出码 1，无命中，符合普通自检窗口未自然形成 D455 已验证观察像素簇的预测。
    错误 / 崩溃 / 异常关键字扫描退出码 1，无命中。

当前判断：
    P6 代码接入、静态验证、构建和真实自检窗口已闭合。
...
关键退出码：
    ...
    真实自检窗口退出码 0，运行日志 `日志/鱼巢_run_20260611_VecI_P7_selfcheck.log`。

关键日志证据：
    `VecI64轮廓比较特征数` / `VecI64轮廓二次特征写入数` 扫描退出码 1，无命中，符合普通自检窗口未自然形成 D455 可比轮廓样本的预测。
...
P5h 完成后进入 P6：
    已完成观察写入接入、差异检查、禁止字段扫描、Debug x64 构建和真实自检窗口。
