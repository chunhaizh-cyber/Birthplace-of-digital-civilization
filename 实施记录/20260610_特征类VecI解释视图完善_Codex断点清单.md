...
切片：P7 识别 / 扫描 / 跟踪读取和比较接入
状态：P0 阻塞已解除；P1/P2/P3/P4 已闭合并提交；P5 已完成总验收；P6 已把 D455 已验证观察像素簇的平面轮廓 / 空间极值轮廓写入接入 VecI64 包装和 P4 校验，并通过 Debug x64 构建与真实自检窗口。普通自检未自然命中 D455 已验证簇，P6 行为命中仍待真实 D455 样本；当前进入 P7。
```
...
P7 仍不得改 `特征值主信息类`，不得新增 VecI64 底层字段，不得拆轮廓子链，不得把二次比较结果写成一阶轮廓特征。
```
...
```text
不得宣称 P7 已实现。
不得把 P6 未命中真实 D455 轮廓样本宣称为观察链业务通过。
