...
切片：P6 观察写入接入
状态：P0 阻塞已解除；P1/P2/P3/P4 已闭合并提交；P5 已完成总验收，`特征值主信息类::比较` 未改变，`特征值主信息类` 无 VecI64 / 编码类型 / 维度 / 轮廓比较字段，P5 各比较和二次特征写入适配入口已覆盖并通过 Debug x64 构建；当前进入 P6 观察写入接入。
```
...
P6 仍不得改 `特征值主信息类`，不得新增 VecI64 底层字段，不得拆轮廓子链。
```
