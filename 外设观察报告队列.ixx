...
namespace {
        ...
        const 结构_外设观察报告队列项& 报告项)
    {
        ...
        外设提交_追加非空句柄(包头.材料句柄集, 报告项.空间候选队列句柄);
        外设提交_追加非空句柄(包头.材料句柄集, 报告项.像素归属账本句柄);
...
std::int64_t 计算观察材料质量收束状态(
    ...
    const 结构_外设观察等待项* 等待项)
{
    constexpr std::int64_t 状态_未评估 = 0;
    constexpr std::int64_t 状态_可用 = 1;
    constexpr std::int64_t 状态_低质量 = 2;
    constexpr std::int64_t 状态_不可用 = 3;
    constexpr std::int64_t 状态_部分可用 = 4;
    constexpr std::int64_t 状态_需复验 = 5;
    ...
    if (判定.等级 == 枚举_观察材料等级::未定义) {
        return 基础质量不足 ? 状态_低质量 : 状态_未评估;
    }
    if (判定.等级 == 枚举_观察材料等级::不合格) {
        return 基础质量不足 ? 状态_低质量 : 状态_不可用;
    }
    if (判定.空间可用状态
    ...
    const 结构_外设观察等待项* 等待项)
{
    ...
    if (质量状态 == 状态_低质量) {
        追加缺口(枚举_外设提交缺口码::低质量);
    } else if (质量状态 == 状态_不可用 && 缺口集合.empty()) {
        追加缺口(枚举_外设提交缺口码::缺材料句柄);
    } else if (质量状态 == 状态_部分可用) {
    ...
    } else if (质量状态 == 状态_需复验 && 缺口集合.empty()) {
        追加缺口(枚举_外设提交缺口码::材料过期);
    }
    ...
    const 结构_外设观察报告队列项& 报告项)
{
    ...
    for (const auto& 簇 : 报告项.观察像素簇集合) {
        ...
        外设提交_追加非空句柄(包.ROI句柄集, 簇.ROI句柄);
        外设提交_追加非空句柄(包.掩码句柄集, 簇.像素集合掩码句柄);
        外设提交_追加非空句柄(包.点集句柄集, 簇.点集句柄);
    ...
    const 结构_外设观察报告队列项& 报告项)
{
    ...
    for (const auto& 簇 : 报告项.观察像素簇集合) {
        ...
        结构_外设已知存在扫描结果 结果{};
        结果.当前ROI句柄 = 簇.ROI句柄;
        结果.当前掩码句柄 = 簇.像素集合掩码句柄;
        结果.当前点集句柄 = 簇.点集句柄;
        结果.当前深度切片句柄 = 簇.深度轮廓局部图句柄;
        结果.当前中心 = 外设提交_簇中心(簇);
