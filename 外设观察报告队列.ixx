...
export enum class 枚举_稳定复现等级状态 : std::int64_t {
    未评估 = 0,
    未复现 = 1,
    弱复现 = 2,
    可复现 = 3,
...
export enum class 枚举_稳定复现阻断原因 : std::int64_t {
    无 = 0,
    缺候选 = 1,
    缺跨帧簇 = 2,
    未匹配 = 4,
...
export struct 结构_共享关联外设材料输入 {
    ...
    std::string 空间候选队列句柄{};
    std::string 像素归属账本句柄{};
    std::vector<std::string> 感兴趣区域句柄集{};
    std::vector<std::string> 点集句柄集{};
    std::vector<std::string> 空间候选句柄集{};
    std::vector<std::string> 彩色轮廓局部图句柄集{};
    std::vector<std::string> 深度轮廓局部图句柄集{};
    std::string 目标约束ROI句柄{};
    std::string 目标约束像素集合掩码句柄{};
    std::string 目标约束点集句柄{};
    std::int64_t 目标约束二维最小X = 0;
    std::int64_t 目标约束二维最大X = 0;
    std::int64_t 目标约束二维最小Y = 0;
    std::int64_t 目标约束二维最大Y = 0;
    std::int64_t 目标约束空间范围最小X = 0;
    std::int64_t 目标约束空间范围最大X = 0;
    std::int64_t 目标约束空间范围最小Y = 0;
    std::int64_t 目标约束空间范围最大Y = 0;
    std::int64_t 目标约束空间范围最小Z = 0;
    std::int64_t 目标约束空间范围最大Z = 0;
    std::int64_t 目标约束最低深度有效率 = 0;
    std::int64_t 目标约束最低空间坐标有效率 = 0;
    std::int64_t 目标约束最低掩码重合率 = 0;
    std::int64_t 目标约束最大预测误差 = 0;
    std::int64_t 目标观察约束可用性状态 = 0;
    std::int64_t 目标约束强匹配条件状态 = 0;
    std::int64_t 目标约束降级匹配条件状态 = 0;
    std::int64_t 目标约束先验可用性收束状态 = 0;
    std::int64_t 材料质量收束状态 = 0;
    枚举_外设材料能力等级 可承接能力等级 = 枚举_外设材料能力等级::未定义;
    std::vector<枚举_外设提交缺口码> 缺口原因{};
};
...
export struct 结构_稳定复现评分 {
    std::int64_t 外设内部簇ID = 0;
    std::int64_t 外设跨帧簇ID = -1;
    std::int64_t 评分Q10000 = 0;
    std::int64_t 状态值 = static_cast<std::int64_t>(枚举_稳定复现等级状态::未评估);
    std::int64_t 阻断原因掩码 = 0;
    std::int64_t 跨帧匹配状态 = 0;
    std::int64_t 跨帧连续命中次数 = 0;
    std::int64_t 跨帧丢失次数 = 0;
    std::int64_t 跨帧复现离散度Q10000 = 0;
    std::int64_t 跨帧连续稳定命中次数 = 0;
    std::int64_t 跨帧连续发布次数 = 0;
};
...
export 结构_共享关联外设材料输入 构造共享关联外设材料输入_由观察报告(
    const 结构_外设观察报告队列项& 报告项,
    const 结构_外设观察等待项* 等待项);
export std::optional<结构_共享关联外设材料输入> 读取共享关联外设材料输入_按报告ID(
    std::uint64_t 报告ID,
    const 结构_外设观察等待项* 等待项);
...
namespace {
    ...
    struct 结构_D455短期材料页保护项 {
        std::uint64_t 报告ID = 0;
        std::int64_t 保护截止毫秒 = 0;
    };
    ...
    struct 结构_外设观察队列共享状态 {
        ...
        std::vector<结构_D455短期材料页保护项> D455材料页保护集合{};
    ...
    // 功能：返回报告对应的 D455 短期材料页来源报告编号。
    std::uint64_t D455_短期材料页来源报告ID(
        const 结构_外设观察报告队列项& 报告项) noexcept
    {
        return 报告项.来源原始报告ID != 0
            ? 报告项.来源原始报告ID
            : 报告项.报告ID;
    }

    void D455_清理短期材料页保护_已加锁(
        结构_外设观察队列共享状态& 状态,
        std::int64_t 当前时间毫秒)
    {
        状态.D455材料页保护集合.erase(
            std::remove_if(
                状态.D455材料页保护集合.begin(),
                状态.D455材料页保护集合.end(),
                [当前时间毫秒](const 结构_D455短期材料页保护项& 项) noexcept {
                    return 项.报告ID == 0
                        || (项.保护截止毫秒 > 0 && 当前时间毫秒 > 项.保护截止毫秒);
                }),
            状态.D455材料页保护集合.end());
    }

    // 功能：判断指定 D455 短期材料页是否仍处于容量清理保护窗口。
    bool D455_材料页受保护_已加锁(
        const 结构_外设观察队列共享状态& 状态,
        std::uint64_t 报告ID,
        std::int64_t 当前时间毫秒) noexcept
    {
        ...
        return std::any_of(
            状态.D455材料页保护集合.begin(),
            状态.D455材料页保护集合.end(),
            [报告ID, 当前时间毫秒](const 结构_D455短期材料页保护项& 项) noexcept {
                return 项.报告ID == 报告ID
                    && (项.保护截止毫秒 <= 0 || 当前时间毫秒 <= 项.保护截止毫秒);
            });
    }
        ...
        std::int64_t 当前时间毫秒)
    {
        ...
        while (!状态.D455材料页队列.empty()) {
            ...
            const bool 受保护 = D455_材料页受保护_已加锁(状态, 页.报告ID, 当前时间毫秒);
            if (!已过期 && !超容量) {
                break;
            }
            if (!已过期 && 受保护) {
...
// 功能：按 D9 跨帧输入材料收束观察候选稳定复现评分，不直接写 C 到 E 的事实裁决。
结构_稳定复现评分 计算观察候选稳定复现评分(
    const 结构_外设观察像素簇摘要& 簇)
{
    auto 限制Q10000 = [](std::int64_t 值) noexcept {
        return (std::min<std::int64_t>)(10000, (std::max<std::int64_t>)(0, 值));
    };
    auto 追加阻断 = [](std::int64_t& 掩码, 枚举_稳定复现阻断原因 原因) noexcept {
        掩码 |= static_cast<std::int64_t>(原因);
    };

    结构_稳定复现评分 输出{};
    输出.外设内部簇ID = 簇.外设内部簇ID;
    输出.外设跨帧簇ID = 簇.外设跨帧簇ID;
    输出.跨帧匹配状态 = 簇.跨帧匹配状态;
    输出.跨帧连续命中次数 = (std::max<std::int64_t>)(0, 簇.跨帧连续命中次数);
    输出.跨帧丢失次数 = (std::max<std::int64_t>)(0, 簇.跨帧丢失次数);
    输出.跨帧复现离散度Q10000 = 限制Q10000(簇.跨帧复现离散度);
    输出.跨帧连续稳定命中次数 = (std::max<std::int64_t>)(0, 簇.跨帧连续稳定命中次数);
    输出.跨帧连续发布次数 = (std::max<std::int64_t>)(0, 簇.跨帧连续发布次数);

    const bool 有候选 = 簇.外设内部簇ID > 0 || 簇.来源空间候选ID > 0;
    const bool 有跨帧簇 = 簇.外设跨帧簇ID >= 0;
    const bool 已匹配 = 簇.跨帧匹配状态 > 0 || 有跨帧簇;
    if (!有候选) {
        追加阻断(输出.阻断原因掩码, 枚举_稳定复现阻断原因::缺候选);
    }
    if (!有跨帧簇) {
        追加阻断(输出.阻断原因掩码, 枚举_稳定复现阻断原因::缺跨帧簇);
    }
    if (!已匹配) {
        追加阻断(输出.阻断原因掩码, 枚举_稳定复现阻断原因::未匹配);
    }
    if (输出.跨帧连续命中次数 <= 0) {
        追加阻断(输出.阻断原因掩码, 枚举_稳定复现阻断原因::连续命中不足);
    }
    if (输出.跨帧丢失次数 > 0 || 簇.轨迹状态 == 枚举_外设Tracker轨迹状态::短时丢失) {
        追加阻断(输出.阻断原因掩码, 枚举_稳定复现阻断原因::丢失中);
    }
    if (输出.跨帧复现离散度Q10000 >= 5000) {
        追加阻断(输出.阻断原因掩码, 枚举_稳定复现阻断原因::复现离散度过高);
    }
    if (输出.跨帧连续稳定命中次数 <= 0 && 簇.跨帧轨迹稳定状态 <= 0) {
        追加阻断(输出.阻断原因掩码, 枚举_稳定复现阻断原因::缺稳定命中);
    }
    if (簇.轨迹状态 == 枚举_外设Tracker轨迹状态::冲突) {
        追加阻断(输出.阻断原因掩码, 枚举_稳定复现阻断原因::轨迹冲突);
    }
    if (簇.轨迹状态 == 枚举_外设Tracker轨迹状态::遮挡) {
        追加阻断(输出.阻断原因掩码, 枚举_稳定复现阻断原因::遮挡保留);
    }

    const auto 连续命中评分 = 限制Q10000(输出.跨帧连续命中次数 * 2500);
    const auto 稳定命中评分 = 限制Q10000((std::max<std::int64_t>)(
        输出.跨帧连续稳定命中次数,
        簇.跨帧轨迹稳定状态) * 2500);
    const auto 发布评分 = 限制Q10000(输出.跨帧连续发布次数 * 2500);
    const auto 离散度评分 = 输出.跨帧复现离散度Q10000 > 0
        ? 限制Q10000(10000 - 输出.跨帧复现离散度Q10000)
        : (已匹配 ? 5000 : 0);
    const auto 丢失扣分 = (std::min<std::int64_t>)(6000, 输出.跨帧丢失次数 * 2000);
    输出.评分Q10000 = 限制Q10000(
        (连续命中评分 + 稳定命中评分 + 发布评分 + 离散度评分) / 4 - 丢失扣分);

    const auto 缺基础掩码 =
        static_cast<std::int64_t>(枚举_稳定复现阻断原因::缺候选)
        | static_cast<std::int64_t>(枚举_稳定复现阻断原因::缺跨帧簇)
        | static_cast<std::int64_t>(枚举_稳定复现阻断原因::未匹配);
    if ((输出.阻断原因掩码 & static_cast<std::int64_t>(枚举_稳定复现阻断原因::轨迹冲突)) != 0) {
        输出.状态值 = static_cast<std::int64_t>(枚举_稳定复现等级状态::冲突复现);
    } else if ((输出.阻断原因掩码 & static_cast<std::int64_t>(枚举_稳定复现阻断原因::遮挡保留)) != 0) {
        输出.状态值 = static_cast<std::int64_t>(枚举_稳定复现等级状态::遮挡保留);
    } else if ((输出.阻断原因掩码 & static_cast<std::int64_t>(枚举_稳定复现阻断原因::丢失中)) != 0) {
        输出.状态值 = static_cast<std::int64_t>(枚举_稳定复现等级状态::丢失中);
    } else if ((输出.阻断原因掩码 & 缺基础掩码) != 0 || 输出.评分Q10000 <= 0) {
        输出.状态值 = static_cast<std::int64_t>(枚举_稳定复现等级状态::未复现);
    } else if (输出.评分Q10000 >= 8000 && 输出.跨帧连续稳定命中次数 >= 3) {
        输出.状态值 = static_cast<std::int64_t>(枚举_稳定复现等级状态::稳定复现);
    } else if (输出.评分Q10000 >= 5000 && 输出.跨帧连续命中次数 >= 2) {
        输出.状态值 = static_cast<std::int64_t>(枚举_稳定复现等级状态::可复现);
    } else {
        输出.状态值 = static_cast<std::int64_t>(枚举_稳定复现等级状态::弱复现);
    }
    return 输出;
}
...
// 功能：从外设观察报告汇总共享关联判断所需的只读材料入口。
结构_共享关联外设材料输入 构造共享关联外设材料输入_由观察报告(
    const 结构_外设观察报告队列项& 报告项,
    const 结构_外设观察等待项* 等待项)
{
    结构_共享关联外设材料输入 输出{};
    ...
    输出.来源外设ID = 报告项.来源外设ID;
    输出.坐标系ID = 报告项.坐标系ID;

    if (外设提交_材料句柄可回查(报告项.空间候选队列句柄)) {
        输出.空间候选队列句柄 = 报告项.空间候选队列句柄;
    } else if (报告项.空间候选队列句柄.empty()) {
        外设提交_追加缺口(输出.缺口原因, 枚举_外设提交缺口码::缺空间候选);
    } else {
        外设提交_追加缺口(输出.缺口原因, 枚举_外设提交缺口码::材料过期);
    }

    if (外设提交_材料句柄可回查(报告项.像素归属账本句柄)) {
        输出.像素归属账本句柄 = 报告项.像素归属账本句柄;
    } else if (报告项.像素归属账本句柄.empty()) {
        外设提交_追加缺口(输出.缺口原因, 枚举_外设提交缺口码::缺像素归属账本);
    } else {
        外设提交_追加缺口(输出.缺口原因, 枚举_外设提交缺口码::材料过期);
    }

    for (const auto& 簇 : 报告项.观察像素簇集合) {
        外设提交_追加可回查句柄(输出.感兴趣区域句柄集, 簇.ROI句柄);
        外设提交_追加可回查句柄(输出.点集句柄集, 簇.点集句柄);
        外设提交_追加可回查句柄(输出.空间候选句柄集, 簇.空间候选句柄);
        外设提交_追加可回查句柄(输出.彩色轮廓局部图句柄集, 簇.彩色轮廓局部图句柄);
        外设提交_追加可回查句柄(输出.深度轮廓局部图句柄集, 簇.深度轮廓局部图句柄);
    }

    const auto 判定 = 判断外设观察材料质量(报告项, 等待项);
    输出.材料质量收束状态 = 计算观察材料质量收束状态(报告项, 判定, 等待项);
    输出.可承接能力等级 = 判定.可承接能力等级;
    外设提交_合并缺口集合(
        输出.缺口原因,
        收集观察材料质量缺口码(报告项, 判定, 等待项));

    std::optional<结构_目标观察约束特征组> 目标约束{};
    if (报告项.目标观察约束ID != 0) {
        结构_外设观察等待项 约束等待项{};
        约束等待项.目标观察约束ID = 报告项.目标观察约束ID;
        目标约束 = 读取目标观察约束_按等待项(约束等待项);
    }
    if (!目标约束 && 等待项) {
        目标约束 = 读取目标观察约束_按等待项(*等待项);
    }
    if (!目标约束 && 等待项 && !等待项->目标存在ID.empty() && !等待项->目标特征类型ID.empty()) {
        目标约束 = 选择最新可用目标观察约束(
            等待项->目标存在ID,
            等待项->目标特征类型ID,
            报告项.时间戳毫秒);
    ...
    if (目标约束) {
        输出.目标观察约束ID = 目标约束->约束ID;
        输出.目标约束ROI句柄 = 目标约束->ROI句柄;
        输出.目标约束像素集合掩码句柄 = 目标约束->像素集合掩码句柄;
        输出.目标约束点集句柄 = 目标约束->点集句柄;
        输出.目标约束二维最小X = 目标约束->二维最小X;
        输出.目标约束二维最大X = 目标约束->二维最大X;
        输出.目标约束二维最小Y = 目标约束->二维最小Y;
        输出.目标约束二维最大Y = 目标约束->二维最大Y;
        输出.目标约束空间范围最小X = 目标约束->空间范围最小X;
        输出.目标约束空间范围最大X = 目标约束->空间范围最大X;
        输出.目标约束空间范围最小Y = 目标约束->空间范围最小Y;
        输出.目标约束空间范围最大Y = 目标约束->空间范围最大Y;
        输出.目标约束空间范围最小Z = 目标约束->空间范围最小Z;
        输出.目标约束空间范围最大Z = 目标约束->空间范围最大Z;
        输出.目标约束最低深度有效率 = 目标约束->最低深度有效率;
        输出.目标约束最低空间坐标有效率 = 目标约束->最低空间坐标有效率;
        输出.目标约束最低掩码重合率 = 目标约束->最低掩码重合率;
        输出.目标约束最大预测误差 = 目标约束->最大预测误差;

        const auto 可用性 = 判断目标观察约束可用性(*目标约束, 报告项.时间戳毫秒, 等待项);
        输出.目标观察约束可用性状态 = static_cast<std::int64_t>(可用性.状态);
        输出.目标约束强匹配条件状态 = 可用性.强匹配条件状态;
        输出.目标约束降级匹配条件状态 = 可用性.降级匹配条件状态;
        输出.目标约束先验可用性收束状态 = 可用性.先验可用性收束状态;
    }

    return 输出;
}

// 功能：按报告 ID 从外设队列读取共享关联判断所需的只读材料入口。
std::optional<结构_共享关联外设材料输入> 读取共享关联外设材料输入_按报告ID(
    std::uint64_t 报告ID,
    const 结构_外设观察等待项* 等待项)
{
    const auto 报告项 = 读取外设观察报告_按ID(报告ID, true);
    if (!报告项) {
        return std::nullopt;
    }
    return 构造共享关联外设材料输入_由观察报告(*报告项, 等待项);
}
...
// 功能：为已选中的 D455 报告短期保护对应材料页，避免容量清理抢先移除正在承接的局部材料。
export bool 保护D455短期观察材料页(
    const 结构_外设观察报告队列项& 报告项,
    std::int64_t 保护毫秒)
{
    const auto 材料来源报告ID = D455_短期材料页来源报告ID(报告项);
    if (材料来源报告ID == 0 || 保护毫秒 <= 0) {
        return false;
    }

    auto& 状态 = 外设观察队列状态();
    const auto 当前时间 = 外设观察当前时间毫秒();
    const auto 保护截止 = 当前时间 + 保护毫秒;
    std::lock_guard<std::mutex> 锁(状态.互斥);

    const auto 材料页存在 = std::any_of(
        状态.D455材料页队列.begin(),
        状态.D455材料页队列.end(),
        [材料来源报告ID](const 结构_D455短期观察材料页& 页) noexcept {
            return 页.报告ID == 材料来源报告ID;
        });
    if (!材料页存在) {
        D455_清理短期材料页_已加锁(状态, 当前时间);
        return false;
    ...
    auto 已有 = std::find_if(
        状态.D455材料页保护集合.begin(),
        状态.D455材料页保护集合.end(),
        [材料来源报告ID](const 结构_D455短期材料页保护项& 项) noexcept {
            return 项.报告ID == 材料来源报告ID;
        });
    if (已有 != 状态.D455材料页保护集合.end()) {
        已有->保护截止毫秒 = (std::max)(已有->保护截止毫秒, 保护截止);
    } else {
        状态.D455材料页保护集合.push_back(
            结构_D455短期材料页保护项{ 材料来源报告ID, 保护截止 });
    }
    D455_清理短期材料页_已加锁(状态, 当前时间);
    return true;
}
