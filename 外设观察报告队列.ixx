...
namespace {
    ...
    std::string 外设提交_材料页ID(const 结构_外设观察报告队列项& 报告项)
    {
        return 报告项.报告ID == 0
            ? std::string{}
            : "D455材料页#报告" + std::to_string(报告项.报告ID);
    }
    ...
    const 结构_外设观察等待项* 等待项)
{
    ...
    const bool 目标区域基础可用 =
        ...
        || 目标区域空间有效率 >= 最低可用有效率;
    if (!全局基础可用 && !目标区域基础可用) {
        return 设置不合格("全局和目标区域均缺少基础深度/空间质量", "重新观察或调整相机工作模式");
    }
    ...
    if (判定.等级 == 枚举_观察材料等级::可用) {
        判定.需补观察原因 = "材料可用但未达到优质门槛或缺少强目标约束";
