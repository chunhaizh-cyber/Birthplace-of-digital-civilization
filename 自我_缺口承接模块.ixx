...
inline 结构_缺口承接快照_最小_v0 读取缺口承接快照_最小_v0(
    const 自我类& 自我对象,
    const 自我线程类* 自我线程对象 = nullptr) noexcept
{
    ...
    const auto 恢复快照 = 自我线程对象->读取最近缺口恢复接口快照();
...
inline bool 同步缺口承接镜像_最小_v0(
    自我类& 自我对象,
    const 结构_缺口承接快照_最小_v0& 快照,
    时间戳 now = 0,
    const std::string& 调用点 = "同步缺口承接镜像_最小_v0") noexcept
{
    if (now == 0) {
        now = 结构体_时间戳::当前_微秒();
    }
