...
class 特征值主信息类 {
public:
    // 特征值主信息本身就是“自有值容器”，这里不额外包关系锚点。
    // 权威本体：原始 VecU 值。任何业务语义必须由特征、二次特征或因果证据解释。
    VecIU64 值{};
    // 索引缓存：只用于去重和快速查找，不表达特征类型或业务含义。
    std::uint64_t 内容哈希 = 0;
    // 统计缓存：只反映证据池复用情况，不作为任务、需求、结算判断来源。
    std::uint64_t 命中次数 = 0;
    时间戳 最后命中 = 0;
