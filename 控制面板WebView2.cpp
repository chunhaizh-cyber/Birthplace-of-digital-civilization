...
// 功能：保存自我场景 WebView2 窗口客户区 PNG 截图。
    ...
    const std::uint32_t 等待渲染毫秒) noexcept
{
    try {
        if (!私有_打开自我场景窗口(nullptr)) {
            私有_记录WebView2诊断("截图打开自我场景窗口失败", 44);
            return false;
        }

        HWND 窗口 = nullptr;
        const auto 开始时间 = std::chrono::steady_clock::now();
        const auto 最大等待 = std::chrono::milliseconds(
            等待渲染毫秒 == 0 ? 1000u : 等待渲染毫秒);
        while (std::chrono::steady_clock::now() - 开始时间 < 最大等待) {
            窗口 = 私有_自我场景窗口句柄.load();
            if (窗口 && IsWindow(窗口)) {
                break;
            }
        ...
        if (!窗口 || !IsWindow(窗口)) {
            私有_记录WebView2诊断("截图等待自我场景窗口超时", 45);
            return false;
        }
        ...
        return 私有_保存窗口客户区PNG(窗口, 输出路径);
    }
    catch (...) {
        私有_记录WebView2诊断("截图捕获未知异常", 46);
        return false;
    }
}
