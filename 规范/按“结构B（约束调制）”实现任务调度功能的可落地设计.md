...
- `处理待重评估队列()`
- `查询需求状态()`  
并且规划了索引结构：主键→需求列表、任务→需求列表、需求状态表、待重评估队列（去重）。fileciteturn102file0L1-L1
...
2) 调度器索引键升级为稳定ID（主键字符串/自增ID/哈希值），而不是裸指针。
...
你补全 `任务调度器` 后，建议最小准备三个“日志验收”用例（优先通过主程序入口或正式调度链触发，再看日志断言，后续再上测试框架）：
