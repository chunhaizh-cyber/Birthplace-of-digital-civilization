...
public:
    ...
    {
        结构_需求派生归因兼容视图 输出{};
        输出.派生来源方法主键 = &主信息.派生来源方法主键;
        输出.派生来源因果主键 = &主信息.派生来源因果主键;
        输出.派生需求类型抽象特征指针 = 主信息.派生需求类型抽象特征指针;
        输出.派生需求类型值 = 主信息.派生需求类型值;
        输出.派生方法需求位抽象特征指针 = 主信息.派生方法需求位抽象特征指针;
        输出.派生方法需求位值 = 主信息.派生方法需求位值;
        输出.派生本能能力缺口类型抽象特征指针 =
            主信息.派生本能能力缺口类型抽象特征指针;
        输出.派生本能能力缺口类型值 = 主信息.派生本能能力缺口类型值;
        return 输出;
    }
    ...
    static const std::string& 读取派生来源因果主键_兼容(
        const 需求主信息类& 主信息) noexcept
    {
        return 读取派生归因兼容视图(主信息).派生来源因果主键文本();
    }
...
private:
        ...
        const 需求主信息类& 候选) noexcept
    {
        ...
        if (目标.派生来源因果主键.empty()) {
            目标.派生来源因果主键 = 候选.派生来源因果主键;
        }
        ...
        const 结构_需求树更新指令& 指令) noexcept
    {
        ...
        case 枚举_需求树更新指令类型::满足需求:
            ...
            if (!指令.派生来源方法主键.empty()) {
                目标需求->主信息.派生来源方法主键 = 指令.派生来源方法主键;
            }
            if (!指令.派生来源因果主键.empty()) {
                目标需求->主信息.派生来源因果主键 = 指令.派生来源因果主键;
            }
            if (指令.派生需求类型抽象特征指针 != 0) {
                目标需求->主信息.派生需求类型抽象特征指针 =
                    指令.派生需求类型抽象特征指针;
            }
            if (指令.派生需求类型值 != 0) {
                目标需求->主信息.派生需求类型值 = 指令.派生需求类型值;
            }
            if (指令.派生方法需求位抽象特征指针 != 0) {
                目标需求->主信息.派生方法需求位抽象特征指针 =
                    指令.派生方法需求位抽象特征指针;
            }
            if (指令.派生方法需求位值 != 0) {
                目标需求->主信息.派生方法需求位值 = 指令.派生方法需求位值;
            }
            if (指令.派生本能能力缺口类型抽象特征指针 != 0) {
                目标需求->主信息.派生本能能力缺口类型抽象特征指针 =
                    指令.派生本能能力缺口类型抽象特征指针;
            }
            if (指令.派生本能能力缺口类型值 != 0) {
                目标需求->主信息.派生本能能力缺口类型值 =
                    指令.派生本能能力缺口类型值;
            }
            if (指令.使用阻塞父任务执行) {
