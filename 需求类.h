...
public:
        ...
        const std::int64_t 子需求数量) noexcept
    {
        ...
        // 权重字段现在只表达“活动需求是否参与对应方向调度”。
        // 父需求的安全 / 服务压力不再向子需求平分，避免压力额度污染学习需求和结算链。
        输出.安全权重 = 父需求安全权重 > 0 ? 1 : 0;
        输出.服务权重 = 父需求服务权重 > 0 ? 1 : 0;
        return 输出;
    }
    ...
    std::size_t 刷新直接子需求权重_已加锁(节点类* 父需求) noexcept
    {
        ...
        do {
            if (当前子节点->主信息.需求有效截止 != 0
                ...
                const auto 原安全权重 = 当前子节点->主信息.安全权重;
                const auto 原服务权重 = 当前子节点->主信息.服务权重;
                const bool 父安全方向 = 父需求->主信息.安全权重 > 0;
                const bool 父服务方向 = 父需求->主信息.服务权重 > 0;
                const bool 子已有服务方向 = 原服务权重 > 0 && 原安全权重 <= 0;
                当前子节点->主信息.安全权重 =
