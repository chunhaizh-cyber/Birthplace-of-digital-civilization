...
// 文件头部规则注释模块：
// 1. 需求类负责需求树结构、需求主信息和需求提交入口，不承载任务状态裁决。
// 2. SQL 控制面板同步只允许作为显示镜像，不得成为需求判断、恢复或业务权威源。
// 3. 新增需求入口必须保持自我线程裁决和统一需求树更新指令边界。
...
enum class 枚举_枚举目标校验结论 : std::uint8_t {
    未检查 = 0,
    缺目标特征或目标状态 = 1,
    非枚举_含非点值状态 = 2,
    非枚举_未识别 = 3,
...
public:
    ...
    enum class 枚举_需求获取途径 : std::uint8_t {
        未定义 = 0,
        // 历史兼容枚举值：当前显示和语义收束为“正式目标承接”，不得解释为凭空普通目标。
        普通目标直接生成 = 1,
        任务筹办原子缺口 = 2,
        任务执行回执原子缺口 = 3,
        自检原子缺口 = 4,
    ...
    enum class 枚举_父子需求关系角色 : std::uint8_t {
        未定义 = 0,
        必须子需求 = 1,
        可选路径子需求 = 2,
        非必须子需求 = 3,
    ...
    static bool 需求获取途径是简化准入输出(
        枚举_需求获取途径 途径) noexcept;

    static 枚举_需求获取途径 需求_按缺口类型选择获取途径(
        bool 根特征缺口,
        bool 任务筹办缺口,
        bool 执行期真实变化缺口,
        bool 可验证服务触发事实,
    ...
    static const std::vector<可解析引用<任务节点类>>* 读取需求任务列表引用(
        const 节点类* 需求) noexcept;
    ...
    static 结构_需求树更新指令 构造删除需求指令(
        节点类* 父需求,
        节点类* 目标需求) noexcept;

    // 功能：构造需求权重和阻塞状态更新指令，不直接修改需求树。
    static 结构_需求树更新指令 构造需求权重阻塞更新指令(
        节点类* 父需求,
        节点类* 目标需求,
    ...
    static 结构_需求树更新指令 构造需求目标关系更新指令(
        节点类* 父需求,
        节点类* 目标需求,
        状态节点类* 当前状态,
        状态节点类* 目标状态,
        ...
        const 结构_需求树更新指令& 指令) noexcept
    {
        ...
        std::string 删除目标主键{};
        {
            ...
            auto* 原根 = this->根指针;
            this->根指针 = 真实根;
            if (指令.指令类型 == 枚举_需求树更新指令类型::删除需求) {
                if (auto* 删除目标 = 定位节点_已加锁(真实根, 指令.目标需求指针, 指令.目标需求主键)) {
                    删除目标主键 = 删除目标->获取主键();
                } else {
                    删除目标主键 = 指令.目标需求主键;
                }
        ...
        if (结果 && SQL投影根) {
            ...
            if (指令.指令类型 == 枚举_需求树更新指令类型::删除需求) {
                同步删除SQL控制面板需求显示项(删除目标主键, "需求树删除指令成功");
            } else {
                同步SQL控制面板需求显示项(结果, "需求树更新指令成功");
            }
        }
        return 结果;
    }
...
private:
    ...
    static void 同步SQL控制面板需求显示项(
        const 节点类* 需求,
        const char* 阶段) noexcept;

    // 功能：把需求节点删除变化同步到 SQL 控制面板运行态显示镜像。
    static void 同步删除SQL控制面板需求显示项(
    ...
    // 功能：读取状态特征类型主键，不创建或修改节点。
    static std::string 状态特征类型主键_已加锁(
        const 可解析引用<特征节点类>& 状态特征) noexcept
    {
        if (const auto* 类型 = 特征类::读取特征语义类型_直接(状态特征.指针)) {
            return 类型->获取主键();
        }
        if (!状态特征.主键.empty()) {
            return 状态特征.主键;
    ...
    static 结构_需求目标键 生成需求目标键_已加锁(const 需求主信息类& 主信息) noexcept
    {
        ...
        可解析引用<基础信息节点类> 显式目标状态主体{};
        可解析引用<特征节点类> 显式目标状态特征{};
        特征值 显式目标状态值{};
        const bool 有显式目标状态材料 = 状态类::读取状态主体特征和值引用(
            主信息.需求状态.指针,
            显式目标状态主体,
            显式目标状态特征,
            显式目标状态值);
        ...
        if (有显式目标状态材料) {
            输出.目标状态值 = 显式目标状态值;
            输出.目标状态值有效 =
                !std::holds_alternative<std::monostate>(显式目标状态值);
        }

        可解析引用<基础信息节点类> 目标状态主体 = 显式目标状态主体;
        可解析引用<特征节点类> 目标状态特征 = 显式目标状态特征;
        bool 有目标状态材料 = 有显式目标状态材料;
        if (!有目标状态材料 && 主信息.被需求当前状态.指针) {
            特征值 当前状态值{};
            有目标状态材料 = 状态类::读取状态主体特征和值引用(
                主信息.被需求当前状态.指针,
                目标状态主体,
                目标状态特征,
                当前状态值);
        }
        if (!有目标状态材料 && !主信息.需求状态.主键.empty()) {
            // 主键未解析时不能可靠提取特征类型，保留空键使调用方退回旧路径。
            return 输出;
        }
        if (!有目标状态材料 && !主信息.被需求当前状态.主键.empty()) {
            return 输出;
        }

        if (有目标状态材料) {
            if (输出.目标主体主键.empty()) {
                输出.目标主体主键 = 基础引用主键_已加锁(目标状态主体);
            }
            输出.目标特征类型主键 = 状态特征类型主键_已加锁(目标状态特征);
        }
    ...
    static void 从状态补齐目标主体和特征缓存_已加锁(需求主信息类& 主信息) noexcept
    {
        ...
        if (!主信息.被需求存在.有效()) {
            if (auto* 存在 = dynamic_cast<存在节点类*>(状态主体.指针)) {
                主信息.被需求存在.指针 = 存在;
                主信息.被需求存在.主键 = 存在->获取主键();
            } else if (!状态主体.主键.empty()) {
                主信息.被需求存在.主键 = 状态主体.主键;
            }
        }
    ...
    // 功能：执行更新或本轮承接收束指令，更新目标需求字段并按需截止需求。
    节点类* 执行更新或满足需求指令_已加锁(
        节点类* 目标需求,
        const 结构_需求树更新指令& 指令) noexcept
    {
