    ...
    const 语素入口节点类* 动作名) noexcept
{
    ...
    if (全局_语素入口同一(动作语义入口, 动作名)) return true;
...
{
    return 全局_语素入口同一(私有_特征语义类型(特征), 期望类型);
}
...
{
    return 全局_语素入口同一(特征类型, 私有_只读查找特征类型词(期望词));
}

// 功能：服务所在模块的内部辅助流程。
static bool 私有_目标特征类型是安全账务值(const 语素入口节点类* 特征类型) noexcept
{
    return 全局_语素入口同一(特征类型, 自我特征定义类::类型_自我_安全值());
}
...
{
    return 全局_语素入口同一(特征类型, 自我特征定义类::类型_自我_服务值());
}
...
{
    return 全局_语素入口同一(特征类型, 自我特征定义类::类型_自我_物理安全())
        || 全局_语素入口同一(特征类型, 自我特征定义类::类型_自我_风险安全());
}
...
{
    return 全局_语素入口同一(特征类型, 自我特征定义类::类型_任务_状态())
        || 全局_语素入口同一(特征类型, 自我特征定义类::类型_任务_局部完成度())
        || 全局_语素入口同一(特征类型, 自我特征定义类::类型_任务_局部运行状态())
        || 全局_语素入口同一(特征类型, 自我特征定义类::类型_任务管理_结果状态())
        || 全局_语素入口同一(特征类型, 自我特征定义类::类型_任务管理_任务执行记账结果());
}
...
{
    return 全局_语素入口同一(特征类型, 自我特征定义类::类型_学习_目标状态())
        || 全局_语素入口同一(特征类型, 自我特征定义类::类型_学习_信息补齐状态())
        || 全局_语素入口同一(特征类型, 自我特征定义类::类型_学习_因果验证状态())
        || 全局_语素入口同一(特征类型, 自我特征定义类::类型_学习_方法可靠度状态())
        || 全局_语素入口同一(特征类型, 自我特征定义类::类型_学习_风险评估状态());
}
...
static 枚举_目标投影维度 私有_按特征类型语义推断目标维度(const 语素入口节点类* 特征类型)
{
    ...
    if (全局_语素入口同一(特征类型, 自我特征定义类::类型_自我_待处理方法数量())
        || 全局_语素入口同一(特征类型, 自我特征定义类::类型_自我_可用方法数量())
        || 全局_语素入口同一(特征类型, 自我特征定义类::类型_自我_当前主需求())
        || 全局_语素入口同一(特征类型, 自我特征定义类::类型_自我_当前主任务())
        || 全局_语素入口同一(特征类型, 自我特征定义类::类型_自我_当前主方法())) {
    ...
    const 结构_自检原子目标因果投影输入& 输入) const
{
    ...
    for (auto 路径 : 投影.目标投影路径) {
        ...
        if (!输入.目标特征类型
            && out.目标特征类型词
            && !全局_语素入口同一(路径.目标特征类型词, out.目标特征类型词)
