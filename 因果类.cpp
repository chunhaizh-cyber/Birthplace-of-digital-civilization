...
static bool 私有_因果模板状态迁移命中动态(
    ...
    const 特征节点类* 动态结果特征)
{
    if (!模板 || !动态信息 || !模板->是抽象因果()) return false;
    ...
    bool 设为主结果)
{
    ...
    (void)私有_补入因果结果特征_因果(instInfo, 动态结果特征);
    if (!果状态迁移模板.empty()) {
        (void)私有_补入因果果侧模板_因果(
            instInfo,
            base,
    ...
    if (有动作 && dm->来源动作.有效()) instInfo->因方法模板 = dm->来源动作;
    if (有动作 && !因动作语义键.empty()) instInfo->因动作语义键 = 因动作语义键;
    else if (有动作 && (动作语义 || 动作主体)) instInfo->因动作语义键 = 私有_节点键(动作语义 ? 动作语义 : 动作主体);
    if (有动作 && dm->来源动作名) instInfo->因动作名称 = 私有_动作名键(dm->来源动作名);
    if (有动作) {
        instInfo->因出现次数 = 1;
    ...
    if (!动态结果特征 && dm->结果状态.有效() && !果状态迁移模板.empty()) {
        ...
        输出内部阶段耗时("缺结果特征，仅创建证据实例", true, dm);
        return result;
    }
    if (果状态迁移模板.empty()) {
    ...
    for (auto* 模板节点 : 枚举因果模板()) {
        ...
        if (模板信息->适用锚点类型 != instInfo->适用锚点类型) continue;
        if (模板信息->适用层级 != dm->动态层级) continue;
        if (!私有_因果模板状态迁移命中动态(
                base,
                模板信息,
                dm,
