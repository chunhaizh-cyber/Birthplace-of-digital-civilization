    ...
    bool 设为主结果)
{
    ...
    if (动态主信息->初始状态.指针) {
        (void)追加条件状态(节点, 动态主信息->初始状态.指针);
    }
    if (!动态主信息->结果状态.指针) {
        return false;
    }
    return 追加结果状态(节点, 动态主信息->结果状态.指针, 设为主结果);
}
...
bool 因果类::二次特征匹配模板(const 二次特征节点类* 二次特征, const 二次特征节点类* 模板) const noexcept
{
    ...
    const auto* 候选主信息 = 基础信息_->取主信息<二次特征主信息类>(二次特征);
    const auto* 模板主信息 = 基础信息_->取主信息<二次特征主信息类>(模板);
    if (!候选主信息 || !模板主信息) return false;

    return 二次特征满足_桥接(模板主信息, 候选主信息);
}
    ...
    const std::vector<二次特征节点类*>& 结果候选) const noexcept
{
    ...
    if (模板主信息->条件模板.empty() && 模板主信息->结果模板.empty()) return false;
    ...
    if (!模板全部命中(模板主信息->条件模板, 条件候选)) return false;
    if (!模板全部命中(模板主信息->结果模板, 结果候选)) return false;
    return true;
}
