...
static void 私有_从因果模板签名索引移除节点(
    std::unordered_map<std::string, std::vector<因果模板节点类*>>& 索引,
    因果模板节点类* 节点)
{
    if (!节点) return;
    for (auto it = 索引.begin(); it != 索引.end();) {
        auto& 节点集合 = it->second;
        节点集合.erase(std::remove(节点集合.begin(), 节点集合.end(), 节点), 节点集合.end());
        if (节点集合.empty()) {
            it = 索引.erase(it);
        } else {
            ++it;
...
{
    return 私有_语素入口同一(特征类型, 私有_只读查找特征类型词(期望词));
}

static bool 私有_目标特征类型是安全账务值(const 语素入口节点类* 特征类型) noexcept
{
    return 私有_语素入口同一(特征类型, 自我特征定义类::类型_自我_安全值());
}
...
{
    return 私有_语素入口同一(特征类型, 自我特征定义类::类型_自我_服务值());
}
...
{
    return 私有_语素入口同一(特征类型, 自我特征定义类::类型_自我_物理安全())
        || 私有_语素入口同一(特征类型, 自我特征定义类::类型_自我_风险安全());
}
...
{
    return 私有_语素入口同一(特征类型, 自我特征定义类::类型_任务_状态())
        || 私有_语素入口同一(特征类型, 自我特征定义类::类型_任务_局部完成度())
        || 私有_语素入口同一(特征类型, 自我特征定义类::类型_任务_局部运行状态())
        || 私有_语素入口同一(特征类型, 自我特征定义类::类型_任务管理_结果状态())
        || 私有_语素入口同一(特征类型, 自我特征定义类::类型_任务管理_任务执行记账结果());
}
...
{
    return 私有_语素入口同一(特征类型, 自我特征定义类::类型_学习_目标状态())
        || 私有_语素入口同一(特征类型, 自我特征定义类::类型_学习_信息补齐状态())
        || 私有_语素入口同一(特征类型, 自我特征定义类::类型_学习_因果验证状态())
        || 私有_语素入口同一(特征类型, 自我特征定义类::类型_学习_方法可靠度状态())
        || 私有_语素入口同一(特征类型, 自我特征定义类::类型_学习_风险评估状态());
}
...
static 枚举_目标投影维度 私有_按特征类型语义推断目标维度(const 语素入口节点类* 特征类型)
{
    ...
    if (私有_语素入口同一(特征类型, 自我特征定义类::类型_自我_待处理方法数量())
        || 私有_语素入口同一(特征类型, 自我特征定义类::类型_自我_可用方法数量())
        || 私有_语素入口同一(特征类型, 自我特征定义类::类型_自我_当前主需求())
        || 私有_语素入口同一(特征类型, 自我特征定义类::类型_自我_当前主任务())
        || 私有_语素入口同一(特征类型, 自我特征定义类::类型_自我_当前主方法())) {
...
void 因果类::标记因果模板签名索引失效() const
{
    std::lock_guard<std::mutex> 锁(因果模板签名索引互斥_);
    因果模板签名索引有效_ = false;
    状态变化关系模板索引_.clear();
    动作致变关系模板索引_.clear();
}

void 因果类::确保因果模板签名索引() const
{
    std::lock_guard<std::mutex> 锁(因果模板签名索引互斥_);
    if (因果模板签名索引有效_) {
        return;
    ...
    for (auto* 节点 : 枚举因果模板()) {
        ...
        if (!节点 || !主信息) continue;
        if (!私有_因果模板果侧可用(主信息)) continue;

        if (私有_因果模板有动作(主信息)) {
            const auto 签名 = 计算模板动作致变签名(节点);
            if (!签名.empty()) {
                动作致变关系模板索引_[签名].push_back(节点);
            }
        } else {
            const auto 签名 = 计算模板状态变化签名(节点);
            if (!签名.empty()) {
                状态变化关系模板索引_[签名].push_back(节点);
            }
        }
    }
    因果模板签名索引有效_ = true;
...
void 因果类::刷新因果模板签名索引项(因果模板节点类* 节点) const
{
    std::lock_guard<std::mutex> 锁(因果模板签名索引互斥_);
    if (!因果模板签名索引有效_ || !节点) {
        return;
    ...
    私有_从因果模板签名索引移除节点(状态变化关系模板索引_, 节点);
    私有_从因果模板签名索引移除节点(动作致变关系模板索引_, 节点);
...
{
    if (状态变化签名.empty()) return {};
    确保因果模板签名索引();
    std::lock_guard<std::mutex> 锁(因果模板签名索引互斥_);
    const auto it = 状态变化关系模板索引_.find(状态变化签名);
    return it != 状态变化关系模板索引_.end() ? it->second : std::vector<因果模板节点类*>{};
}

std::vector<因果模板节点类*> 因果类::查找动作致变关系模板(const std::string& 动作致变签名) const
{
    if (动作致变签名.empty()) return {};
    确保因果模板签名索引();
    std::lock_guard<std::mutex> 锁(因果模板签名索引互斥_);
    const auto it = 动作致变关系模板索引_.find(动作致变签名);
    return it != 动作致变关系模板索引_.end() ? it->second : std::vector<因果模板节点类*>{};
}
    ...
    const 结构_自检原子目标因果投影输入& 输入) const
{
    ...
    for (auto 路径 : 投影.目标投影路径) {
        ...
        if (!输入.目标特征类型
            && out.目标特征类型词
            && !私有_语素入口同一(路径.目标特征类型词, out.目标特征类型词)
    ...
    std::unordered_set<std::string> 已计来源因果{};
    for (const auto& 路径 : out.候选投影路径) {
        ...
        for (const auto& 边 : 路径.证据路径) {
            ...
            const auto 键 = 模板->获取主键();
            if (已计来源因果.insert(键).second) {
                out.来源因果主键集.push_back(键);
            }
        }
    }
    ...
    bool 设为主结果)
{
    ...
    auto 输出内部阶段耗时 = [&](
        ...
        std::string 最慢阶段 = "状态变化签名计算";
        std::uint64_t 最慢阶段耗时微秒 = 状态变化签名计算耗时微秒;
        ...
        日志 << "按动态补全两层因果/内部阶段耗时"
            ...
            << " | 最慢阶段=" << 最慢阶段
            << " | 最慢阶段耗时微秒=" << 最慢阶段耗时微秒
            << " | 状态变化签名计算=" << 状态变化签名计算耗时微秒
            << " | 动作致变签名计算=" << 动作致变签名计算耗时微秒
            << " | 状态模板查找=" << 状态模板查找耗时微秒
            << " | 状态关系创建=" << 状态关系创建耗时微秒
    ...
    auto 桥接已登记证据动态的结果状态 = [&](因果模板节点类* 节点) {
        ...
        for (auto* t : 取果状态迁移模板()) {
            ...
            if (!私有_向量含二次特征(m->果比较模板, t)) {
                m->果比较模板.push_back(私有_制引用(t));
                签名相关变化 = true;
            }
        }
        if (签名相关变化) {
    ...
    const auto 状态变化签名开始 = 私有_因果阶段当前时间点();
    result.状态变化签名 = 计算动态状态变化签名(动态节点);
    私有_累加因果阶段耗时(状态变化签名计算耗时微秒, 状态变化签名开始);
    const auto 动作致变签名开始 = 私有_因果阶段当前时间点();
    result.动作致变签名 = 计算动态动作致变签名(动态节点, 动作主体, 动作语义, 因动作语义键);
    私有_累加因果阶段耗时(动作致变签名计算耗时微秒, 动作致变签名开始);

    const auto& 果状态迁移模板 = 取果状态迁移模板();
    有动作 = 动作主体 || 动作语义 || !因动作语义键.empty() || dm->来源动作.有效() || dm->来源动作名 != nullptr;

    const auto 证据实例创建开始 = 私有_因果阶段当前时间点();
    ...
    私有_累加因果阶段耗时(证据实例创建耗时微秒, 证据实例创建开始);

    if (!result.证据实例) {
        输出内部阶段耗时("缺证据实例，跳过抽象因果模板", false, dm);
        return result;
    }
    if (果状态迁移模板.empty()) {
        输出内部阶段耗时("缺果状态迁移模板，仅创建证据实例", true, dm);
        return result;
    }
...
{
    auto* base = 基础信息_ ? 基础信息_ : &获取基础信息集();
    const bool 已删除 = base && 节点 ? base->删除节点(reinterpret_cast<基础信息节点类*>(节点)) : false;
    if (已删除) {
        标记因果模板签名索引失效();
    }
    return 已删除;
...
bool 因果类::按动态桥接结果状态(因果节点类* 节点, 动态节点类* 动态节点, bool 设为主结果)
{
    ...
    bool 签名相关变化 = 原证据动态样本为空 && !m->证据动态样本.empty();
    if (设为主结果) {
    ...
    if (签名相关变化) {
        刷新因果模板签名索引项(reinterpret_cast<因果模板节点类*>(节点));
    }
    return true;
}
...
bool 因果类::追加状态迁移模板(因果节点类* 节点, 二次特征节点类* 果状态迁移模板, bool 设为主迁移)
{
    ...
    bool 签名相关变化 = false;
    if (!私有_向量含二次特征(m->果比较模板, 果状态迁移模板)) {
        m->果比较模板.push_back(私有_制引用(果状态迁移模板));
        签名相关变化 = true;
    }
    if (设为主迁移 || !m->主果比较模板.有效()) {
        m->主果比较模板 = 私有_制引用(果状态迁移模板);
        签名相关变化 = true;
    }
    if (签名相关变化) {
        刷新因果模板签名索引项(reinterpret_cast<因果模板节点类*>(节点));
