...
struct 结构_方法结果关系模板 {
    // 方法结果能力的关系模板表达。旧 结构_方法结果项 字段仍保留为兼容输入，
    // 新逻辑应优先读取本结构中的目标特征类型、二次特征模板和允许方向。
    const 语素入口节点类* 目标特征类型 = nullptr;
    bool 目标特征类型由参数决定 = false;
    const 语素入口节点类* 目标特征类型参数 = nullptr;
    std::vector<可解析引用<二次特征节点类>> 结果二次特征模板{};
    方向掩码 允许方向 = 0;
...
struct 结构_方法结果项 {
    // 为空且 特征类型由参数决定=true 时，表示方法头只声明“可改变输入指定的特征类型”。
    // 具体特征类型必须由条件节点/输入包中的 特征类型参数 绑定，方法头只做大范围召回。
    const 语素入口节点类* 特征类型 = nullptr;
    bool 特征类型由参数决定 = false;
    const 语素入口节点类* 特征类型参数 = nullptr;
    ...
    void 同步兼容字段到关系模板() noexcept
    {
        ...
        if (!关系模板.目标特征类型由参数决定 && 特征类型由参数决定) {
            关系模板.目标特征类型由参数决定 = true;
        }
        if (!关系模板.目标特征类型参数 && 特征类型参数) {
            关系模板.目标特征类型参数 = 特征类型参数;
        }
        if (关系模板.允许方向 == 0) {
            关系模板.允许方向 = 方法结果变化方向允许方向(方向);
        }
        if (!关系模板.有目标值 && !std::holds_alternative<std::monostate>(目标值)) {
            关系模板.有目标值 = true;
            关系模板.目标值 = 目标值;
        }
        if (关系模板.最小变化量 == 0) {
...
public:
    ...
    std::int64_t 生成比较量(const 方法主信息类& 对象, 枚举_比较字段 字段) const {
        ...
        case 枚举_比较字段::名称:
            return 私有_比较入口指针(
                左首节点 ? 左首节点->动作名 : nullptr,
                右首节点 ? 右首节点->动作名 : nullptr);
        case 枚举_比较字段::类型:
            if (公共.节点种类 == 对象.公共.节点种类) return 0;
            return static_cast<std::uint8_t>(公共.节点种类) < static_cast<std::uint8_t>(对象.公共.节点种类) ? -1 : 1;
        case 枚举_比较字段::值:
            if (!左首节点 || !右首节点) return 左首节点 == 右首节点 ? 0 : (左首节点 ? 1 : -1);
            return 私有_动作句柄相等(左首节点->动作句柄, 右首节点->动作句柄) ? 0 : -1;
        default:
            return -1;
        }
    }
