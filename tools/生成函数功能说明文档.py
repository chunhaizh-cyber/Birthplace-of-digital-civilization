...
def is_excluded(path: Path) -> bool:
    ...
    if any(part in EXCLUDED_DIRS for part in rel.parts):
        return True
    return path.name.startswith(EXCLUDED_FILE_PREFIXES)
...
def candidate_is_function(signature: str, name: str, declaration: bool) -> bool:
    ...
    if lowered.startswith(STATEMENT_PREFIXES):
        return False
    if any(token in lowered for token in (" if ", "if constexpr", "else if", " return ", " static_cast", "reinterpret_cast", "dynamic_cast", "const_cast")):
        return False
...
def normalize_signature_start(text: str, start: int, open_paren: int) -> int:
    prefix = text[start:open_paren]
    m = re.match(r"\s*(public|private|protected)\s*:\s*", prefix)
    if not m:
        return start
    return start + m.end()
...
def extract_definitions(path: Path, text: str, masked: str, items: dict[str, str]) -> list[FunctionRecord]:
    ...
    for m in re.finditer(r"\{", masked):
        ...
        for close in reversed(close_positions):
            ...
            start = normalize_signature_start(text, start, open_paren)
...
def has_function_comment(lines: list[str], line_no: int) -> bool:
    idx = line_no - 2
    ...
    return any(mark in text for mark in ("功能", "用途", "说明"))


def write_missing_function_comments(records: list[FunctionRecord]) -> int:
    ...
    for path, file_records in by_file.items():
        ...
        for record in sorted(file_records, key=lambda r: r.line):
            if record.line < 1 or record.line > len(lines):
                continue
            if has_function_comment(lines, record.line):
                continue
            indent_match = re.match(r"\s*", lines[record.line - 1])
            indent = indent_match.group(0) if indent_match else ""
            block = [f"{indent}// 功能：{record.note}"]
            prev = lines[record.line - 2].strip() if record.line >= 2 else ""
            if prev and prev not in {"public:", "private:", "protected:"}:
                block.insert(0, "")
            inserts.append((record.line - 1, block))
...
def remove_generated_function_comments(files: list[Path]) -> int:
    removed = 0
    pattern = re.compile(r"^\s*// 功能：")
    for path in files:
        text = read_text(path)
        newline = detect_newline(text)
        trailing_newline = text.endswith(("\n", "\r\n"))
...
def render(records: list[FunctionRecord], files: list[Path]) -> str:
    ...
    definition_count = sum(1 for r in records if r.kind == "定义")
    declaration_count = sum(1 for r in records if r.kind == "声明")
    source_note_count = sum(1 for r in records if r.note_source == "源码注释")
    ...
    out.append("python .\\tools\\生成函数功能说明文档.py --write-comments")
    out.append("```")
    out.append("")
    out.append("## 查找规则")
    out.append("")
    ...
    out.append(f"- 函数定义：{definition_count}")
    out.append(f"- 函数声明：{declaration_count}")
    out.append(f"- 已读取源码功能说明：{source_note_count}")
    out.append("")
    out.append("## 函数索引")
    out.append("")
    out.append("| 函数 | 功能 | 位置 | 类型 |")
    out.append("| --- | --- | --- | --- |")
    for record in sorted(records, key=lambda r: (r.name, r.file.relative_to(ROOT).as_posix(), r.line, r.kind)):
        rel = record.file.relative_to(ROOT).as_posix()
        out.append(
            "| `{name}` | {note} | `{rel}:{line}` | {kind} |".format(
                name=markdown_escape(record.name),
                note=markdown_escape(record.note),
                rel=markdown_escape(rel),
                line=record.line,
                kind=record.kind,
            )
        )
...
def scan(items: dict[str, str], files: list[Path]) -> list[FunctionRecord]:
    records: list[FunctionRecord] = []
    for path in files:
        text = read_text(path)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="生成鱼巢函数功能说明文档。")
    parser.add_argument("--write-comments", action="store_true", help="给缺少标准功能说明的函数定义补充 // 功能： 注释。")
    parser.add_argument("--remove-generated-comments", action="store_true", help="移除脚本生成的 // 功能： 注释后再生成文档。")
    args = parser.parse_args()

    items = project_items()
    files = source_files()
    removed = 0
    if args.remove_generated_comments:
        removed = remove_generated_function_comments(files)
    records = scan(items, files)
    inserted = 0
    if args.write_comments:
        inserted = write_missing_function_comments(records)
        records = scan(items, files)
    ...
    if args.remove_generated_comments:
        print(f"移除功能注释：{removed}")
    if args.write_comments:
        print(f"新增功能注释：{inserted}")
