...
enum class 枚举_生命周期状态 : std::uint8_t {
    待机 = 0,
    运行 = 1,
    收束 = 2,
    停止 = 3,
...
enum class 枚举_运行模式 : std::uint8_t {
    正常 = 0,
    降级 = 1,
    只读 = 2,
};
...
bool 校验上下文包(const 结构_线程轮次上下文包& 包, std::string* 错误 = nullptr) noexcept;
bool 校验任务包(const 结构_一步治理任务包& 包, std::string* 错误 = nullptr) noexcept;
bool 校验结算包(const 结构_回流结算包& 包, std::string* 错误 = nullptr) noexcept;
bool 封口上下文包(结构_线程轮次上下文包& 包, 时间戳 封口时间 = 0, std::string* 错误 = nullptr) noexcept;
