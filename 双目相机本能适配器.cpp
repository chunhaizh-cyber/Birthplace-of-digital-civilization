...
namespace {
    ...
    bool g_相机已打开 = false;
    constexpr double 圆周率 = 3.14159265358979323846;
    ...
    // 功能：由相机内参计算单轴视野角，单位为毫度。
    std::int64_t 计算相机视野角毫度(const int 像素跨度, const double 焦距) noexcept
    {
        if (像素跨度 <= 0 || 焦距 <= 0.0) {
            return 0;
        }
        const double 弧度 = 2.0 * std::atan(static_cast<double>(像素跨度) / (2.0 * 焦距));
        const double 毫度 = 弧度 * 180000.0 / 圆周率;
        if (!std::isfinite(毫度) || 毫度 <= 0.0) {
            return 0;
        }
        return static_cast<std::int64_t>(std::llround(毫度));
    }

    // 功能：把 D455 深度内参派生为控制面板可复现的视野元数据。
    void 填充相机视野参数(
        const 结构体_原始场景帧& 帧,
        双目相机本能适配器::调用结果& 结果) noexcept
    {
        const int 宽 = 帧.深度内参.宽度 > 0 ? 帧.深度内参.宽度 : 帧.宽度;
        const int 高 = 帧.深度内参.高度 > 0 ? 帧.深度内参.高度 : 帧.高度;
        if (!帧.深度内参.有效) {
            结果.视野参数状态 = 0;
            return;
        }

        结果.水平视野角毫度 = 计算相机视野角毫度(宽, 帧.深度内参.fx);
        结果.垂直视野角毫度 = 计算相机视野角毫度(高, 帧.深度内参.fy);
        结果.视野参数状态 = 结果.水平视野角毫度 > 0 && 结果.垂直视野角毫度 > 0 ? 1 : 0;
    }
    ...
    struct 结构_轻量深度网格统计 {
        ...
        std::int64_t 稳定性合计 = 0;
        std::int64_t 邻域一致性合计 = 0;
        std::int64_t 原始来源像素数 = 0;
        std::int64_t 滤波来源像素数 = 0;
        std::int64_t 补全来源像素数 = 0;
    };

    // 功能：把像素深度按粗网格压成高频报告候选，避免每帧执行全图 BFS 和全点云生成。
    std::vector<双目相机本能适配器::空间候选摘要> 提取轻量深度网格候选摘要(
        const 结构体_原始场景帧& 帧,
        双目相机本能适配器::调用结果& 结果)
    {
        ...
        if (宽 <= 0
            ...
            || 帧.深度内参.fx == 0.0
            || 帧.深度内参.fy == 0.0
            || 帧.深度.size() != static_cast<std::size_t>(宽) * static_cast<std::size_t>(高)) {
            return 输出;
        }
        ...
        std::sort(
            候选单元.begin(),
            候选单元.end(),
            [&](std::size_t 左, std::size_t 右) noexcept {
                const auto& L = 统计[左];
                const auto& R = 统计[右];
                if (L.像素数量 != R.像素数量) {
                    return L.像素数量 > R.像素数量;
                }
                const auto L平均 = L.像素数量 > 0 ? L.深度合计 / L.像素数量 : std::numeric_limits<std::int64_t>::max();
                const auto R平均 = R.像素数量 > 0 ? R.深度合计 / R.像素数量 : std::numeric_limits<std::int64_t>::max();
                return L平均 < R平均;
            });
        if (候选单元.size() > 最大候选数量) {
            候选单元.resize(最大候选数量);
        ...
        for (std::size_t i = 0; i < 输出.size(); ++i) {
            ...
            const auto ROI面积 = static_cast<std::size_t>(
                (单元.投影最大X - 单元.投影最小X + 1)
                * (单元.投影最大Y - 单元.投影最小Y + 1));
            const auto 深度跨度 = 绝对差I64(单元.深度最大, 单元.深度最小);
            const auto 深度一致率 = 限制I64(10000 - (深度跨度 * 10000) / 3000, 0, 10000);
            const auto 投影覆盖率 = 比例万分比(
                static_cast<std::size_t>(std::max<std::int64_t>(0, 像素数量)),
                std::max<std::size_t>(1, ROI面积));
            const auto 大小评分 = 比例万分比(
                static_cast<std::size_t>(std::min<std::int64_t>(像素数量, 4000)),
                static_cast<std::size_t>(4000));
            const auto 置信度 = 限制I64((深度一致率 * 45 + 投影覆盖率 * 35 + 大小评分 * 20) / 100, 0, 10000);
            ...
            候选.投影最小Y = 单元.投影最小Y;
            候选.投影最大Y = 单元.投影最大Y;
            候选.平均深度 = 单元.深度合计 / std::max<std::int64_t>(1, 单元.像素数量);
            候选.最近深度 = 单元.深度最小;
            候选.最远深度 = 单元.深度最大;
            候选.空间连续性评分 = 置信度;
            候选.范围稳定性评分 = 深度一致率;
            ...
            候选.轮廓颜色支持率 = 0;
            候选.轮廓空间支持率 = 投影覆盖率;
            候选.轮廓跨越深度断裂 = 深度一致率 < 6000 ? 1 : 0;
            候选.轮廓穿过深度无效区域 = 0;
            候选.轮廓依赖补全深度 = 0;
            候选.轮廓置信度 = 置信度;
            候选.像素归属验证状态 = 置信度 >= 5000 ? 1 : 0;
            候选.观察确认补偿状态 = 置信度 >= 7000
                ? 观察确认补偿状态_部分确认
                : 观察确认补偿状态_待验证;
            候选.缺失原因 = 深度一致率 < 4000 ? 缺失原因_边界不稳定 : 0;
            候选.低置信轮廓状态 = 置信度 < 7000 ? 1 : 0;
            候选.待补观察区域数量 = 候选.低置信轮廓状态 ? 1 : 0;
            候选.补观察需求建议 = 候选.低置信轮廓状态 ? 补观察建议_内部世界细分 : 0;
        ...
        输出.erase(
            std::remove_if(
                输出.begin(),
                输出.end(),
                [](const auto& 候选) noexcept { return 候选.像素数量 <= 0 || 候选.像素索引集合.empty(); }),
            输出.end());
        for (std::size_t i = 0; i < 输出.size(); ++i) {
            输出[i].候选编号 = static_cast<std::int64_t>(i);
        }

        结果.空间候选数量 = 转换像素计数(输出.size());
        结果.空间候选有效点数量 = 候选像素总数;
        结果.点云有效像素数量 = 候选像素总数;
        结果.空间坐标有效率 = 比例万分比(
        ...
        结果.融合轮廓数量 = 0;
        结果.轮廓来源分层状态 = 2;
        结果.轮廓闭合率 = 输出.empty() ? 0 : 10000;
        结果.轮廓深度支持率 = 输出.empty() ? 0 : 深度支持累计 / static_cast<std::int64_t>(输出.size());
        结果.轮廓颜色支持率 = 0;
        结果.轮廓空间支持率 = 输出.empty() ? 0 : 结果.空间坐标有效率;
        结果.融合轮廓置信度 = 输出.empty() ? 0 : 置信累计 / static_cast<std::int64_t>(输出.size());
        结果.边界深度稳定率 = 结果.轮廓深度支持率;
        结果.候选存在验证通过率 = 比例万分比(
            static_cast<std::size_t>(std::max<std::int64_t>(0, 验证通过数量)),
            输出.size());
        结果.未解释区域比例 = 像素总数 > 0
            ? 比例万分比(
                static_cast<std::size_t>(std::max<std::int64_t>(
                    0,
                    static_cast<std::int64_t>(像素总数) - std::min<std::int64_t>(static_cast<std::int64_t>(像素总数), 候选像素总数))),
                像素总数)
            : 0;
        结果.补观察缺口状态 = 输出.empty() || 结果.轮廓深度支持率 < 4000 ? 1 : 0;
        结果.补观察缺失原因 = 输出.empty() ? 缺失原因_有效点不足 : 0;
        结果.待补观察区域数量 = 0;
        for (const auto& 候选 : 输出) {
            结果.补观察缺失原因 |= 候选.缺失原因;
            结果.待补观察区域数量 += std::max<std::int64_t>(0, 候选.待补观察区域数量);
            if (候选.低置信轮廓状态 > 0) {
                ++结果.低置信轮廓数量;
            }
            if (候选.观察确认补偿状态 == 观察确认补偿状态_待验证) {
                ++结果.待验证候选数量;
            } else if (候选.观察确认补偿状态 == 观察确认补偿状态_部分确认) {
                ++结果.部分确认候选数量;
            }
        ...
        if (结果.补观察缺失原因 != 0) {
            结果.补观察需求建议 = 补观察建议_继续观察未解释区域;
        }
        结果.帧质量评分 = (
            结果.深度有效率 * 35
            + 结果.空间坐标有效率 * 25
            + 结果.候选存在验证通过率 * 25
            + (10000 - std::min<std::int64_t>(10000, 结果.未解释区域比例)) * 15
            ) / 100;
        return 输出;
    }

    // 功能：按稳定深度来源生成轻量观察候选，复用旧多线程网格提取并补三口径深度诊断。
    std::vector<双目相机本能适配器::空间候选摘要> 提取轻量深度网格候选摘要_v2(
        const 结构体_原始场景帧& 帧,
        双目相机本能适配器::调用结果& 结果)
    {
        std::vector<双目相机本能适配器::空间候选摘要> 输出{};
        ...
        const std::size_t 像素总数 = 帧.深度.size();
        const bool 有滤波深度 = 帧.滤波深度.size() == 像素总数;
        const bool 有原始深度 = 帧.原始深度.size() == 像素总数;
        const 结构体_原始场景帧* 分割帧 = &帧;
        结构体_原始场景帧 分割帧副本{};
        if (有滤波深度 || 有原始深度) {
            分割帧副本 = 帧;
            if (有滤波深度) {
                分割帧副本.深度 = 帧.滤波深度;
                分割帧副本.深度有效 = 帧.滤波深度有效.size() == 像素总数
                    ? 帧.滤波深度有效
                    : std::vector<std::uint8_t>{};
            } else {
                分割帧副本.深度 = 帧.原始深度;
                分割帧副本.深度有效 = 帧.原始深度有效.size() == 像素总数
                    ? 帧.原始深度有效
                    : 帧.深度有效;
            }
            分割帧 = &分割帧副本;
        ...
        const auto& 原始深度 = 帧.原始深度.size() == 像素总数 ? 帧.原始深度 : 帧.深度;
        const auto& 原始有效 = 帧.原始深度有效.size() == 原始深度.size() ? 帧.原始深度有效 : 帧.深度有效;
        填充深度毫米结构(原始深度, 原始有效, 结果.原始深度毫米);
        if (帧.滤波深度.size() == 像素总数) {
            填充深度毫米结构(帧.滤波深度, 帧.滤波深度有效, 结果.滤波深度毫米);
        } else {
        ...
        结果.原始深度毫米结构状态 = 结果.原始深度毫米.size() == 像素总数 ? 1 : 0;
        结果.滤波深度毫米结构状态 = 结果.滤波深度毫米.size() == 像素总数 ? 1 : 0;
        结果.补全深度毫米结构状态 = 结果.补全深度毫米.size() == 像素总数 ? 1 : 0;
        结果.原始深度来源像素数量 = 0;
        结果.滤波深度来源像素数量 = 0;
        结果.补全深度来源像素数量 = 0;
        结果.无有效深度来源像素数量 = 0;
        ...
        if (结果.深度稳定性平均值 > 0) {
            结果.边界深度稳定率 = 结果.边界深度稳定率 > 0
                ? (结果.边界深度稳定率 + 结果.深度稳定性平均值 + 结果.深度邻域一致性平均值) / 3
                : (结果.深度稳定性平均值 + 结果.深度邻域一致性平均值) / 2;
            结果.轮廓深度支持率 = 结果.轮廓深度支持率 > 0
                ? (结果.轮廓深度支持率 + 结果.边界深度稳定率) / 2
                : 结果.边界深度稳定率;
            结果.帧质量评分 = (
                结果.深度有效率 * 25
                + 结果.空间坐标有效率 * 20
    ...
    // 功能：根据 D455 轻量观察帧生成稳定深度网格版高频调用结果。
    双目相机本能适配器::调用结果 从轻量观察帧生成结果_v2(结构体_原始场景帧 帧)
    {
        const auto 结果构建开始 = std::chrono::steady_clock::now();
        const auto 计算耗时 = [](auto 起点, auto 终点) noexcept -> std::uint64_t {
            return static_cast<std::uint64_t>(
        ...
        if (预期像素数 == 0 || 帧.深度.size() != 预期像素数) {
            结果.成功 = false;
            结果.原因 = 双目相机本能适配器::失败原因::采集失败;
            结果.消息 = "D455 轻量稳定深度观察帧结构不完整";
            结果.轻量结果构建耗时微秒 = 计算耗时(结果构建开始, std::chrono::steady_clock::now());
            return 结果;
        }

        const auto 候选摘要开始 = std::chrono::steady_clock::now();
        结果.空间候选列表 = 提取轻量深度网格候选摘要_v2(帧, 结果);
        const auto 候选摘要结束 = std::chrono::steady_clock::now();
        结果.轻量候选摘要耗时微秒 = 计算耗时(候选摘要开始, 候选摘要结束);
        if (结果.空间候选列表.empty()) {
            结果.成功 = false;
            结果.原因 = 双目相机本能适配器::失败原因::采集失败;
            结果.消息 = "D455 轻量稳定深度观察帧缺少有效深度候选";
        }
        结果.轻量结果构建耗时微秒 = 计算耗时(结果构建开始, std::chrono::steady_clock::now());
        return 结果;
    }

    // 功能：根据 D455 轻量视频帧生成适配器结果。
    双目相机本能适配器::彩色视频帧 从D455彩色视频帧生成结果(
        const 结构体_D455彩色视频帧& 帧)
    {
        双目相机本能适配器::彩色视频帧 结果{};
        结果.成功 = true;
        结果.相机已打开 = true;
        结果.消息 = "D455 彩色视频帧采集完成";
        结果.宽度 = 帧.宽度;
        结果.高度 = 帧.高度;
        结果.系统到达时间_us = 帧.系统到达时间_us;
        结果.设备时间_us = 帧.设备时间_us;
        结果.时间域 = 帧.时间域;
        结果.深度帧号 = 帧.深度帧号;
        结果.彩色帧号 = 帧.彩色帧号;
        const std::size_t 预期像素数 = 帧.宽度 > 0 && 帧.高度 > 0
        ...
        结果.预期像素数量 = 转换像素计数(预期像素数);
        结果.颜色RGB = 帧.颜色RGB;
        if (预期像素数 == 0 || 结果.颜色RGB.size() != 预期像素数 * 3) {
            结果.成功 = false;
            结果.原因 = 双目相机本能适配器::失败原因::采集失败;
            结果.消息 = "D455 彩色视频帧像素结构不完整";
        }
        return 结果;
    }
}
...
namespace 双目相机本能适配器 {
    ...
    // 功能：按函数名执行对应处理。
    调用结果 打开(bool 使用低延迟观察配置) noexcept
    {
    ...
    彩色视频帧 采集彩色视频帧() noexcept
    {
        ...
        try {
            ...
            if (!g_相机->采集彩色视频帧(帧)) {
                结果.原因 = 失败原因::采集失败;
                结果.消息 = "D455 彩色视频帧采集失败";
                return 结果;
            }
            return 从D455彩色视频帧生成结果(帧);
        }
        catch (const std::exception& e) {
            结果.原因 = 失败原因::采集失败;
            结果.消息 = std::string("D455 彩色视频帧采集异常: ") + e.what();
            return 结果;
        }
        catch (...) {
            结果.原因 = 失败原因::采集失败;
            结果.消息 = "D455 彩色视频帧采集未知异常";
            return 结果;
        }
    }
    ...
    轻量观察原始帧包 采集轻量观察原始帧() noexcept
    {
        ...
        try {
            ...
            结果.原因 = 失败原因::采集失败;
            结果.消息 = std::string("D455 轻量观察帧采集异常: ") + e.what();
            return 结果;
        }
        catch (...) {
            结果.原因 = 失败原因::采集失败;
