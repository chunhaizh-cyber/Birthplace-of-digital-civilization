...
// 文件头部规则注释模块：
// 1. 存在类只维护存在节点主信息、概念集、内部世界和基础存在操作，不新增抽象存在专用结构。
// 2. 概念存在和实例存在靠所在树与概念集关系区分，不用枚举或文本标签作机器判断。
// 3. 名称是显示入口，类型表达抽象归属；实例名称可为空，实例类型应优先来自主概念。
...
namespace {
    ...
    // 功能：从父概念推导子概念的上位抽象归属类型。
    const 语素入口节点类* 私有_概念上位归属类型(
        const 存在类& 存在服务,
        const 基础信息节点类* 概念父节点) noexcept
    {
        const auto* 父存在 = dynamic_cast<const 存在节点类*>(概念父节点);
        const auto* 父主信息 = 存在服务.取存在主信息(父存在);
        if (!父主信息) return nullptr;
        return 父主信息->名称 ? 父主信息->名称 : 父主信息->类型;
    }

    // 功能：从概念集推导实例存在的主抽象归属类型。
    const 语素入口节点类* 私有_实例主抽象类型(
        const 存在类& 存在服务,
        const std::vector<存在节点类*>& 概念集) noexcept
    {
        for (const auto* 概念 : 概念集) {
            const auto* 概念主信息 = 存在服务.取存在主信息(概念);
            if (!概念主信息) continue;
            if (概念主信息->名称) return 概念主信息->名称;
            if (概念主信息->类型) return 概念主信息->类型;
        }
        return nullptr;
    }
...
// 功能：判断存在节点是否为实例存在，不创建或修改节点。
bool 存在类::是实例存在(const 存在节点类* 节点, const 存在节点类* 存在概念根) const noexcept
{
    return 节点
        && 是存在节点(节点)
        && !是存在概念(节点, 存在概念根);
...
// 功能：按相对坐标表选择第一条可用参考并换算场景绝对坐标。
结构_存在空间绝对坐标换算结果 存在类::计算场景绝对坐标_由相对坐标表(
    ...
    const 语素入口节点类* 名称)
{
    ...
    return 创建存在_按名称类型(概念父节点, 名称, 私有_概念上位归属类型(*this, 概念父节点));
}
...
// 功能：按相对坐标表写入目标存在场景绝对坐标和坐标来源。
bool 存在类::写入存在场景绝对坐标_由相对坐标表(
    存在节点类* 目标存在,
    ...
    时间戳 now)
{
    ...
    auto* 目标场景 = 基础信息_ ? 私有_取存在所在场景(*基础信息_, 目标存在) : nullptr;
    auto* 参考场景 = 基础信息_ ? 私有_取存在所在场景(*基础信息_, 换算结果.参考存在) : nullptr;
